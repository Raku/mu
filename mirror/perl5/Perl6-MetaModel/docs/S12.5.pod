...
Calling C<.meta> on either a class or an instance of the same class should return an 
instance of the same C<Perl6::MetaClass> object. Each class is associated with one 
metaclass instance which describes that class. The following are the methods which can 
be called on this metaclass instance.
...
is associated with. Its default value is C<undef>, which is essentially an anonymous class.

=item C<version>

Also a read-write accessor method, this is the version number of the class the metaclass 
is associated with. Its default value is C<0.0.0>.
...
them into a single string seperated by '-' characters. This is the format found in the class 
defintions:
