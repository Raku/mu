...
class Algorithm::TokenBucket;
use v6;

try { use Time::HiRes <time> } # use if available
# XXX -- will need later fixing, probably Time::HiRes is included in the core
# of Pugs/Parrot/whatever
...
=head1 SYNOPSIS
    ...
    my Algorithm::TokenBucket $bucket .= new(
      rate => 100 / 3600,
      burst_size => 5
    );
...
=item new(Num $rate, Int $burst_size)
...
has Num $.rate;
has Int $.burst_size;
has Num $:last_check_time;
has Int $:tokens;

submethod BUILD($.rate, $.burst_size) {
...
method conform(Num $n) returns Bool {
  .:token-flow();
  return $:tokens >= $n;
}
...
Go, go, go!
    ...
    my Algorithm::TokenBucket $rl1 .= new(rate => 2/60,    burst_size => 1);
    my Algorithm::TokenBucket $rl2 .= new(rate => 20/3600, burst_size => 10);
