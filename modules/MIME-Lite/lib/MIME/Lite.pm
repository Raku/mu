...
method :gen_boundary() { "_----------=_" ~ int(time) ~ $*PID ~ $BCount++ }
...
method :known_field(Str $field is copy) {
...
method :is_mime_field(Str $field) { $field ~~ m:i/^(mime\-|content\-)/ }
...
rule atom      { <-[\000-\037()<>@,;:\134"\056\133\135]>+ } #"#--vim
rule qstr      { ".*?" }
rule word      { [ <qstr> | <atom> ] }
rule domain    { [ <atom> [\.<atom>]* ] }
rule localpart { [ <word> [\.<word>]* ] }
rule addr      { [ <localpart> @ <domain> ] }
rule phrase    { <word>+ }
rule sep       { [ ^\s* | \s*,\s* ] }      ### before elems in a list

method :extract_addrs(Str $addr) { map { $_.format } Mail::Address.parse($addr) }
...
method :encode_8bit(Str $str is copy) { $str ~~ s:g/^(.**{990})/$1\n/; $str }
...
method attach(MIME::Lite | Hash *$part) {
  my $part1 = $part ~~ Hash
    ?? ::?CLASS.new(top => 0, *$part)
    :: $part;

  ### Do the "attach-to-singlepart" hack:
  unless .attr("content-type") ~~ m:i{^(multipart|message)/}) {
    ### Create part zero:
    my $part0 = ::?CLASS.new;

    ### Cut MIME stuff from self, and paste into part zero:
    for <:attrs :data :path fh> {
      $part0.$_ = .$_; # XXX - correct? (don't think so)
      .$_ = undef;
    }
    ...
    %:attrs = ();   ### reset
    .attr('content-type')              = 'multipart/mixed';
    .attr('content-type.boundary')     = gen_boundary();
    .attr('content-transfer-encoding') = '7bit';
    .top_level = 1;     ### activate top-level attributes
...
has (%:attrs, @:header, @.parts);
has Str ($:data, $:path);
has $.fh;
has @:fieldorder;
has Str $.preamble is rw;

submethod BUILD(
  %:attrs, @:header, @.parts,
  Str ?$path, ?$fh, Str ?$data, Str ?$type,
  Str ?$id,
  Bool ?$readnow,
  Str ?$filename,
  Str ?$encoding,
  ...
  die "Supply exactly zero or one of (Data|Path|FH).\n"
    unless defined $path & $fh & $data;

  ### CONTENT-TYPE....
  ###

  ### Get content-type or content-type-macro:
  my $type ||= $AUTO_CONTENT_TYPE ?? 'AUTO' :: 'TEXT';

  ### Interpret content-type-macros:
  given $type {
    when "TEXT"   { $type = 'text/plain' }
    when "BINARY" { $type = 'application/octet-stream' }
    when "AUTO"   { $type = .:suggest_type($path) }
  }

  ### We now have a content-type; set it:
  $type .= lc;
  .attr('content-type') = $type;

  ### Get some basic attributes from the content type:
  my $is_multipart = ($type ~~ m:i{^(multipart)/});

  ### Add in the multipart boundary:
  if $is_multipart {
    my $boundary = .:gen_boundary;
    .attr('content-type.boundary') = $boundary;
  }

  ### CONTENT-ID...
  ###
  .attr('content-id') = $id if defined $id;
  ...
  ### Miko's note: this part works much like the path routine just above,
    ...
    $.fh = $fh;
    .:read_now if $readnow;  ### implement later
  }
  ...
  my $enc =
    $encoding ||
    $AUTO_ENCODE and .:suggest_encoding($type) ||
    'binary';
  .attr('content-transfer-encoding') = lc $enc;

  ### Sanity check:
  if $type ~~ m{^(multipart|message)/} {
    $enc ~~ m{^(7bit|8bit|binary)$} or
      die "Illegal MIME: Can't have encoding \"$enc\" with type \"$type\"\n";
  }

  ### CONTENT-DISPOSITION...
  ###    Default is inline for single, none for multis:
  ###
  my $disp = $disposition or ($is_multipart ?? undef : 'inline');
  .attr('content-disposition') => $disp;

  ### CONTENT-LENGTH...
  ###
  if defined $length {
    .attr('content-length') = $length;
  }
  ...
  my $ds_defaulted = $is_top && !$datestamp;
  if ($datestamp or $ds_defaulted) and not $date {
    # XXX - Perl 6 way?
  ...
  ### Set message headers:
  my @paramz = @params; # XXX XXX XXX
  my $field;
  while @paramz {
    my ($tag, $value) = (shift @paramz, shift @paramz);

    ### Get tag, if a tag:
    if $tag ~~ m/^-(.*)/ {      ### old style, backwards-compatibility
      $field = lc $1;
    } elsif $tag ~~ m/^(.*):$/ {  ### new style
      $field = lc $1;
...
method top_level() is rw {
  return new Proxy:
    FETCH => { ... },
    STORE => -> Bool $onoff {
      if $onoff {
	.attr('MIME-Version') = '1.0';
	.replace('X-Mailer')  = "MIME::Lite";
      } else {
	.attr('MIME-Version') = undef;
	.delete('X-Mailer');
      }
    };
}
...
method add(Str $tag is copy, Str *@values is copy) {
  $tag .= lc;

  ### If a dangerous option, warn them:
  die "Explicitly setting a MIME header field ($tag) is dangerous:\n" ~
...
sub attr(Str $attr is copy) is rw {
  $attr .= lc;

  ### Break attribute name up:
  my ($tag, $subtag) = split m/\./, $attr;
  $subtag //= "";

  ### Set or get?
  return new Proxy:
    FETCH => { %:attrs{$tag}{$subtag} },
    STORE => -> $value is copy {
      delete %:attrs{$tag}{$subtag};   ### delete first
      if defined $value {                  ### set...
	$value ~~ s:g/<[\r\n]>//;                   ### make clean
	%:attrs{$tag}{$subtag} = $value;
      }
    };
}

method :safe_attr(Str $attr) { .attr($attr) // "" }
...
method delete(::?CLASS $self: Str $tag is copy) {
  $tag .= lc;
...
method field_order(Class|::?CLASS $invoc:) is rw {
  return new Proxy:
    FETCH => { ... },
    STORE => -> Str *@values {
      if $invoc ~~ Class {
	@FieldOrder  = @values>>.lc;
      } else {
	@:fieldorder = @values>>.lc;
      }
    };
...
method filename() is rw {
  return new Proxy:
    FETCH => { .attr("content-disposition.filename") },
    STORE => {
      .attr('content-type.name')            = $^filename;
      .attr('content-disposition.filename') = $^filename;
    };
}
...
method get(Str $tag is copy, Int ?$index) {
  $tag .= lc;
  die "get: Can't be used with MIME fields\n" if .:is_mime_field($tag);

  my @all = map { $_[0] eq $tag ?? $_[1] :: () } @:header;
  defined $index
    ?? @all[$index]
    :: @all;
}
...
method get_length() {
  ...
  my $is_multipart = (.attr('content-type') ~~ m:i{^multipart/});
  my $enc          = lc(.attr('content-transfer-encoding') || 'binary');

  my $length;
  if !$is_multipart && $enc eq "binary" {  ### might figure it out cheap:
    if defined $:data {               ### it's in core
      $length = length $:data;
    } elsif defined $.fh {                 ### it's in a filehandle
      ### no-op: it's expensive, so don't bother
    } elsif defined $:path {               ### it's a simple file!
      $length = -s $:path if -e $:path;
    }
  }
...
method parts_DFS(::?CLASS $self) {
  return (
    $self,
    map { $_.parts_DFS } @.parts,
  );
}
...
method replace(Str $tag) is rw {
  return new Proxy:
    FETCH => {...},
    STORE => {
      .delete($tag);
      .add($tag, $^value) if defined $^value;
    };
}
...
method scrub(*@a) {
  ...
  unless @a {         ### guess
    ...
    if
      !.:safe_attr('content-disposition.filename') ||
      .:safe_attr('content-type.name') {
      .replace('content-disposition') = "";
    }

    ### Scrub encoding if effectively unencoded:
    if .:safe_attr('content-transfer-encoding') ~~ m:i/^(7bit|8bit|binary)$/) {
      .replace('content-transfer-encoding') = "";
    }

    ### Scrub charset if US-ASCII:
    if .:safe_attr('content-type.charset') ~~ m:i/^(us-ascii)/ {
      .attr('content-type.charset') = undef;
    }

    ### TBD: this is not really right for message/digest:
    if
      keys %:attrs<content-type> == 1 and
      .:safe_attr('content-type') eq "text/plain"
    {
      .replace('content-type') = '';
    }
  } elsif $expl and $expl ~~ Array {
    for $expl -> { .replace($_) = '') }
  }
...
method sign(Str ?$path, Str ?$data) {
  $path //= "$*ENV{HOME}/.signature";

  ### Force message in-core:
  defined $:data or .:read_now;

  ### Load signature:
  my $sig;
  unless defined($sig = $data) {      ### not given explicitly:
    $sig = slurp $path or die "Couldn't slurp \"$path\": $!\n";
  }
