...
class URI {
  ...
  rule reserved   { <[;/?:@&=+$,\[\]]>               };
  rule mark       { <[-_.!~*'()]>                    };   #'#; vim, emacs :)
  rule unreserved { rule { <[A-Za-z0-9]+<mark>>      };
  rule uric       { <+<reserved>+<unreserved>+["%"]> };
  rule scheme     { <[a-zA-Z]><[a-zA-Z0-9.+-]>*      };

  subtype Str::Scheme of Str where { $^scheme ~~ m/^<scheme>$/ };
  ...
  submethod BUILD($uri is copy, ?$scheme is copy) {
    ...
    if $uri =~ m/^(<scheme>):/ {
      ...
      if $scheme.meta.isa(URI) {
	# XXX: get class of object -- how?
	$impclass = $scheme.XXX_get_class;
	$scheme = $scheme.scheme;
      } elsif $scheme && $scheme ~~ m/^(<scheme_re>)[:|$]/) {
	$scheme = $1;
      }
    }
  ...
  method new_abs(Str $uri, Str $base) {
    my $uri = .new($uri, $base);
    $uri.abs($base);
  }

  method :init(::?CLASS $class: Str $str, Str $scheme) {
    $str ~~ s:g/(<-<uric>-[#]>)/%URI::Escape::escapes{$1}/;
    $str = "$scheme:$str" unless $str ~~ /^<scheme>:/ ||
                                 $class.:no_scheme_ok;

    return bless $class: str => $str;
  }

  method :implementor(Str $scheme, Class ?$impclass) {
    if !$scheme || not $scheme ~~ /^<scheme_re>$/) {
      return use URI::_generic;
    }
    ...
    if $impclass {
      ...
      $impclass.:init_implementor($scheme);
    ...
    $ic = use $ic;
    $ic.:init_implementor($scheme);
    %implements{$scheme} = $ic;
    return $ic;
  }

  method :init_implementor(Str $scheme) {
    # Remember that one implementor class may actually
    # serve to implement several URI schemes.
  }

  method :no_scheme_ok() { 0 }

  method :scheme() is rw {
    return new Proxy:
      FETCH => {
	return unless $:str ~~ /^(<scheme>):/;
	return $1;
      },
      STORE => {
	my $old;
	if length $^new {
	  $old = $1 if $:str ~~ s/^(<scheme>)://;
	  my $newself = new URI: uri => "$^new:$:str";
	  $:str = $newself.:str; 
	  bless .XXX_GET_CLASS: str => $:str; # XXX
	} else {
	  if $self.:no_scheme_ok {
	    $old = $1 if $.str ~~ s/^(<scheme>://;
	    die "Oops, opaque part now look like scheme"
	      if $:str ~~ m/^<scheme>:/;
	  } else {
	    $old = $1 if $:str ~~ m/^(<scheme>):/;
	  }
	}
  ...
  method opaque() is rw {
	...
	return $1;
      ...
      STORE => -> $new_opaque is copy {
	my ($old_scheme, $old_opaque, $old_frag) =
	  $:str ~~ /^(<scheme>:)?    # optional scheme
		     (<-[\#]>*)      # opaque
		     (\#.*)?         # optional fragment
		    $/ or die;

	$new_opaque //= "";
	$new_opaque ~~ s:g/(<-<uric>>)/%URI::Escape::escapes{$1}/;

	$:str  = $old_scheme // "";
	$:str ~= $new_opaque;
	$:str ~= $old_frag if defined $old_frag;
  ...
  *path = \&opaque;  # alias
  ...
  method fragment() is rw {
	...
	return $1;
      ...
      STORE => -> $new_frag is copy {
	my $old;
	$old = $1 if $:str ~~ s/\#(.*)//;

	$new_frag ~~ s:g/(<-<uric>>)/%URI::Escape::escapes{$1}/;
	$:str ~= "#$new_frag";
  ...
  multi sub *coerce:<as> (::?CLASS $self, Str ::to) { $:str }

  method canonical(::?CLASS $self:) {
    # Make sure scheme is lowercased, that we don't escape unreserved chars,
    # and that we use upcase escape sequences.

    my $scheme    = $.:scheme || "";
    my $uc_scheme = $scheme ~~ m/[A-Z]/;
    my $esc       = $:str ~~ m/%<[a-fA-F0-9]>{2}/;
    return $self unless $uc_scheme or $esc;
    ...
    if $esc { # XXX
      $other.:str ~=~ s:g/%(<[0-9a-fA-F]>{2})/{
	my $a = chr :16($1); # XXX - correct?
	$a ~~ m/^<unreserved>$/ ?? $a :: "%{uc $1}"
      /; #/# -- vim
    }
  ...
  multi sub eq($self is copy, $other is copy) {
      $self  = new URI: uri => $self, scheme => $other unless $self ~~ URI;
      $other = new URI: uri => $other, scheme => $self unless $other ~~ URI;
      ~$self.canonical eq ~$other.canonical;
  }

  # generic-URI transformation methods
  method abs(::?CLASS $self:) { $self }
  method rel(::?CLASS $self:) { $self }
...
=head1 SYNOPSIS
 ...
 # XXX: Documentation still for Perl 5!
