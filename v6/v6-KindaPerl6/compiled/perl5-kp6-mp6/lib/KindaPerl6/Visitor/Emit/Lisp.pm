...
sub set_secure_mode {
    ...
    do {
        if ( ( $args_secure != 0 ) ) {
            return (
                ( '(pushnew :kp6-lisp-secure *features*)' . Main::newline() ) );
        }
        else { return ('') }
      }
}
...
sub emit_lisp {
    ...
    ( '(make-instance \'kp6-Str :value '
          . ( '"' . ( Main::mangle_string( $self->{buf} ) . ( '"' . ')' ) ) ) );
}
...
sub emit_lisp {
    ...
    (
        '(kp6-error '
          . (
            $interpreter . ' \'kp6-not-implemented :feature "literal objects")'
          )
    );
}
    ...
    (
        '(list '
          . (
            Main::join(
                [
                    map { $_->emit_lisp( $interpreter, $indent ) }
                      @{ $self->{seq} }
                ],
                ' '
              )
                      ...
                      . (
                        $interpreter
                          . (
                            ' :store '
                              . (
                                $field->[0]->emit_lisp( $interpreter, $indent )
                                  . (
                                    ' '
                                      . (
                                        $field->[1]
...
sub emit_lisp {
    ...
    (
        '(make-instance \'kp6-pair :key '
          . (
            $self->{key}->emit_lisp( $interpreter, $indent )
              . (
                ' :value '
                  . ( $self->{value}->emit_lisp( $interpreter, $indent ) . ')' )
              )
          )
    );
}
...
sub emit_body {
    ...
    Main::join(
        [ map { $_->emit_lisp( $interpreter, $indent ) } @{ $self->{body} } ],
        Main::newline() );
}
...
sub emit_lisp {
    ...
    (
        '(kp6-lookup '
          . (
            $self->{obj}->emit_lisp( $interpreter, $indent )
              . (
                ' (perl->cl '
                  . (
                    $self->{index}->emit_lisp( $interpreter, $indent ) . '))' )
              )
          )
    );
}
...
sub emit_lisp {
    ...
    do {
        if ( Main::isa( $node, 'Var' ) ) {
            return (
                $node->emit_lisp_assignment(
                    $self->{arguments}->emit_lisp( $interpreter, $indent )
                )
            );
        }
        else { }
    ...
    do {
        if (
            (
                ( Main::isa( $node, 'Lookup' ) || Main::isa( $node, 'Index' ) )
                && Main::isa( $node->obj(), 'Var' )
            )
          )
        {
            return (
    ...
    do {
        if (
            (
                Main::isa( $node, 'Call' )
                && (
                    Main::isa( $node->invocant(), 'Var' )
                    && (   ( $node->method() eq 'INDEX' )
                        || ( $node->method() eq 'LOOKUP' ) )
                )
            )
          )
        {
...
sub emit_lisp {
    ...
    do {
        if ( Main::isa( $self->{arguments}, 'Var' ) ) {
            return (
                $self->{parameters}->emit_lisp_assignment(
                    $self->{arguments}->emit_lisp_lookup(1), 1
                )
            );
        }
        else { }
    ...
    do {
        if ( Main::isa( $self->{arguments}, 'Sub' ) ) {
            return (
                $self->{parameters}->emit_lisp_assignment(
                    $self->{arguments}->emit_lisp( $interpreter, $indent )
                )
...
sub emit_lisp {
    ...
    do {
        if (
            (
                Main::isa( $self->{invocant}, 'Var' )
                && (   ( $self->{method} eq 'LOOKUP' )
                    || ( $self->{method} eq 'INDEX' ) )
            )
          )
        {
            return (
                (
                    '(kp6-dispatch '
                      . (
                        $self->{invocant}->emit_lisp( $interpreter, $indent )
                          . (
                            ' '
                              . (
                                $interpreter
    ...
    do {
        if ( Main::isa( $self->{invocant}, 'Proto' ) ) {
            ...
            $invocant = $self->{invocant}->emit_lisp( $interpreter, $indent );
        }
    };
    do {
                    ...
                    '(make-instance \'kp6-Bit :value (or (kp6-true ' . (
                        $apply_func . (
                            Main::join(
                                [
                                    map {
                                        $_->emit_lisp( $interpreter, $indent )
                                      } @{ $self->{arguments} }
                                ],
                                '))) (kp6-true '
                              )
                                    ...
                                    $interpreter
                                      . (
                                        ' :true) '
                                          . (
                                            $interpreter
                                              . ' :cl-landish) (kp6-dispatch (kp6-dispatch '
                                          )
                                      )
                                  )
                            )
...
'(make-instance \'kp6-Bit :value (or (kp6-dispatch (kp6-dispatch '
    ...
    my $str =
      ( '(kp6-apply-function '
          . ( $interpreter . ( ' (perl->cl ' . ( $op . ') (list' ) ) ) );
    do {
        for my $arg ( @{ $self->{arguments} } ) {
            $str =
              ( $str . ' (make-instance \'kp6-positional-parameter :value ' );
            do {
                if ( Main::isa( $arg, 'Var' ) ) {
                    ...
                    $str = (
                        $str
                          . (
                            '(make-kp6-cell '
                              . (
                                $arg->emit_lisp( $interpreter, $indent ) . ')'
                              )
                          )
                    );
                }
...
sub emit_lisp {
    ...
    return (
        (
            'return('
              . ( $self->{result}->emit_lisp( $interpreter, $indent ) . ')' )
        )
    );
}
                ...
                $str
                  . (
                    ' (make-instance \'kp6-named-parameter :name '
                      . ( $decl->emit_lisp_name( $interpreter, $indent ) . ')' )
                  )
            );
        }
    };
...
sub emit_lisp {
        ...
        if ( defined( $self->{array} ) ) {
            ...
            do {
                for my $item ( @{ $self->{array} } ) {
                    $s =
                      ( $s
                          . ( $item->emit_lisp( $interpreter, $indent ) . ', ' )
                      );
                }
            };
            $s = ( $s . ' ] } ),' );
    ...
    do {
        if ( defined( $self->{hash} ) ) {
            ...
            my $item;
            do {
                for my $item ( @{ $self->{hash} } ) {
                    $s = (
                        $s
                          . (
                            $item->[0]->emit_lisp( $interpreter, $indent )
                              . (
                                '->{_value} => '
                                  . (
                                    $item->[1]->emit_lisp($interpreter) . ', '
                                  )
                              )
                          )
                    );
...
sub emit_lisp {
    ...
    return (
        (
            '(kp6-error '
              . ( $interpreter . ' \'kp6-not-implemented :feature "subsets")' )
        )
    );
}
...
sub emit_lisp {
    ...
    return (
        (
            '(kp6-error '
              . ( $interpreter . ' \'kp6-not-implemented :feature "methods")' )
        )
    );
}
...
sub emit_lisp {
    ...
    return (
        (
            '(make-kp6-sub ('
              . (
                $self->{block}->emit_signature( $interpreter, $indent )
                  . (
                    ')'
                      . (
                        Main::newline()
                          . (
                            $self->{block}->emit_body( $interpreter, $indent )
                              . ')'
                          )
                      )
                  )
...
sub emit_lisp {
    ...
    return (
        (
            '(kp6-error '
              . (
                $interpreter . ' \'kp6-not-implemented :feature "BEGIN blocks")'
              )
        )
    );
}
