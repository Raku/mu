...
sub new { shift; bless {@_}, "KindaPerl6::Visitor::Emit::Parrot" }
...
sub emit_parrot {
    ...
    my $List__ = \@_;
    do { [] };
    my $s = ( '  save $P1' . Main::newline() );
    $s = ( $s . $self->{obj}->emit_parrot() );
    $s = ( $s . ( '  $P1 = $P0' . Main->newline() ) );
    $s = ( $s . $self->{index}->emit_parrot() );
    $s = ( $s . ( '  $P0 = $P1[$P0]' . Main->newline() ) );
    my $s = ( $s . ( '  restore $P1' . Main::newline() ) );
    return ($s);
}
...
sub emit_parrot {
    ...
    my $id = $label;
    do {
        if ( ( Main::isa( $cond, 'Var' ) && ( $cond->sigil() ne '@' ) ) ) { $cond = Lit::Array->new( 'array' => [$cond], ) }
        else                                                              { }
    };
    return (
        (   ''
                                                                                                    ...
                                                                                                    '  goto test_iter'
                                                                                                        ...
                                                                                                        $id
                                                                                                            ...
                                                                                                            Main::newline()
                                                                                                                . (
                                                                                                                ' iter_done'
                                                                                                                    . ( $id . ( ':' . ( Main::newline() . ( '  restore $P2' . ( Main::newline() . ( '  restore $P1' . ( Main::newline() . '' ) ) ) ) ) ) )
                                                                                                                )
                                                                                                            )
                                                                                                        )
                                                                                                    )
