...
sub visitor_args {
    @_ == 1 ? ( $_[0]->{visitor_args} ) : ( $_[0]->{visitor_args} = $_[1] );
}
        ...
        for my $field ( @{ $self->{sig}->positional() } ) {
            my $bind_named = Bind->new(
                ...
                'arguments'  => Call->new(
                    'invocant' => $hash_,
                    'arguments' =>
                      [ Val::Buf->new( 'buf' => $field->key()->name(), ) ],
                    'method' => 'LOOKUP',
                ),
            );
...
sub emit_perl5 {
    ...
    my $str =
      ( '::MODIFIED('
          . ( $self->{parameters}->emit_perl5() . ( ');' . Main::newline() ) )
      );
    $str = (
        $str
          . (
            $self->{parameters}->emit_perl5()
              . ( ' = ' . $self->{arguments}->emit_perl5() )
          )
    );
    return ( ( 'do {' . ( $str . '}' ) ) );
...
sub emit_perl5 {
    ...
    my $call =
      Main::join( [ map { $_->emit_perl5() } @{ $self->{arguments} } ], ', ' );
    do {
        if ( $self->{hyper} ) {
            (
...
sub emit_perl5 {
    ...
    return (
        (
            'return('
              . ( $self->{result}->emit_perl5() . ( ')' . Main::newline() ) )
        )
    );
}
...
'}'
            ...
            return (
                ( $s . ( $self->{var}->emit_perl5() . Main::newline() ) ) );
        }
        else { }
    };
    do {
...
sub emit_perl5 {
    ...
    my $inv = '$::Undef';
    do {
        if ( Main::isa( $self->{invocant}, 'Var' ) ) {
            $inv = Main::perl( $self->{invocant}, );
        }
        else { }
    };
    my $pos;
...
sub emit_perl5 {
    ...
    do {
        if ( defined( $self->{invocant} ) ) {
            $s = (
                $s
                  . (
                    'invocant => ' . ( $self->{invocant}->emit_perl5() . ', ' )
                  )
            );
        }
    ...
    do {
        if ( defined( $self->{array} ) ) {
            $s =
              ( $s . 'array => ::DISPATCH( $::Array, "new", { _array => [ ' );
            my $item;
            do {
