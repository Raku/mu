...
package KindaPerl6::Visitor::Emit::SMOPGrammar;
sub new { shift; bless {@_}, "KindaPerl6::Visitor::Emit::SMOPGrammar" }
...
sub emit_c {
    ...
    ( '/*' . ( $self->{name} . ( '*/' . ( Main::newline() . $self->{body}->emit_c() ) ) ) );
}
...
sub emit_c {
    ...
    my $List__ = \@_;
    do { [] };
    ( '(strncmp("' . ( $self->{constant} . ( '",str+pos,' . ( length( $self->{constant} ) . ( ') == 0 && (pos += ' . ( length( $self->{constant} ) . '))' ) ) ) ) ) );
}
...
sub emit_c {
    ...
    my $List__ = \@_;
    do { [] };
    ( 'printf("Rule::Block stub:' . ( $self->{closure} . '\n")' ) );
}
