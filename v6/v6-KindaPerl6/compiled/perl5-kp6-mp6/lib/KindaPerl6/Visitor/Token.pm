            ...
            my $source       = (
                'method '
                    ...
                    $node->name()
                        ...
                        ' ( $str, $pos ) { '
                            . (
                            'if (%*ENV{"KP6_TOKEN_DEBUGGER"}) { say ">>> token '
                                . (
                                $node->name()
                                    . (
                                    ' at " ~ $pos ~ " of (" ~ $str ~ ")"; };'
                                            ...
                                            '$MATCH = Match.new(); $MATCH.match_str = $str; $MATCH.from = $pos; $MATCH.to = ($pos + 0); $MATCH.bool = 1; '
                                                ...
                                                '$MATCH.bool = '
                                                    ...
                                                    $perl6_source
                                                        ...
                                                        '; '
                                                            . (
                                                            'if (%*ENV{"KP6_TOKEN_DEBUGGER"}) { if ($MATCH.bool) { say "<<< token '
                                                                . ( $node->name() . ( ' returned true to ("~$MATCH.to~")"; } else {say "<<< token ' . ( $node->name() . ( ' returned false "; } };' . 'return $MATCH }' ) ) ) )
                                                            )
                                                        )
                                                    )
                                                )
...
sub constant {
    ...
    do {
        if ($len) {
            (   'do {if (chars($str) <  '
                    . (
                    $len . ( ') {(0)} else { if (' . ( Main::singlequote() . ( $str . ( Main::singlequote() . ( ' eq substr($str, $MATCH.to, ' . ( $len . ( ')) {' . ( '$MATCH.to = (' . ( $len . ' + $MATCH.to);  1;} else {(0)}}}' ) ) ) ) ) ) ) ) )
                    )
            );
        }
        else { return ('1') }
...
sub emit_token {
    ...
    my $List__ = \@_;
    do { [] };
    ( 'do { ' . ( 'my $pos1 = ($MATCH.to + 0); do{ ' . ( Main::join( [ map { $_->emit_token() } @{ $self->{or} } ], '} || do { $MATCH.to = ($pos1 + 0); ' ) . '} }' ) ) );
}
...
sub emit_token {
    ...
    my $List__ = \@_;
    do { [] };
    ( 'do { my $m2 = match_p5rx("' . ( $self->{regex} . '",$str,($pos+0)); if ($m2) { $MATCH.to = $m2.to + 0; 1 } else { 0 } }' ) );
}
...
sub emit_token {
    ...
    my $List__ = \@_;
    do { [] };
    ( '(' . ( Main::join( [ map { $_->emit_token() } @{ $self->{concat} } ], ' && ' ) . ')' ) );
}
...
sub emit_token {
    ...
    do { [] };
    my $meth = ( ( 1 + index( $self->{metasyntax}, '.' ) ) ? $self->{metasyntax} : ( 'self.' . $self->{metasyntax} ) );
    ( 'do { ' . ( 'my $m2 = ' . ( $meth . ( '($str, $MATCH.to); ' . ( 'if $m2 { $MATCH.to = ($m2.to + 0); 1 } else { 0 } ' . '}' ) ) ) ) );
}
...
sub emit_token {
    ...
    my $List__ = \@_;
    do { [] };
    ( 'do { if (\'\' ne substr( $str, $MATCH.to, 1 )) {' . ( '   ($MATCH.to = (1 + $MATCH.to )); 1 } else {' . ( '   0 } ' . '}' ) ) );
}
...
sub emit_token {
    ...
    my $List__ = \@_;
    do { [] };
    Main::say( ( '# TODO: interpolate var ' . ( $self->{var}->emit_token() . '' ) ) );
    die();
}
...
sub emit_token {
    ...
    my $List__ = \@_;
    do { [] };
    Main::say( ( '# TODO: named capture ' . ( $self->{ident} . ( ' := ' . ( $self->{rule}->emit_token() . '' ) ) ) ) );
    die();
}
            ...
            return (
                (   'do { '
                        ...
                        'my $MATCH; '
                            . (
                            '$MATCH = Match.new(); $MATCH.match_str = $str; $MATCH.from = $pos; $MATCH.to = ($pos + 0); $MATCH.bool = 1; '
                                . ( '$MATCH.bool = !(' . ( $self->{rule}->emit_token() . ( '); $MATCH.to = ($MATCH.from + 0); ' . ( '$MATCH.bool; ' . '}' ) ) ) )
                            )
                        )
                )
            );
