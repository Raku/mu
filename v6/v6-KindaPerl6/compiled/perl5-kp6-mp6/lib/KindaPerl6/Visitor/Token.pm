...
sub emit_token {
    ...
    do { [] };
    do {
        if ( ( substr( $self->{metasyntax}, 0, 1 ) eq Main::singlequote() ) ) {
            return (
                Rule::constant(
                    substr(
                        substr( $self->{metasyntax}, 1 ),
                        0,
                        ( Main::chars( $self->{metasyntax}, ) - 2 )
                    )
                )
            );
        }
...
sub emit_token {
    ...
    do {
        if ( ( $char eq 'N' ) ) {
            my $rul =
              Rule::SubruleNoCapture->new( 'metasyntax' => 'not_newline', );
            $rul = $rul->emit_token();
            return ($rul);
        }
        else { }
...
sub emit_token {
    ...
    do { [] };
    Main::say(
        ( '# TODO: interpolate var ' . ( $self->{var}->emit_token() . '' ) ) );
    die();
}
