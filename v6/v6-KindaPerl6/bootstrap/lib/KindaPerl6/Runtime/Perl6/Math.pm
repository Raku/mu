...
do { if (::DISPATCH(::DISPATCH(::DISPATCH( $GLOBAL::Code_VAR_defined, 'APPLY', $::Math )
,"true"),"p5landish") ) { }  else { do {::MODIFIED($::Math);
...
; ::DISPATCH( ::DISPATCH( $::Math, 'HOW',  )
, 'add_method', ::DISPATCH( $::Str, 'new', 'zzz' )
, ::DISPATCH( $::Method, 'new', sub  { my $ret; $ret = ::DISPATCH( $::Scalar, 'new', { modified => $_MODIFIED, name => '$ret' } )  unless defined $ret; BEGIN { $ret = ::DISPATCH( $::Scalar, 'new', { modified => $_MODIFIED, name => '$ret' } ) }
;
...
::DISPATCH_VAR($CAPTURE,"STORE",::CAPTURIZE(\@_));do {::MODIFIED($List__);
$List__ = ::DISPATCH( $CAPTURE, 'array',  )
};::DISPATCH_VAR( $ret, 'STORE', ::DISPATCH( $::Int, 'new', 42 )
 )
 }
 )
 )
