...
package KindaPerl6::Visitor::EmitHTML; sub new { shift; bless { @_ }, "KindaPerl6::Visitor::EmitHTML" } sub visit { my $self = shift; my $List__ = \@_; my $node; do {  $node = $List__->[0]; [$node] }; $node->emit_html() }
;
package CompUnit; sub new { shift; bless { @_ }, "CompUnit" } sub emit_html { my $self = shift; my $List__ = \@_; do { [] }; ('<font color=blue>{ module ' . ($self->{name} . ('; ' . ($self->{body}->emit_html() . ' }</font><br>')))) }
;
package Val::Int; sub new { shift; bless { @_ }, "Val::Int" } sub emit_html { my $self = shift; my $List__ = \@_; do { [] }; ('<font color=red>' . ($self->{int} . '</font>')) }
;
...
;
package Val::Buf; sub new { shift; bless { @_ }, "Val::Buf" } sub emit_html { my $self = shift; my $List__ = \@_; do { [] }; ('<font color=green>' . ('\'' . ($self->{buf} . ('\'' . '</font>')))) }
;
package Val::Undef; sub new { shift; bless { @_ }, "Val::Undef" } sub emit_html { my $self = shift; my $List__ = \@_; do { [] }; '(undef)' }
;
...
;
package Lit::Hash; sub new { shift; bless { @_ }, "Lit::Hash" } sub emit_html { my $self = shift; my $List__ = \@_; do { [] }; my  $fields = $self->{hash}; my  $str = ''; do { for my $field ( @{$fields} ) { $str = ($str . ($field->[0]->emit_html() . (' => ' . ($field->[1]->emit_html() . ',')))) } }; ('{ ' . ($str . ' }')) }
;
...
;
package Assign; sub new { shift; bless { @_ }, "Assign" } sub emit_html { my $self = shift; my $List__ = \@_; do { [] }; ($self->{parameters}->emit_html() . (' = ' . ($self->{arguments}->emit_html() . ''))) }
;
...
;
package Return; sub new { shift; bless { @_ }, "Return" } sub emit_html { my $self = shift; my $List__ = \@_; do { [] }; return(('return(' . ($self->{result}->emit_html() . ')'))) }
;
package If; sub new { shift; bless { @_ }, "If" } sub emit_html { my $self = shift; my $List__ = \@_; do { [] }; ('do { if ( ${' . ($self->{cond}->emit_html() . ('->FETCH} ) { ' . ($self->{body}->emit_html() . (' } ' . (($self->{otherwise} ? (' else { ' . ($self->{otherwise}->emit_html() . ' }')) : '') . ' }')))))) }
;
package For; sub new { shift; bless { @_ }, "For" } sub emit_html { my $self = shift; my $List__ = \@_; do { [] }; my  $cond = $self->{cond}; do { if ((Main::isa($cond, 'Var') && ($cond->sigil() eq '@'))) { $cond = Apply->new( 'code' => 'prefix:<@>','arguments' => [$cond], ) } else {  } }; ('do { for my ' . ($self->{topic}->emit_html() . (' ( ' . ($cond->emit_html() . (' ) { ' . ($self->{body}->emit_html() . ' } }')))))) }
;
...
;
package Do; sub new { shift; bless { @_ }, "Do" } sub emit_html { my $self = shift; my $List__ = \@_; do { [] }; ('do { ' . ($self->{block}->emit_html() . ' }')) }
;
