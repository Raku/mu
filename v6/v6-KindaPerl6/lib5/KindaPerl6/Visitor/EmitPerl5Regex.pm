...
;
package Token; sub new { shift; bless { @_ }, "Token" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; my  $regex_source = $self->{regex}->emit_perl5(); my  $source = ('use vars qw($_rule_' . ($self->{name} . ('); ' . ('$_rule_' . ($self->{name} . (' = qr/' . ('(?{ ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, \'create\', pos(), \\$_ ]; ' . ('$GLOBAL::_M2 = $GLOBAL::_M; ' . ('})' . ($regex_source . ('(?{ ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, \'to\', pos() ]; ' . ('$GLOBAL::_M2 = $GLOBAL::_M; ' . ('})' . ('/x; ' . ('$::X->{_dispatch}( $::X, "HOW", )->{_dispatch}( ' . ('$::X->{_dispatch}( $::X, "HOW", ), ' . ('"add_method", ' . ('$::Str->{_dispatch}( $::Str, "new", "' . ($self->{name} . ('" ), ' . ('$::Method->{_dispatch}( $::Method, "new", ' . ('sub { ' . ('local $GLOBAL::_Class = shift; ' . ('undef $GLOBAL::_M2; ' . ('( ref($_) ? $_->{_dispatch}( $_, "str" )->{_value} : $_ ) =~ ' . ('/$_rule_' . ($self->{name} . ('/; ' . ('Match::from_global_data( $GLOBAL::_M2 ); ' . ('$MATCH = ' . ('$GLOBAL::MATCH = shift @Match::Matches; ' . ('} ' . ('), ' . '); '))))))))))))))))))))))))))))))))))); return($source) }
;
...
;
package Rule::After; sub new { shift; bless { @_ }, "Rule::After" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; Main::say('# TODO: <after> '); die() }
;
package Rule::Before; sub new { shift; bless { @_ }, "Rule::Before" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; do { if (($self->{assertion_modifier} eq '!')) { return(('(?!' . ($self->{rule}->emit_perl5() . ')'))) } else {  } }; do { if (($self->{assertion_modifier} eq '?')) { return(('(?=' . ($self->{rule}->emit_perl5() . ')'))) } else {  } }; do { if ($self->{capture_to_array}) { ('(?=' . ('(?{ ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, \'create\', pos(), \\$_ ]; ' . ('})' . ($self->{rule}->emit_perl5() . ('(?{ ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, \'to\', pos() ]; ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, "named_capture_to_array", "before" ]; ' . ('})' . ')'))))))))) } else { ('(?=' . ('(?{ ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, \'create\', pos(), \\$_ ]; ' . ('})' . ($self->{rule}->emit_perl5() . ('(?{ ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, \'to\', pos() ]; ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, "named_capture", "before" ]; ' . ('})' . ')'))))))))) } } }
;
...
;
package Rule::Subrule; sub new { shift; bless { @_ }, "Rule::Subrule" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; my  $meth = ('\'$_rule_' . ($self->{metasyntax} . '\'')); do { if ($self->{capture_to_array}) { ('(?:' . ('(??{ eval ' . ($meth . (' })' . ('(?{ ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, "named_capture_to_array", "' . ($self->{ident} . ('" ]; ' . ('})' . ')'))))))))) } else { ('(?:' . ('(??{ eval ' . ($meth . (' })' . ('(?{ ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, "named_capture", "' . ($self->{ident} . ('" ]; ' . ('})' . ')'))))))))) } } }
;
