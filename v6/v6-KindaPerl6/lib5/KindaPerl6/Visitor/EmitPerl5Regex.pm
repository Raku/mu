...
;
package Rule::Capture; sub new { shift; bless { @_ }, "Rule::Capture" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; do { if ($self->{capture_to_array}) { ('(?:' . ('(?{ ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, \'create\', pos(), \\$_ ]; ' . ('})' . ($self->{rule}->emit_perl5() . ('(?{ ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, \'to\', pos() ]; ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, "positional-capture-to-array", ' . ($self->{position} . (' ]; ' . ('})' . ')'))))))))))) } else { ('(?:' . ('(?{ ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, \'create\', pos(), \\$_ ]; ' . ('})' . ($self->{rule}->emit_perl5() . ('(?{ ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, \'to\', pos() ]; ' . ('local $GLOBAL::_M = [ $GLOBAL::_M, "positional-capture", ' . ($self->{position} . (' ]; ' . ('})' . ')'))))))))))) } } }
;
1;
