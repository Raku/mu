...
;
package Rule::Before; sub new { shift; bless { @_ }, "Rule::Before" } sub capture_count { my $self = shift; my $List__ = \@_; my $count; my $quantified; my $seen; do {  $count = $List__->[0];  $quantified = $List__->[1];  $seen = $List__->[2]; [$count, $quantified, $seen] }; $self->{capture_to_array} = ($quantified || ($seen->{'before'} && 1)); do { if ($seen->{'before'}) { $seen->{'before'}->capture_to_array(1) } else {  } }; $seen->{'before'} = $self; $self->{rule}->capture_count(0, 0, {  }); $count }
;
package Rule::NotBefore; sub new { shift; bless { @_ }, "Rule::NotBefore" } sub capture_count { my $self = shift; my $List__ = \@_; my $count; my $quantified; my $seen; do {  $count = $List__->[0];  $quantified = $List__->[1];  $seen = $List__->[2]; [$count, $quantified, $seen] }; $count }
;
