...
;
package Method; sub new { shift; bless { @_ }, "Method" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; my  $sig = $self->{block}->sig(); my  $invocant = $sig->invocant(); my  $pos = $sig->positional(); my  $decl_ = Decl->new( 'var' => Var->new( 'sigil' => '@','twigil' => '','name' => '_', ),'decl' => 'my','type' => '', ); my  $str = $decl_->emit_perl5(); $str = ($str . '$List__->{_value}{_array} = \@_;'); my  $pos = $sig->positional(); do { for my $field ( @{$pos} ) { my  $decl = Decl->new( 'var' => $field,'type' => '','decl' => 'my', );$str = ($str . $decl->emit_perl5()) } }; ('sub ' . ($self->{name} . (' { ' . ('my ' . ($invocant->emit_perl5() . (' = shift; ' . ($str . ($self->{block}->emit_perl5() . ' }')))))))) }
;
