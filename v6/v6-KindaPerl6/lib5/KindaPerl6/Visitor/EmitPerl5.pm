...
;
package Lit::Code; sub new { shift; bless { @_ }, "Lit::Code" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; ($self->emit_declarations() . $self->emit_body()) }; sub emit_with_arguments_perl5 { my $self = shift; my $List__ = \@_; do { [] }; ($self->emit_declarations() . ($self->emit_arguments() . $self->emit_body())) }; sub emit_body { my $self = shift; my $List__ = \@_; do { [] }; return(Main::join([ map { $_->emit_perl5() } @{ $self->{body} } ], '; ')) }; sub emit_declarations { my $self = shift; my $List__ = \@_; do { [] }; my  $s; do { for my $name ( @{$self->{pad}->variable_names()} ) { my  $decl = Decl->new( 'decl' => 'my','type' => '','var' => Var->new( 'sigil' => '','twigil' => '','name' => $name, ), );$s = ($s . ($name->emit_perl5() . '; ')) } }; return($s) }; sub emit_arguments { my $self = shift; my $List__ = \@_; do { [] }; my  $array_ = Var->new( 'sigil' => '@','twigil' => '','name' => '_', ); my  $str = ''; do { if ($self->{sig}->invocant()) { $str = ($str . '$self = shift; ') } else { $str = ('#no invocant' . Main::newline()) } }; $str = ($str . '$List__->{_value}{_array} = \@_;'); my  $i = 0; do { for my $field ( @{$self->{sig}->positional()} ) { my  $bind = Bind->new( 'parameters' => $field,'arguments' => Index->new( 'obj' => $array_,'index' => Val::Int->new( 'int' => $i, ), ), );$str = ($str . ($bind->emit_perl5() . ';'));$i = ($i + 1) } }; return($str) }
;
...
;
package Method; sub new { shift; bless { @_ }, "Method" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; ('sub ' . ($self->{name} . (' { ' . ($self->{block}->emit_with_arguments_perl5() . ' }')))) }
;
