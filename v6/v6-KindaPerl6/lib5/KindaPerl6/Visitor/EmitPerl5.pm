...
;
package Bind; sub new { shift; bless { @_ }, "Bind" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; return(($self->{parameters}->emit_perl5() . (' = ' . $self->{arguments}->emit_perl5()))) }
;
...
;
package Method; sub new { shift; bless { @_ }, "Method" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; my  $sig = $self->{block}->sig(); my  $invocant = $sig->invocant(); my  $pos = $sig->positional(); my  $array_ = Var->new( 'sigil' => '@','twigil' => '','name' => '_', ); my  $decl_ = Decl->new( 'var' => $array_,'decl' => 'my','type' => '', ); my  $str = $decl_->emit_perl5(); $str = ($str . '$List__->{_value}{_array} = \@_;'); my  $i = 0; do { for my $field ( @{$pos} ) { my  $decl = Decl->new( 'var' => $field,'type' => '','decl' => 'my', );$str = ($str . $decl->emit_perl5());my  $bind = Bind->new( 'parameters' => $field,'arguments' => Index->new( 'obj' => $array_,'index' => Val::Int->new( 'int' => $i, ), ), );$str = ($str . ($bind->emit_perl5() . ';'));$i = ($i + 1) } }; ('sub ' . ($self->{name} . (' { ' . ('my ' . ($invocant->emit_perl5() . (' = shift; ' . ($str . ($self->{block}->emit_perl5() . ' }')))))))) }
;
package Sub; sub new { shift; bless { @_ }, "Sub" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; my  $str = ''; my  $code = ('::DISPATCH( $::Code, \'new\', { ' . ('code => sub { ' . ($str . ($self->{block}->emit_perl5() . (' }' . (', src => q#sub { ' . (COMPILER::emit_perl6($self->{block}) . (' }#' . ' } )')))))))); do { if ($self->{name}) { return(('$Code_' . ($self->{name} . ('->BIND( ' . ($code . ')'))))) } else {  } }; return($code) }
;
