...
;
package Sig; sub new { shift; bless { @_ }, "Sig" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; my  $pos; do { for my $decl ( @{$self->{positional}} ) { $pos = ($pos . (Main::perl($decl, ) . ', ')) } }; ('::DISPATCH( $::Signature, "new", { ' . ('invocant => ' . (Main::perl($self->{invocant}, ) . (', ' . ('array    => [ ' . ($pos . (' ], ' . ('hash     => { }, ' . ('return   => undef, ' . '} )'))))))))) }
;
