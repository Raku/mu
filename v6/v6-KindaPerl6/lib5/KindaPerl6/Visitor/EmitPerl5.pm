...
;
package Lit::Code; sub new { shift; bless { @_ }, "Lit::Code" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; ($self->emit_declarations() . $self->emit_body()) }; sub emit_body { my $self = shift; my $List__ = \@_; do { [] }; return(Main::join([ map { $_->emit_perl5() } @{ $self->{body} } ], '; ')) }; sub emit_signature { my $self = shift; my $List__ = \@_; do { [] }; ('::DISPATCH( $::Signature, \'new\', { ' . ('invocant => undef, ' . ('array    => [ ], ' . ('hash     => { }, ' . ('return   => undef, ' . '} )'))))) }; sub emit_declarations { my $self = shift; my $List__ = \@_; do { [] }; my  $s; do { for my $name ( @{$self->{pad}->variable_names()} ) { my  $decl = Decl->new( 'decl' => 'my','type' => '','var' => Var->new( 'sigil' => '','twigil' => '','name' => $name, ), );$s = ($s . ($name->emit_perl5() . '; ')) } }; return($s) }; sub emit_arguments { my $self = shift; my $List__ = \@_; do { [] }; my  $array_ = Var->new( 'sigil' => '@','twigil' => '','name' => '_', ); my  $str = ''; $str = ($str . '$List__->{_value}{_array} = \@_;'); my  $i = 0; do { for my $field ( @{$self->{sig}->positional()} ) { my  $bind = Bind->new( 'parameters' => $field,'arguments' => Index->new( 'obj' => $array_,'index' => Val::Int->new( 'int' => $i, ), ), );$str = ($str . ($bind->emit_perl5() . ';'));$i = ($i + 1) } }; return($str) }
;
...
;
package For; sub new { shift; bless { @_ }, "For" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; my  $cond = $self->{cond}; do { if ((Main::isa($cond, 'Var') && ($cond->sigil() eq '@'))) {  } else { $cond = Apply->new( 'code' => Var->new( 'sigil' => '&','twigil' => '','name' => 'prefix:<@>', ),'arguments' => [$cond], ) } }; ('do { for my ' . ($self->{topic}->emit_perl5() . (' ( @{ ' . ($cond->emit_perl5() . ('->{_value}{_array} } )' . (' { ' . ($self->{body}->emit_perl5() . (' } ' . '}')))))))) }
;
...
;
package Sig; sub new { shift; bless { @_ }, "Sig" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; ('::DISPATCH( $::Signature, \'new\', { ' . '} )') }
;
...
;
package Sub; sub new { shift; bless { @_ }, "Sub" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; ('::DISPATCH( $::Code, \'new\', { ' . ('code => sub { ' . ($self->{block}->emit_declarations() . ($self->{block}->emit_arguments() . ($self->{block}->emit_body() . (' }, ' . ('signature => ' . ($self->{block}->emit_signature() . (', ' . ' } )'))))))))) }
;
