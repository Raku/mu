...
;
package Bind; sub new { shift; bless { @_ }, "Bind" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; do { if (Main::isa($self->{parameters}, 'Var')) { return(($self->{parameters}->emit_perl5() . ('->{_dispatch_VAR}( ' . ($self->{parameters}->emit_perl5() . (', \'BIND\', ' . ($self->{arguments}->emit_perl5() . ' )')))))) } else {  } }; do { if (Main::isa($self->{parameters}, 'Proto')) { return(($self->{parameters}->emit_perl5() . (' = ' . $self->{arguments}->emit_perl5()))) } else {  } } }
;
package Proto; sub new { shift; bless { @_ }, "Proto" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; ("" . $self->{name}) }
;
package Call; sub new { shift; bless { @_ }, "Call" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; my  $invocant; do { if (Main::isa($self->{invocant}, 'Str')) { $invocant = ('$::' . $self->{invocant}) } else { do { if (Main::isa($self->{invocant}, 'Val::Buf')) { $invocant = ('$::' . $self->{invocant}->buf()) } else { do { if (Main::isa($self->{invocant}, 'Proto')) { do { if (($self->{invocant}->name() eq 'self')) { $invocant = '$self' } else { $invocant = ('$::' . $self->{invocant}->emit_perl5()) } } } else { $invocant = $self->{invocant}->emit_perl5() } } } } } }; do { if (($invocant eq 'self')) { $invocant = '$self' } else {  } }; do { if ((($self->{method} eq 'yaml') || (($self->{method} eq 'say') || (($self->{method} eq 'join') || (($self->{method} eq 'chars') || ($self->{method} eq 'isa')))))) { do { if ($self->{hyper}) { return(('[ map { Main::' . ($self->{method} . ('( $_, ' . (', ' . (Main::join([ map { $_->emit_perl5() } @{ $self->{arguments} } ], ', ') . (')' . (' } @{ ' . ($invocant . ' } ]'))))))))) } else { return(('Main::' . ($self->{method} . ('(' . ($invocant . (', ' . (Main::join([ map { $_->emit_perl5() } @{ $self->{arguments} } ], ', ') . ')'))))))) } } } else {  } }; my  $meth = $self->{method}; do { if (($meth eq 'postcircumfix:<( )>')) { $meth = '' } else {  } }; my  $call = Main::join([ map { $_->emit_perl5() } @{ $self->{arguments} } ], ', '); do { if ($self->{hyper}) { ('[ map { $_' . ('->' . ($meth . ('(' . ($call . (') } @{ ' . ($invocant . ' } ]'))))))) } else { do { if (($meth eq '')) { ('dispatch( ' . ($invocant . (', \'APPLY\', ' . ($call . ' )')))) } else { ('dispatch( ' . ($invocant . (', ' . ('\'' . ($meth . ('\', ' . ($call . ' )'))))))) } } } } }
;
...
;
package Decl; sub new { shift; bless { @_ }, "Decl" } sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; my  $decl = $self->{decl}; my  $name = $self->{var}->name(); do { if (($decl eq 'has')) { return(('sub ' . ($name . (' { ' . ('@_ == 1 ' . ('? ( $_[0]->{' . ($name . ('} ) ' . (': ( $_[0]->{' . ($name . ('} = $_[1] ) ' . '}'))))))))))) } else {  } }; my  $create = (', \'new\', { modified => $_MODIFIED, name => \'' . ($self->{var}->emit_perl5() . '\' } ) ')); do { if (($decl eq 'our')) { my  $s;$s = 'our ';do { if (($self->{var}->sigil() eq '$')) { return(($s . ($self->{var}->emit_perl5() . (' = dispatch( $::Scalar' . ($create . (' unless defined ' . ($self->{var}->emit_perl5() . ('; ' . ('BEGIN { ' . ($self->{var}->emit_perl5() . (' = dispatch( $::Scalar' . ($create . (' unless defined ' . ($self->{var}->emit_perl5() . ('; ' . '}'))))))))))))))) } else {  } };do { if (($self->{var}->sigil() eq '&')) { return(($s . ($self->{var}->emit_perl5() . (' = dispatch( $::Routine' . $create)))) } else {  } };do { if (($self->{var}->sigil() eq '%')) { return(($s . ($self->{var}->emit_perl5() . (' = dispatch( $::Hash' . $create)))) } else {  } };do { if (($self->{var}->sigil() eq '@')) { return(($s . ($self->{var}->emit_perl5() . (' = dispatch( $::Array' . $create)))) } else {  } };return(($s . ($self->{var}->emit_perl5() . ' '))) } else {  } }; do { if (($self->{var}->sigil() eq '$')) { return(($self->{decl} . (' ' . ($self->{type} . (' ' . ($self->{var}->emit_perl5() . ('; ' . ($self->{var}->emit_perl5() . (' = dispatch( $::Scalar' . ($create . (' unless defined ' . ($self->{var}->emit_perl5() . ('; ' . ('BEGIN { ' . ($self->{var}->emit_perl5() . (' = dispatch( $::Scalar' . ($create . '}'))))))))))))))))) } else {  } }; do { if (($self->{var}->sigil() eq '&')) { return(($self->{decl} . (' ' . ($self->{type} . (' ' . ($self->{var}->emit_perl5() . ('; ' . ($self->{var}->emit_perl5() . (' = dispatch( $::Routine' . ($create . (' unless defined ' . ($self->{var}->emit_perl5() . ('; ' . ('BEGIN { ' . ($self->{var}->emit_perl5() . (' = dispatch( $::Routine' . ($create . '}'))))))))))))))))) } else {  } }; do { if (($self->{var}->sigil() eq '%')) { return(($self->{decl} . (' ' . ($self->{type} . (' ' . ($self->{var}->emit_perl5() . (' = dispatch( $::Hash' . $create))))))) } else {  } }; do { if (($self->{var}->sigil() eq '@')) { return(($self->{decl} . (' ' . ($self->{type} . (' ' . ($self->{var}->emit_perl5() . (' = dispatch( $::Array' . $create))))))) } else {  } }; return(($self->{decl} . (' ' . ($self->{type} . (' ' . $self->{var}->emit_perl5()))))) }
;
