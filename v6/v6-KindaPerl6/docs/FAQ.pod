...
=head3 What do you need?
 ...
 - Perl5 (Perl 5.10.RC2 worked with the kp6 test cases.)
...
=head2 Cannot see the forest for the trees.

One of the problems I had when I started working on kp6 was I could not see
how everything worked together.

mp6.pl (miniperl6) is used to compile/translate some of the KP6 perl6 code into
perl5 code.   The resulting perl5 code is used to provide a run time enviroment
for code (a script or a one liner) translated by kp6 (script/kp6) into perl5
code.

Urk!.  That translates into this.

1. Compile kp6 (written in perl6) libraries (src/KindaPerl6/???) using miniperl6
to provide a run time foundation for code compiled by script/kp6.

TODO: pmurias @tell dlocaus file in KindaPerl6::Runtime::Perl6 are compiled by kp6 (script/kp6) not mp6

L<http://irclog.perlgeek.de/perl6/2007-12-06#i_157744>

2. Use kp6 to compile src/KindaPerl6/Runtime/*
These go into lib/compiled-kp6-mp6/KindaPerl6/Runtime/* ..
These libraries provide an emulation of the perl6 enviroment.
Miniperl6 converts only a small subset of perl6 into perl5 code.

3. Compile a script script/kp6.  C<perl script/kp6 -o perl5code.pl source-perl6.pl>.

4. Execute the resulting perl5code.pl using the lib/compiled-kp6-mp6/...
libraries to emulate the perl6 enviroment, which is class creation, variable
allocation, inheritance, etc...
