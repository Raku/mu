...
{ # Perl5 tests
  ...
  local $Test::Harness::Switches = "$Test::Harness::Switches -Ilib-kp6-kp6-p5 -Ilib-modules-p5";
  $ok &&= eval { runtests glob("t/p5/*.t") };
  warn $@ if $@;
  my $perl5 = $ENV{HARNESS_PERL} || $^X;
  local $ENV{HARNESS_PERL} = "$^X run_kp6_kp6_perl5.pl -Ilib-kp6-kp6-p5 -Ilib-modules-p5";
  local $ENV{PERL5LIB} = '';
