...
sub set_secure_mode { my $List__ = \@_; my $args_secure; do {  $args_secure = $List__->[0]; [$args_secure] }; my  $value = '0'; do { if (($args_secure != 0)) { $value = '1' } else {  } }; return(('use constant KP6_DISABLE_INSECURE_CODE => ' . ($value . (';' . Main::newline())))) };
sub emit_perl5 { my $self = shift; my $List__ = \@_; my $args_secure; do {  $args_secure = $List__->[0]; [$args_secure] }; ('{ package ' . ($self->{name} . ('; ' . (Main::newline() . ('# Do not edit this file - Perl 5 generated by ' . ($Main::_V6_COMPILER_NAME . (Main::newline() . ('use v5;' . (Main::newline() . ('use strict;' . (Main::newline() . ('no strict \'vars\';' . (Main::newline() . (set_secure_mode($args_secure) . ('use ' . (Main::get_compiler_target_runtime() . (';' . (Main::newline() . ('my $_MODIFIED; BEGIN { $_MODIFIED = {} }' . (Main::newline() . ('BEGIN { ' . ('$_ = ::DISPATCH($::Scalar, "new", { modified => $_MODIFIED, name => "$_" } ); ' . ('}' . (Main::newline() . ($self->{body}->emit_perl5() . (Main::newline() . ('; 1 }' . Main::newline()))))))))))))))))))))))))))) }
...
sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; ('::DISPATCH( $::Method, \'new\', { ' . ('code => sub { ' . ($self->{block}->emit_declarations() . ('$self = shift; ' . ($self->{block}->emit_arguments() . ($self->{block}->emit_body() . (' }, ' . ('signature => ' . ($self->{block}->emit_signature() . (', ' . (' } )' . Main::newline()))))))))))) }
