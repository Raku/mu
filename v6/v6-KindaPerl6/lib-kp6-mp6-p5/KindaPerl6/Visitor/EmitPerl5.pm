...
sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; do { if (($self->{code}->name() eq 'self')) { return('$self') } else {  } }; my  $op = $self->{code}->emit_perl5(); do { if (($op eq '$GLOBAL::Code_infix_58__60__124__124__62_')) { return(('::DISPATCH( $::Bit, "new", ' . ('::DISPATCH(' . ($self->{arguments}->[0]->emit_perl5() . (',"true")->{_value} || ' . ('::DISPATCH(' . ($self->{arguments}->[1]->emit_perl5() . (',"true")->{_value})' . Main::newline())))))))) } else {  } }; do { if (($op eq '$GLOBAL::Code_infix_58__60__38__38__62_')) { return(('::DISPATCH( $::Bit, "new", ' . ('::DISPATCH(' . ($self->{arguments}->[0]->emit_perl5() . (',"true")->{_value} && ' . ('::DISPATCH(' . ($self->{arguments}->[1]->emit_perl5() . (',"true")->{_value})' . Main::newline())))))))) } else {  } }; return(('::DISPATCH( ' . ($op . (', \'APPLY\', ' . (Main::join([ map { $_->emit_perl5() } @{ $self->{arguments} } ], ', ') . (' )' . Main::newline())))))) }
...
sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; ('sub ' . ($self->{name} . (' { ' . ($self->{block}->emit_declarations() . ('$self = shift; ' . ($self->{block}->emit_arguments() . ($self->{block}->emit_body() . (' }' . Main::newline())))))))) }
