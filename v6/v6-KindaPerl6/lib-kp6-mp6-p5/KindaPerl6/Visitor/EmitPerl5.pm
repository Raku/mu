...
sub emit_perl5 { my $self = shift; my $List__ = \@_; my $args_secure; do {  $args_secure = $List__->[0]; [$args_secure] }; ('{ package ' . ($self->{name} . ('; ' . (Main::newline() . ('# Do not edit this file - Perl 5 generated by ' . ($Main::_V6_COMPILER_NAME . (Main::newline() . ('use v5;' . (Main::newline() . ('use strict;' . (Main::newline() . ('no strict \'vars\';' . (Main::newline() . (set_secure_mode($args_secure) . ('use KindaPerl6::Runtime::Perl5::Runtime;' . (Main::newline() . ('my $_MODIFIED; BEGIN { $_MODIFIED = {} }' . (Main::newline() . ('BEGIN { ' . ('$_ = ::DISPATCH($::Scalar, "new", { modified => $_MODIFIED, name => "$_" } ); ' . ('}' . (Main::newline() . ($self->{body}->emit_perl5() . (Main::newline() . ('; 1 }' . Main::newline()))))))))))))))))))))))))) }
...
sub perl { my $self = shift; my $List__ = \@_; do { [] }; ('::DISPATCH( $::Signature::Item, "new", { ' . ('sigil  => \'' . ($self->{sigil} . ('\', ' . ('twigil => \'' . ($self->{twigil} . ('\', ' . ('name   => \'' . ($self->{name} . ('\', ' . ('} )' . Main::newline()))))))))))) }
...
sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; do { if (($self->{code}->name() eq 'self')) { return('$self') } else {  } }; return(('::DISPATCH( ' . ($self->{code}->emit_perl5() . (', \'APPLY\', ' . (Main::join([ map { $_->emit_perl5() } @{ $self->{arguments} } ], ', ') . (' )' . Main::newline())))))) }
...
sub emit_perl5 { my $self = shift; my $List__ = \@_; do { [] }; my  $cond = $self->{cond}; do { if ((Main::isa($cond, 'Var') && ($cond->sigil() eq '@'))) {  } else { $cond = Apply->new( 'code' => Var->new( 'sigil' => '&','twigil' => '','name' => 'GLOBAL::prefix:<@>', ),'arguments' => [$cond], ) } }; ('for ' . ($self->{topic}->emit_perl5() . (' ( @{ ' . ($cond->emit_perl5() . ('->{_value}{_array} } )' . (' { ' . ($self->{body}->emit_perl5() . (' } ' . Main::newline())))))))) }
