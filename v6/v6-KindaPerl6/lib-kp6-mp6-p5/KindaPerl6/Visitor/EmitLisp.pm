...
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; my  $name = $self->{code}->name(); do { if (($name eq 'infix:<//>')) { my  $apply_func = ('(kp6-apply-function ' . ($interpreter . (' ' . ('(perl->cl (lookup-package-variable (kp6-generate-variable "&" "defined") "GLOBAL")) ' . '(list '))));return(('(or (kp6-true ' . ($apply_func . (Main::join([ map { $_->emit_lisp($interpreter, $indent) } @{ $self->{arguments} } ], '))) (kp6-true ') . '))')))) } else {  } }; do { if (($name eq 'infix:<&&>')) { return(('(and (kp6-true ' . (Main::join([ map { $_->emit_lisp($interpreter, $indent) } @{ $self->{arguments} } ], ') (kp6-true ') . '))'))) } else {  } }; do { if (($name eq 'infix:<||>')) { return(('(or (kp6-true ' . (Main::join([ map { $_->emit_lisp($interpreter, $indent) } @{ $self->{arguments} } ], ') (kp6-true ') . '))'))) } else {  } }; do { if (($name eq 'ternary:<?? !!>')) { return(('(if (kp6-true ' . ($self->{arguments}->[0]->emit_lisp($interpreter, $indent) . (') (progn ' . ($self->{arguments}->[1]->emit_lisp($interpreter, $indent) . (') (progn ' . ($self->{arguments}->[2]->emit_lisp($interpreter, $indent) . '))'))))))) } else {  } }; my  $op = $self->{code}->emit_lisp($interpreter, $indent); return(('(kp6-apply-function ' . ($interpreter . (' (perl->cl ' . ($op . (') (list ' . (Main::join([ map { $_->emit_lisp($interpreter, $indent) } @{ $self->{arguments} } ], ' ') . '))'))))))) }
...
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; my  $str = '(make-instance \'kp6-signature'; do { if ($self->{invocant}) { $str = ($str . (' :invocant ' . $self->{invocant}->emit_lisp($interpreter, $indent))) } else {  } }; $str = ($str . ' :value (list'); do { for my $decl ( @{$self->{positional}} ) { $str = ($str . (' (kp6-sig-item \'positional ' . ($decl->emit_lisp_name($interpreter, $indent) . ')'))) } }; $str = ($str . ')'); $str = ($str . ')'); return($str); my  $inv = '$::Undef'; do { if (Main::isa($self->{invocant}, 'Var')) { $inv = Main::perl($self->{invocant}, ) } else {  } }; my  $pos; my  $decl; do { for my $decl ( @{$self->{positional}} ) { $pos = ($pos . (Main::perl($decl, ) . ', ')) } }; my  $named = ''; ('(kp6-new \'signature ' . ('invocant: ' . ($inv . (', ' . ('array: ::DISPATCH( $::Array, "new", { _array => [ ' . ($pos . (' ] } ), ' . ('hash: ::DISPATCH( $::Hash,  "new", { _hash  => { ' . ($named . (' } } ), ' . ('return: $::Undef, ' . ')'))))))))))) }
