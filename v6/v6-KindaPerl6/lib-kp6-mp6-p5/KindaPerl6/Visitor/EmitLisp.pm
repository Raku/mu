...
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; my  $str = '(make-instance \'kp6-signature'; do { if ($self->{invocant}) { $str = ($str . (' :invocant ' . $self->{invocant}->emit_lisp($interpreter, $indent))) } else {  } }; $str = ($str . ' :array (list'); do { for my $decl ( @{$self->{positional}} ) { $str = ($str . (' ' . $decl->emit_lisp_name($interpreter, $indent))) } }; $str = ($str . ')'); $str = ($str . ')'); return($str); my  $inv = '$::Undef'; do { if (Main::isa($self->{invocant}, 'Var')) { $inv = Main::perl($self->{invocant}, ) } else {  } }; my  $pos; my  $decl; do { for my $decl ( @{$self->{positional}} ) { $pos = ($pos . (Main::perl($decl, ) . ', ')) } }; my  $named = ''; ('(kp6-new \'signature ' . ('invocant: ' . ($inv . (', ' . ('array: ::DISPATCH( $::Array, "new", { _array => [ ' . ($pos . (' ] } ), ' . ('hash: ::DISPATCH( $::Hash,  "new", { _hash  => { ' . ($named . (' } } ), ' . ('return: $::Undef, ' . ')'))))))))))) }
...
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; return(('(make-kp6-sub (' . ($interpreter . (' ' . ($self->{block}->emit_signature($interpreter, $indent) . (')' . (Main::newline() . ($self->{block}->emit_lisp($interpreter, $indent) . ')')))))))) }
