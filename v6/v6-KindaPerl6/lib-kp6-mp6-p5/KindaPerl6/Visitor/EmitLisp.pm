...
sub emit_lisp { my $self = shift; my $List__ = \@_; my $args_secure; do {  $args_secure = $List__->[0]; [$args_secure] }; my  $interpreter = ('|' . ($self->{name} . '|')); (';; Do not edit this file - Lisp generated by ' . ($Main::_V6_COMPILER_NAME . (Main::newline() . ('(eval-when (:compile-toplevel :load-toplevel :execute)' . (Main::newline() . ('  (when (null (find-package \'kp6-lisp))' . (Main::newline() . ('   ' . (set_secure_mode($args_secure) . ('   (load "lib/KindaPerl6/Runtime/Lisp/Runtime.lisp")))' . (Main::newline() . ('(defpackage #:' . ($self->{name} . (Main::newline() . ('  (:use #:cl #:kp6-lisp))' . (Main::newline() . ('(in-package #:kp6-lisp-user)' . (Main::newline() . ('(kp6-add-program (' . ($interpreter . (')' . (Main::newline() . (' (with-kp6-package (' . ($interpreter . (' "GLOBAL")' . (Main::newline() . ($self->{body}->emit_lisp($interpreter, 3) . ('))' . Main::newline())))))))))))))))))))))))))))) }
