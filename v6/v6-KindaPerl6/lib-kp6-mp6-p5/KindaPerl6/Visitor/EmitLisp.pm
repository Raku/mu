...
sub emit_lisp { my $self = shift; my $List__ = \@_; do { [] }; my  $node = $self->{parameters}; do { if ((Main::isa($node, 'Var') && @{$node->namespace()})) { $node = Apply->new( 'code' => Var->new( 'name' => 'ternary:<?? !!>','twigil' => '','sigil' => '&','namespace' => ['GLOBAL'], ),'arguments' => [Apply->new( 'arguments' => [$node],'code' => Var->new( 'name' => 'VAR_defined','twigil' => '','sigil' => '&','namespace' => ['GLOBAL'], ), ), $node, Bind->new( 'parameters' => $node,'arguments' => Call->new( 'invocant' => Var->new( 'name' => '::Scalar','twigil' => '','sigil' => '$','namespace' => [], ),'method' => 'new','hyper' => '', ), )], ) } else {  } }; ('(setf ' . ($self->{node}->emit_lisp() . (' ' . ($self->{arguments}->emit_lisp() . ')')))) }


;
package Var;
sub new { shift; bless { @_ }, "Var" }
sub emit_lisp { my $self = shift; my $List__ = \@_; do { [] }; my  $table = { '$' => 'kp6-','@' => 'kp6-List_','%' => 'kp6-Hash_','&' => 'kp6-Code_', }; do { if (($self->{twigil} eq '.')) { return(('::DISPATCH( $self, "' . ($self->{name} . ('" )' . Main::newline())))) } else {  } }; do { if (($self->{name} eq '/')) { return(($table->{$self->{sigil}} . 'MATCH')) } else {  } }; my  $namespace = $self->{namespace}; do { if ((@{$namespace} ? 0 : 1)) { $namespace = ['GLOBAL'] } else {  } }; return(('(kp6-lookup (kp6-lookup *kp6-packages* "' . (join('::', @{$namespace}) . ('") (cons \'' . ($self->{sigil} . (' "' . ($self->{name} . '"))'))))))) };
