...
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; my  $namespace = $self->{namespace}; do { if (@{$namespace}) { return(('(lookup-package-variable ' . ($self->emit_lisp_name() . (' ' . ($self->emit_lisp_namespace() . ')'))))) } else {  } }; return(('(lookup-lexical-variable ' . ($self->emit_lisp_name() . ')'))) };
...
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; do { if ((Main::isa($self->{arguments}, 'Var') && (@{$self->{arguments}->namespace()} ? 0 : 1))) { return(('(set-lexical-variable/c ' . ($self->{parameters}->emit_lisp_name() . (' (lookup-lexical-variable/c ' . ($self->{arguments}->emit_lisp_name() . '))'))))) } else {  } }; do { if (Main::isa($self->{parameters}, 'Call')) { return(Assign->new( 'parameters' => $self->{parameters},'arguments' => $self->{arguments}, )->emit_lisp($interpreter, $indent)) } else {  } }; do { if (Main::isa($self->{parameters}, 'Lookup')) { return(Assign->new( 'parameters' => $self->{parameters},'arguments' => $self->{arguments}, )->emit_lisp($interpreter, $indent)) } else {  } }; do { if (Main::isa($self->{parameters}, 'Index')) { return(Assign->new( 'parameters' => $self->{parameters},'arguments' => $self->{arguments}, )->emit_lisp($interpreter, $indent)) } else {  } }; my  $str = ''; $str = ($str . ('(setf ' . ($self->{parameters}->emit_lisp($interpreter, $indent) . (' ' . ($self->{arguments}->emit_lisp($interpreter, $indent) . ')'))))); return($str) }
