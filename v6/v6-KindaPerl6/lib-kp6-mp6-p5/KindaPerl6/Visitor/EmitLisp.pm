...
sub emit_lisp { my $self = shift; my $List__ = \@_; my $args_secure; do {  $args_secure = $List__->[0]; [$args_secure] }; my  $interpreter = ('|' . ($self->{name} . '|')); (';; Do not edit this file - Lisp generated by ' . ($Main::_V6_COMPILER_NAME . (Main::newline() . ('(eval-when (:compile-toplevel :load-toplevel :execute)' . (Main::newline() . ('  (when (null (find-package \'kp6-lisp))' . (Main::newline() . ('   ' . (set_secure_mode($args_secure) . ('   (load "lib/KindaPerl6/Runtime/Lisp/Runtime.lisp")))' . (Main::newline() . ('(in-package #:kp6-lisp-user)' . (Main::newline() . ('(kp6-add-program (' . ($interpreter . (')' . (Main::newline() . (' (kp6-ensure-package ' . ($interpreter . (' "' . ($self->{name} . ('")' . (Main::newline() . (' (with-kp6-package (' . ($interpreter . (' "' . ($self->{name} . ('")' . (Main::newline() . ($self->{body}->emit_lisp($interpreter, 3) . ('))' . Main::newline()))))))))))))))))))))))))))))))) }
...
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; return(('(kp6-error ' . ($interpreter . ' \'kp6-not-implemented :feature "literal objects")'))) }
...
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; do { if (Main::isa($self->{arguments}, 'Var')) { return($self->{parameters}->emit_lisp_assignment($self->{arguments}->emit_lisp_lookup(1), 1)) } else {  } }; do { if (Main::isa($self->{arguments}, 'Sub')) { return($self->{parameters}->emit_lisp_assignment($self->{arguments}->emit_lisp($interpreter, $indent))) } else {  } }; return(('(kp6-error ' . ($interpreter . ' \'kp6-not-implemented :feature "binding anything other than variables")'))) }


;
package Proto;
sub new { shift; bless { @_ }, "Proto" }
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; return(('(kp6-error ' . ($interpreter . ' \'kp6-not-implemented :feature "proto-objects")'))) }


;
package Call;
sub new { shift; bless { @_ }, "Call" }
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; do { if ((Main::isa($self->{invocant}, 'Var') && (($self->{method} eq 'LOOKUP') || ($self->{method} eq 'INDEX')))) { return(('(kp6-lookup ' . ($self->{invocant}->emit_lisp($interpreter, $indent) . (' (perl->cl ' . ($self->{arguments}->[0]->emit_lisp($interpreter, $indent) . '))'))))) } else {  } }; my  $invocant; do { if (Main::isa($self->{invocant}, 'Proto')) { do { if (($self->{invocant}->name() eq 'self')) { $invocant = '$self' } else { $invocant = $self->{invocant}->emit_lisp() } } } else { $invocant = $self->{invocant}->emit_lisp() } }; do { if (($invocant eq 'self')) { $invocant = '$self' } else {  } }; my  $meth = $self->{method}; do { if (($meth eq 'postcircumfix:<( )>')) { $meth = '' } else {  } }; my  $call = Main::join([ map { $_->emit_lisp($interpreter, $indent) } @{ $self->{arguments} } ], ' '); do { if ($self->{hyper}) { return('XXX: Hyper') } else { do { if (($meth eq '')) { ('::DISPATCH( ' . ($invocant . (', \'APPLY\', ' . ($call . (' )' . Main::newline()))))) } else { ('(kp6-dispatch ' . ($invocant . (' ' . ('"' . ($meth . ('"' . ($call . (')' . Main::newline())))))))) } } } } }


;
package Apply;
sub new { shift; bless { @_ }, "Apply" }
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; my  $name = $self->{code}->name(); do { if (($name eq 'infix:<//>')) { my  $apply_func = ('(kp6-apply-function ' . ($interpreter . (' ' . ('(perl->cl (lookup-package-variable (kp6-generate-variable "&" "defined") "GLOBAL")) ' . '(list '))));return(('(make-instance \'kp6-Bit :value (or (kp6-true ' . ($apply_func . (Main::join([ map { $_->emit_lisp($interpreter, $indent) } @{ $self->{arguments} } ], '))) (kp6-true ') . ')))')))) } else {  } }; do { if (($name eq 'infix:<&&>')) { return(('(make-instance \'kp6-Bit :value (and (kp6-true ' . (Main::join([ map { $_->emit_lisp($interpreter, $indent) } @{ $self->{arguments} } ], ') (kp6-true ') . ')))'))) } else {  } }; do { if (($name eq 'infix:<||>')) { return(('(make-instance \'kp6-Bit :value (or (kp6-true ' . (Main::join([ map { $_->emit_lisp($interpreter, $indent) } @{ $self->{arguments} } ], ') (kp6-true ') . ')))'))) } else {  } }; do { if (($name eq 'ternary:<?? !!>')) { return(('(if (kp6-true ' . ($self->{arguments}->[0]->emit_lisp($interpreter, $indent) . (') (progn ' . ($self->{arguments}->[1]->emit_lisp($interpreter, $indent) . (') (progn ' . ($self->{arguments}->[2]->emit_lisp($interpreter, $indent) . '))'))))))) } else {  } }; my  $op = $self->{code}->emit_lisp($interpreter, $indent); my  $str = ('(kp6-apply-function ' . ($interpreter . (' (perl->cl ' . ($op . ') (list')))); do { for my $arg ( @{$self->{arguments}} ) { $str = ($str . ' (make-instance \'kp6-positional-parameter :value ');do { if (Main::isa($arg, 'Var')) { $str = ($str . $arg->emit_lisp_lookup(1)) } else { $str = ($str . ('(make-kp6-cell ' . ($arg->emit_lisp($interpreter, $indent) . ')'))) } };$str = ($str . ')') } }; $str = ($str . '))'); return($str) }
...
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; return(('(kp6-error ' . ($interpreter . ' \'kp6-not-implemented :feature "subsets")'))) }


;
package Method;
sub new { shift; bless { @_ }, "Method" }
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; return(('(kp6-error ' . ($interpreter . ' \'kp6-not-implemented :feature "methods")'))) }
...
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; return(('(kp6-error ' . ($interpreter . ' \'kp6-not-implemented :feature "BEGIN blocks")'))) }


;
package Use;
sub new { shift; bless { @_ }, "Use" }
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; do { if (($self->{mod} eq 'v6')) { return((undef)) } else {  } }; return(('(kp6-error ' . ($interpreter . ' \'kp6-not-implemented :feature "importing modules")'))) }
