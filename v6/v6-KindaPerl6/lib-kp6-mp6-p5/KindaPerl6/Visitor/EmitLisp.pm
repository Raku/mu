...
sub emit_lisp { my $self = shift; my $List__ = \@_; my $interpreter; my $indent; do {  $interpreter = $List__->[0];  $indent = $List__->[1]; [$interpreter, $indent] }; ('(with-kp6-pad (' . ($interpreter . (')' . (Main::newline() . ($self->emit_declarations($interpreter, $indent) . ($self->emit_body($interpreter, $indent) . ')')))))) };
