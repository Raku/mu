...
sub set_secure_mode { my $List__ = \@_; my $args_secure; do {  $args_secure = $List__->[0]; [$args_secure] }; my  $value = 'nil'; do { if (($args_secure != 0)) { $value = 't' } else {  } }; return(('(defconstant +KP6_DISABLE_INSECURE_CODE+ ' . ($value . (')' . Main::newline())))) };
sub emit_lisp { my $self = shift; my $List__ = \@_; my $args_secure; do {  $args_secure = $List__->[0]; [$args_secure] }; ('(in-package #:cl-user)' . (Main::newline() . ('(load (compile-file "lib/KindaPerl6/Runtime/Lisp/Runtime.lisp"))' . (Main::newline() . ('(defpackage #:' . ($self->{name} . (Main::newline() . ('  (:use #:cl #:kp6-cl))' . (Main::newline() . ('(in-package #:' . ($self->{name} . (')' . (Main::newline() . (';; Do not edit this file - Lisp generated by ' . ($Main::_V6_COMPILER_NAME . (Main::newline() . (set_secure_mode($args_secure) . ($self->{body}->emit_lisp() . Main::newline())))))))))))))))))) }
...
sub emit_lisp { my $self = shift; my $List__ = \@_; do { [] }; my  $table = { '$' => 'kp6-','@' => 'kp6-List_','%' => 'kp6-Hash_','&' => 'kp6-Code_', }; do { if (($self->{name} eq 'KP6')) { return('*kp6-packages*') } else {  } }; do { if (($self->{twigil} eq '.')) { return(('::DISPATCH( $self, "' . ($self->{name} . ('" )' . Main::newline())))) } else {  } }; do { if (($self->{name} eq '/')) { return(($table->{$self->{sigil}} . 'MATCH')) } else {  } }; return(Main::mangle_name_lisp($self->{sigil}, $self->{twigil}, $self->{name}, $self->{namespace})) };
...
sub emit_lisp { my $self = shift; my $List__ = \@_; do { [] }; ('(if (kp6-value ' . ($self->{cond}->emit_lisp() . (')' . (($self->{body} ? ('(progn ' . ($self->{body}->emit_lisp() . ') ')) : '(progn)') . (($self->{otherwise} ? (' (progn ' . ($self->{otherwise}->emit_lisp() . ' )')) : '(progn)') . (' )' . Main::newline())))))) }
