...
sub emit_lisp { my $self = shift; my $List__ = \@_; my $args_secure; do {  $args_secure = $List__->[0]; [$args_secure] }; ('{ package ' . ($self->{name} . ('; ' . (Main::newline() . ('# Do not edit this file - Perl 5 generated by ' . ($Main::_V6_COMPILER_NAME . (Main::newline() . ('use v5;' . (Main::newline() . ('use strict;' . (Main::newline() . ('no strict \'vars\';' . (Main::newline() . (set_secure_mode($args_secure) . ('use ' . (Main::get_compiler_target_runtime() . (';' . (Main::newline() . ('my $_MODIFIED; BEGIN { $_MODIFIED = {} }' . (Main::newline() . ('BEGIN { ' . ('$_ = ::DISPATCH($::Scalar, "new", { modified => $_MODIFIED, name => "$_" } ); ' . ('}' . (Main::newline() . ($self->{body}->emit_lisp() . (Main::newline() . ('; 1 }' . Main::newline()))))))))))))))))))))))))))) }
