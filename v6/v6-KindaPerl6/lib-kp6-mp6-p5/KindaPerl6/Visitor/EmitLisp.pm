...
sub emit_lisp { my $self = shift; my $List__ = \@_; my $args_secure; do {  $args_secure = $List__->[0]; [$args_secure] }; ('(defpackage :' . ($self->{name} . (')' . (Main::newline() . ('(in-package :' . ($self->{name} . (')' . (Main::newline() . (';; Do not edit this file - Lisp generated by ' . ($Main::_V6_COMPILER_NAME . (Main::newline() . (set_secure_mode($args_secure) . ('(load (compile-file "lib/KindaPerl6/Runtime/Lisp/Runtime.lisp"))' . (Main::newline() . ($self->{body}->emit_lisp() . Main::newline()))))))))))))))) }
