...
;
package Rule::Before; sub new { shift; bless { @_ }, "Rule::Before" } sub capture_count { my $self = shift; my $List__ = \@_; my $count; my $quantified; my $seen; do {  $count = $List__->[0];  $quantified = $List__->[1];  $seen = $List__->[2]; [$count, $quantified, $seen] }; do { if (($self->{assertion_modifier} ne '')) { return($count) } else {  } }; $self->{capture_to_array} = ($quantified || ($seen->{'before'} && 1)); do { if ($seen->{'before'}) { $seen->{'before'}->capture_to_array(1) } else {  } }; $seen->{'before'} = $self; $self->{rule}->capture_count(0, 0, {  }); $count }
;
package Rule::After; sub new { shift; bless { @_ }, "Rule::After" } sub capture_count { my $self = shift; my $List__ = \@_; my $count; my $quantified; my $seen; do {  $count = $List__->[0];  $quantified = $List__->[1];  $seen = $List__->[2]; [$count, $quantified, $seen] }; do { if (($self->{assertion_modifier} ne '')) { return($count) } else {  } }; $self->{capture_to_array} = ($quantified || ($seen->{'after'} && 1)); do { if ($seen->{'after'}) { $seen->{'after'}->capture_to_array(1) } else {  } }; $seen->{'after'} = $self; $self->{rule}->capture_count(0, 0, {  }); $count }
;
...
;
package Rule::Capture; sub new { shift; bless { @_ }, "Rule::Capture" } sub capture_count { my $self = shift; my $List__ = \@_; my $count; my $quantified; my $seen; do {  $count = $List__->[0];  $quantified = $List__->[1];  $seen = $List__->[2]; [$count, $quantified, $seen] }; $self->{position} = $count; $self->{capture_to_array} = ($quantified || ($seen->{$self->{ident}} && 1)); do { if ($seen->{$self->{ident}}) { $seen->{$self->{ident}}->capture_to_array(1) } else {  } }; $seen->{$count} = $self; $self->{rule}->capture_count(0, 0, {  }); ($count + 1) }
;
1;
