...
;
package Var; sub new { shift; bless { @_ }, "Var" } sub emit_html { my $self = shift; my $List__ = \@_; do { [] }; my  $table = { '$' => '$','@' => '$List_','%' => '$Hash_','&' => '$Code_', }; do { if (($self->{twigil} eq '.')) { return(('<span class="variable">$self->{' . ($self->{name} . '}</span>'))) } else {  } }; do { if (($self->{name} eq '/')) { return(($table->{$self->{sigil}} . 'MATCH')) } else {  } }; return(('<span class="variable">' . (Main::mangle_name($self->{sigil}, $self->{twigil}, $self->{name}) . '</span>'))) }
;
