...
sub constant { my $List__ = \@_; my $str; do {  $str = $List__->[0]; [$str] }; my  $len = Main::chars($str, ); do { if (($str eq Main::backslash())) { $str = (Main::backslash() . Main::backslash()) } else {  } }; do { if (($str eq Main::singlequote())) { $str = (Main::backslash() . Main::singlequote()) } else {  } }; do { if ($len) { ('( ' . ('( length($str) < ' . ($len . (' ) ?? (0) !! ( ' . ('  ( do { ' . (Main::singlequote() . ($str . (Main::singlequote() . (' eq substr( $str, $MATCH.to, ' . ($len . (') } ) ' . ('  ?? (1 + ($MATCH.to = ' . ($len . (' + $MATCH.to ))' . ('  !! (0) ' . '))'))))))))))))))) } else { return('1') } } }
