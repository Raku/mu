...
{ # kp6-perl5.pl tests
  ...
  local $ENV{HARNESS_PERL} = "$^X run_kp6_mp6_perl5.pl -Icompiled/perl5/lib";
  local $ENV{PERL5LIB} = '';
...
{ # kp6-perl5.pl tests
  my $perl5 = $ENV{HARNESS_PERL} || $^X;
  warn $@ if $@;
  local $ENV{HARNESS_PERL} = "$^X run_kp6_mp6_perl5.pl -Ilib-modules-kp6-mp6-p5 -Icompiled/perl5/lib";
  local $ENV{PERL5LIB} = '';
  local $Test::Harness::Switches = '';
  open(TESTS,"TESTS") || die "Can not open test list";
