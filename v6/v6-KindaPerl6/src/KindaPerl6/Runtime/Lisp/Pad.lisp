...
(defun kp6-with-pad-functions (pad interpreter-var parent-var)
  `((enclosing-pad () ,pad)
    ...
    (set-lexical-variable/k (name value)
     (when (kp6-exists ,pad name)
       (kp6-delete ,pad name)
       (kp6-store ,pad name (make-kp6-cell value))))
    ...
    (lookup-lexical-variable/c (name)
	 ...
	 (if ,parent-var
	     (lookup-lexical-variable/c name)
	     (kp6-error ,interpreter-var 'kp6-variable-not-found :name name))))
    (lookup-lexical-variable/k (name)
     (error 'fax-not-implemented))
