...
collection, the possible existance of some object system that does,
requires us to implement the mechanisms that make it possible. That is
why, besides MESSAGE, three other functions are defined in the
responder interface lowlevel type. The REFERENCE, RELEASE and WEAKREF
functions. This functions are supposed to be the hooks for
implementing a refcount garbage collector.

=head2 Who owns the object?

This is the most important question in the matter of when to call
REFERENCE and when to call RELEASE. And a simple policy shall describe
this decision:
...
When an object is created, it's owned by the code who called the
method that creates it.

=item

Passing an object to a capture implies transfering the ownership of
the object.

=item

Passing an object as the capture of a MESSAGE transfer the ownership
to the code implementing that.
...
Using an object as Intepreter, Responder Interface or Identifier of a
MESSAGE doesn't transfer the ownership.

=item

WEAKREF is used to return a weak reference to that object, it returns
a different pointer, owned by the code that called it. Calling WEAKREF
doesn't change the refcount.
