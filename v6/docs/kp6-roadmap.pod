...
As a side effect, kp6 will also be able to use native OO models, if available.
For example, PGE in the Parrot backend, and Moose in the Perl 5 backend, and native-perl5 in 
the MiniPerl6-on-Perl5 backend.
The kp6-MO-on-mp6 engine (still mostly unwritten) will still be supported, for benefit of other backends.
...
* Status: implemented a proof-of-concept

=item * Method and Sub refactoring

* Status: fixed some namespace problems, added more tests
...
* Status: added tests for positional multis, TODO: multi token tests
...
- method .APPLY() on Junction objects needs more work. Some tests fail because the runtime can't tell
...
* Status: it is now possible to implement lazy lists in plain Perl 6;
  not started

=item * gather / take

* Status: studying and testing lots of thread implementations.
...
- Implemented on the Perl 5 backend. Gather returns a lazy list.
...
* Status: implemented mkdir(), rmdir(); implemented the 'unsafe' flag
...
* Status: KP6 can compile itself, but the regex runtime still have problems.
...
* needs more tests; needs Makefile support.
...
=item * Upgrade the grammar engine in MiniPerl6

* we need MiniPerl6 in order to compile KindaPerl6

=item * Upgrade the KindaPerl6 grammar source code
