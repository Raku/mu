...
sub arguments { @_ == 1 ? ( $_[0]->{arguments} ) : ( $_[0]->{arguments} = $_[1] ) };
sub emit { my $self = shift; my $List__ = \@_; do { [] }; (my  $code = $self->{code}); do { if (($code eq 'self')) { return('sv-self') } else {  } }; do { if (($code eq 'make')) { return(('(return-from mp6-function ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')'))) } else {  } }; do { if (($code eq 'substr')) { return(('(sv-substr ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')'))) } else {  } }; do { if (($code eq 'say')) { return(('(format t ' . ('"' . ('~{~a~}~%' . ('"' . (' (list ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . '))'))))))) } else {  } }; do { if (($code eq 'print')) { return(('(format t ' . ('"' . ('~{~a~}' . ('"' . (' (list ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . '))'))))))) } else {  } }; do { if (($code eq 'infix:<~>')) { return(('(format nil ' . ('"' . ('~{~a~}' . ('"' . (' (list ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . '))'))))))) } else {  } }; do { if (($code eq 'warn')) { return(('(write-line (format nil ' . ('"' . ('~{~a~}' . ('"' . (' (list ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')) *error-output*)'))))))) } else {  } }; do { if (($code eq 'die')) { return(('(progn (write-line (format nil ' . ('"' . ('~{~a~}' . ('"' . (' (list ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')) *error-output*) (sb-ext:quit))'))))))) } else {  } }; do { if (($code eq 'array')) { return(Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ')) } else {  } }; do { if (($code eq 'prefix:<~>')) { return(('(format nil ' . ('"' . ('~{~a~}' . ('"' . (' (list ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . '))'))))))) } else {  } }; do { if (($code eq 'prefix:<!>')) { return(('(not (sv-bool ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ' ))'))) } else {  } }; do { if (($code eq 'prefix:<?>')) { return(('(sv-bool ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ' )'))) } else {  } }; do { if (($code eq 'prefix:<$>')) { return(('(sv-scalar ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')'))) } else {  } }; do { if (($code eq 'prefix:<@>')) { return(Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ')) } else {  } }; do { if (($code eq 'prefix:<%>')) { return(Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ')) } else {  } }; do { if (($code eq 'infix:<+>')) { return(('(+ ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')'))) } else {  } }; do { if (($code eq 'infix:<->')) { return(('(-' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')'))) } else {  } }; do { if (($code eq 'infix:<>>')) { return(('(> ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')'))) } else {  } }; do { if (($code eq 'infix:<x>')) { return(('(x ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')'))) } else {  } }; do { if (($code eq 'infix:<&&>')) { return(('(and ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')'))) } else {  } }; do { if (($code eq 'infix:<||>')) { return(('(or ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')'))) } else {  } }; do { if (($code eq 'infix:<eq>')) { return(('(sv-eq ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')'))) } else {  } }; do { if (($code eq 'infix:<ne>')) { return(('(not (sv-eq ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . '))'))) } else {  } }; do { if (($code eq 'infix:<==>')) { return(('(eql ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')'))) } else {  } }; do { if (($code eq 'infix:<!=>')) { return(('(not (eql ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . '))'))) } else {  } }; do { if (($code eq 'ternary:<?? !!>')) { return(('(if (sv-bool ' . ($self->{arguments}->[0]->emit() . (') ' . ($self->{arguments}->[1]->emit() . (' ' . ($self->{arguments}->[2]->emit() . ')'))))))) } else {  } }; ('(' . (Main::to_lisp_identifier($self->{code}) . (' ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')')))) }
