...
use v6-alpha;
class CompUnit { has $.name; has $.attributes; has $.methods; has $.body; method emit { do { [] }; ('class ' ~ ($.name ~ (' { ' ~ (@.body>>.emit().join('; ') ~ '} ')))) }} 
;
...
;
class Val::Object { has $.class; has $.fields; method emit { do { [] }; ($.class.perl() ~ ('.new(' ~ (%.fields.perl() ~ ')'))) }} 
;
...
;
class Var { has $.sigil; has $.twigil; has $.name; method emit { do { [] }; do { if (($.name eq '/')) { return('$MATCH') } else {  } }; return(($.sigil ~ ($.twigil ~ $.name))) }; method name { do { [] }; $.name }} 
;
...
;
class Call { has $.invocant; has $.hyper; has $.method; has $.arguments; method emit { do { [] }; my  $invocant = $.invocant.emit(); do { if (($invocant eq 'self')) { $invocant = 'self' } else {  } }; my  $meth = $.method; do { if (($meth eq 'postcircumfix:<( )>')) { $meth = '' } else {  } }; my  $call = ('.' ~ ($meth ~ ('(' ~ (@.arguments>>.emit().join(', ') ~ ')')))); do { if ($.hyper) { ($invocant ~ ('>>' ~ $call)) } else { ($invocant ~ $call) } } }} 
;
class Apply { has $.code; has $.arguments; method emit { do { [] }; my  $code = $.code; do { if ($code.isa('Str')) {  } else { return(('(' ~ ($.code.emit() ~ (').(' ~ (@.arguments>>.emit().join(', ') ~ ')'))))) } }; do { if (($code eq 'self')) { return('self') } else {  } }; do { if (($code eq 'say')) { return(('say(' ~ (@.arguments>>.emit().join(', ') ~ ')'))) } else {  } }; do { if (($code eq 'print')) { return(('print(' ~ (@.arguments>>.emit().join(', ') ~ ')'))) } else {  } }; do { if (($code eq 'array')) { return(('@(' ~ (@.arguments>>.emit().join(' ') ~ ')'))) } else {  } }; do { if (($code eq 'prefix:<~>')) { return(('~(' ~ (@.arguments>>.emit().join(' ') ~ ')'))) } else {  } }; do { if (($code eq 'prefix:<!>')) { return(('!(' ~ (@.arguments>>.emit().join(' ') ~ ')'))) } else {  } }; do { if (($code eq 'prefix:<?>')) { return(('?(' ~ (@.arguments>>.emit().join(' ') ~ ')'))) } else {  } }; do { if (($code eq 'prefix:<$>')) { return(('$(' ~ (@.arguments>>.emit().join(' ') ~ ')'))) } else {  } }; do { if (($code eq 'prefix:<@>')) { return(('@(' ~ (@.arguments>>.emit().join(' ') ~ ')'))) } else {  } }; do { if (($code eq 'prefix:<%>')) { return(('%(' ~ (@.arguments>>.emit().join(' ') ~ ')'))) } else {  } }; do { if (($code eq 'infix:<~>')) { return(('(' ~ (@.arguments>>.emit().join(' ~ ') ~ ')'))) } else {  } }; do { if (($code eq 'infix:<+>')) { return(('(' ~ (@.arguments>>.emit().join(' + ') ~ ')'))) } else {  } }; do { if (($code eq 'infix:<->')) { return(('(' ~ (@.arguments>>.emit().join(' - ') ~ ')'))) } else {  } }; do { if (($code eq 'infix:<&&>')) { return(('(' ~ (@.arguments>>.emit().join(' && ') ~ ')'))) } else {  } }; do { if (($code eq 'infix:<||>')) { return(('(' ~ (@.arguments>>.emit().join(' || ') ~ ')'))) } else {  } }; do { if (($code eq 'infix:<eq>')) { return(('(' ~ (@.arguments>>.emit().join(' eq ') ~ ')'))) } else {  } }; do { if (($code eq 'infix:<ne>')) { return(('(' ~ (@.arguments>>.emit().join(' ne ') ~ ')'))) } else {  } }; do { if (($code eq 'infix:<==>')) { return(('(' ~ (@.arguments>>.emit().join(' == ') ~ ')'))) } else {  } }; do { if (($code eq 'infix:<!=>')) { return(('(' ~ (@.arguments>>.emit().join(' != ') ~ ')'))) } else {  } }; do { if (($code eq 'ternary:<?? !!>')) { return(('(' ~ (@.arguments.[0].emit() ~ (' ?? ' ~ (@.arguments.[1].emit() ~ (' !! ' ~ (@.arguments.[2].emit() ~ ')'))))))) } else {  } }; ($.code ~ ('(' ~ (@.arguments>>.emit().join(', ') ~ ')'))) }} 
;
class Return { has $.result; method emit { do { [] }; ('return(' ~ ($.result.emit() ~ ')')) }} 
;
class If { has $.cond; has $.body; has $.otherwise; method emit { do { [] }; ('do { if (' ~ ($.cond.emit() ~ (') { ' ~ (@.body>>.emit().join(';') ~ (' } else { ' ~ (@.otherwise>>.emit().join(';') ~ ' } }')))))) }} 
;
class For { has $.cond; has $.body; has $.topic; method emit { do { [] }; my  $cond = $.cond; do { if (($cond.isa('Var') && ($cond.sigil() eq '@'))) { $cond = Apply.new( 'code' => 'prefix:<@>','arguments' => [$cond], ) } else {  } }; ('do { for ( ' ~ ($cond.emit() ~ (' ) -> ' ~ ($.topic.emit() ~ (' { ' ~ (@.body>>.emit().join(';') ~ ' } }')))))) }} 
;
class Decl { has $.decl; has $.type; has $.var; method emit { do { [] }; my  $decl = $.decl; my  $name = $.var.name(); (($decl eq 'has') ?? ('has $.' ~ $.var.name()) !! ($.decl ~ (' ' ~ ($.type ~ (' ' ~ $.var.emit()))))) }} 
;
