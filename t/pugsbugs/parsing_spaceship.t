...
my $result_2 = [+] %ball{10..12} <=> [+] %ball{1..3};
my $result_3 = ([+] %ball{10..12}) <=> ([+] %ball{1..3});
my $result_4 = ([+] %ball{11,12} <=> [+] %ball{1,2});
my $result_5 = [+] %ball{11,12} <=> [+] %ball{1,2};
my $result_6 = ([+] %ball{11,12}) <=> ([+] %ball{1,2});
my $result_7 = ([0] <=> [0,1]);
...
is($result_4, -1, 'When spaceship terms are non-trivial members it parses incorrectly'); 
is($result_5, -1, 'When spaceship terms are non-trivial members it parses incorrectly'); 
is($result_6, -1, 'When spaceship terms are non-trivial members it parses incorrectly'); 
is($result_7, -1, 'When spaceship terms are non-trivial members it parses incorrectly');