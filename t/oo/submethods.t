...
', "class definitions were parsed/run/compiled", :todo(1);

eval_ok 'my $a = Foo.new()',  "Foo.new() worked (1)", :todo(1);
is      $was_in_foo_build, 1, "Foo's BUILD was called", :todo(1);
# is instead of todo_is to avoid unexpected succeedings
is           $was_in_bar_build, 0, "Bar's BUILD was not called";

eval_ok 'my $b = Bar.new()',  "Bar.new() worked", :todo(1);
is      $was_in_foo_build, 2, "Foo's BUILD was called again", :todo(1);
is      $was_in_bar_build, 1, "Bar's BUILD was called, too", :todo(1);
...
eval_ok 'my $c = Foo.new()',  "Foo.new() worked (2)", :todo(1);
is      $was_in_foo_build, 3, "Foo's BUILD was called again", :todo(1);
is      $was_in_bar_build, 1, "Bar's BUILD was not called again", :todo(1);
