...
todo_is(["a", "b", "c"].join("|"), "a|b|c", '[].join("|") works'); # unTODOme

my $joined1 = ("a", "b", "c").join("|");
todo_is($joined1, "a|b|c", '().join("|") works'); # unTODOme

my @list = ("a", "b", "c");

todo_is(@list.join("|"), "a|b|c", '@list.join("|") works'); # unTODOme
...
todo_is(["a", "b", "c"].join($sep), "a, b, c", '[].join($sep) works'); # unTODOme

my $joined1a = ("a", "b", "c").join($sep);
todo_is($joined1a, "a, b, c", '().join($sep) works'); # unTODOme

todo_is(@list.join($sep), "a, b, c", '@list.join($sep) works'); # unTODOme
...
todo_is(["a", "b", "c"].join(''), "abc", '[].join("") works'); # unTODOme

my $joined1d = ("a", "b", "c").join('');
todo_is($joined1d, "abc", '().join("") works'); # unTODOme

my @list = ("a", "b", "c");

todo_is(@list.join(''), "abc", '@list.join("") works'); # unTODOme
