...
{
    my $j = (1 | 2 | 3);
    is(ref($j), 'Junc', '$j is a Junc');

    todo_is(eval '$j.dump', 'all(1, 2, 3)', 'got the right stringified junction'); 
    ...
    my $sums = $j + 3;
    is(ref($sums), 'Junc', '$sums is a Junc');
                    
    my @sums_values = $sums.values;
    is(+@sums_values, 3, 'our junction has three values in it');
    is(@sums_values[0], 1, 'our junctions first value is 4');
    is(@sums_values[1], 2, 'our junctions second value is 5');
    is(@sums_values[2], 3, 'our junctions third value is 6');  
