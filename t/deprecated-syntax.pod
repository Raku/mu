...
=head2 comparison to undef

Nearly all comparisons of the form

    is $something, undef, '$something is undef';

are wrong, because in Perl 6 there are various undefined values, and usually
it's legal to return something that's undefined, but not the same as the most
general C<undef>. The correct comparison is one of those:

    ok !defined($something), 'description';
    ok !$something.defined,  'description';
    ok $something ~~ undef,  'description';
...
Use C<lives_ok> or C<dies_ok> where appropriate, or explicit parenthesis if
C<try> if used as an expression.

=head2 Interpolation in test descriptions
...
=head2 Type contraints on containers
...
=head2 Junction.values is now Junction.eigenstates

Calling C<.values> on a junction ordinarily autothreads. To get the values,
use C<Junction.eigenstates>.

=head2 Junctions are unordered

Junctions are unordered assemblies, and C<Junction.eigenstates> returns these
