...
resource limits, etc.). Finally, C<evalhelper.p5> runs C<pugs>.

=head1 INSTALLATION

There's no separate installation step needed, simply run C<evalbot.p6> and
supply a nick and an IRC server to connect to:

    $ pugs evalbot.p6 evalbot6 irc.freenode.net:6667

You don't have to restart C<evalbot.p6> when you install a new Pugs, as a new
C<pugs> is spawned on each command to eval.

C<evalhelper.p5> needs the C<BSD::Resource> module to be able to set
appropriate resource limits, so you can't run Evalbot under Win32.

=head1 REMOTE CONTROL

Once C<evalbot.p6> is connected to IRC, everybody can send it expressions to
eval:
