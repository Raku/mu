...
=head2 Flags separation

Pugs uses C<--> to separate flags of Pugs itself and arguments to the program.
For example, this will print C<--help>:

    $ pugs -e 'say @ARGS' -- --help

But this will show the help message of Pugs:

    $ pugs -e 'say @ARGS' --help

For the same reason, to use Pugs in the Unix C<#!> line, add an extra C<-->
after it:
...
backends are C<Parrot>, C<PIR>, C<Perl5>, and C<JS>.
...
backends are C<Pugs>, C<Parrot>, C<PIR>, C<Haskell>, C<JS>, and C<PIL> with its
