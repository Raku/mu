...
sub get_attribute {
    my ($class, $label) = @_;    
    debug "get_attribute called on $class with $label";
    $class->metaclass->get_attribute($label);    
}

sub has_attribute {
    my ($class, $label) = @_;
    debug "has_attribute called on $class with $label";
    $class->metaclass->has_attribute($label);        
}
...
sub has_method {
    my ($class, $label) = @_;
    $class->metaclass->has_method($label);
}

sub get_method {
    my ($class, $label) = @_;
    return $class->metaclass->get_method($label);
}

sub add_method {
    my ($class, $label, $method) = @_;
    debug "adding $label to $class";
    $class->metaclass->add_method($label, $method);    
}
...
sub add_method {
    ...
    die "You should not use $self -> add_method (called from " . (join ", " => caller()) . ") ";
}

sub has_method {
    my ($self, $label) = @_;
    die "You should not use $self -> has_method (called from " . (join ", " => caller()) . ") ";    
}

sub add_attribute {
    my ($self, $label, $property) = @_;
    die "You should not use $self -> add_attribute (called from " . (join ", " => caller()) . ") ";
}
...
sub class {
    warn "class called from " . (join ", " => caller()) . ") ";    
    (shift)->class_name . '::Class';    
}
...
"A Core Caculus for MetaClasses". The main idea of the system is that objects not only have
...
is how it might be described in "regular" OO:
...
Now with the MCP system not only handles some of the details for you, but the class heirarchy it creates
ends up mapping to the real world much more smoothly.
