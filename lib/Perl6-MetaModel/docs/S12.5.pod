...
This is an unofficial ammendment to the Synopsis 12, which descibes the 
design and implementation of the Perl6 Object MetaModel. It is very rough
at the moment, but will be filled in more as the work on this meta-model 
progresses.
...
A Class in Perl 6 is much the same as classes in other OO languagues. It is
a collection of methods and attributes which can be used to construct an 
Instance from.
...
A Role at it's most basic can be thought of as a collection of methods and
attributes which get included into a class. A very simplistic view of this
is that it is not unlike a language level  #INCLUDE directive. Roles share
similarities to other familiar OO concepts as mix-ins, and interfaces, but 
they differ in how they are composed into the class as well as the rules
which govern that composition. Roles will be discussed in further detail
...
(Note: Traits are what inspired the Perl6 concepts of Roles)
...
There is a good amount of documentation about the Objective C Runtime which details the class
and object structures. Here are some relevant links:
