...
Since there are no barewords in Perl 6, bare class names must be predeclared.
Alternately, for a literal package name you know will exist by the time of
evaluation, you can use the package literal prefix C<::> to introduce the
name, as in C<::ClassName>.  The C<::> prefix does not imply top-levelness
as it does in Perl 5.  (Use C<::*> or C<GLOBAL::> for that.)

The C<::> prefix always refers to a package name that is defined elsewhere.
In contrast, you can define a local class or type variable using the
C<^> type sigil.  In an rvalue context it is identical to C<::>, but
in a declarational context, it binds a new type name within its declared
scope.
...
operator by appending a colon:
    ...
    .doit(1): 2,3	# okay, no arguments plus list
