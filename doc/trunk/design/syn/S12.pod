...
In theory you can have an anonymous enum, but in practice it's pretty useless:
...
type name.  (All colliding values are hidden, not just the new one,
...
Two built-in enums are:
    ...
    our bit enum *bool handles «false true»;
    our bit enum *taint handles «untainted tainted»;
