...
Consider the use of C<roles> when you simply want to factor out
common code.
...
request that the class stay open or non-final.  Platforms that do
dynamic loading of sub-applications probably don't want to close or
finalize classes wholesale, however.

A private classname begins with a single colon.  It is completely invisible
to the ordinary classname mechanisms.
...
It is completely invisible to the ordinary method dispatch system.
...
It can use a variable for the method name:
...
Use C<tied> to get at the container of a scalar variable.
...
the default behavior.  If the name of a named argument corresponds
to a class or role being built, the list value of that argument is
passed as a list of named arguments to that class or role's C<BUILD>.
Otherwise all the arguments to C<bless> are passed to the C<BUILD>.
...
that could handle the request.  The ordinary "dot" operator dispatches
...
multiple long names to share a short name, as long as all of them are
...
them as multi, you can force a multi on the roles' methods by saying:

    multi shake {...}

If the original invocant is in a variable that is constrained to be
of one role or another, then that type can be used to dispatch to
the correct role's method.
...
compile-time alias, like this:
    ...
    subtype Str_not2b ::= Str where /^[isnt|arent|amnot|aint]$/;

A subtype is not a subclass.  A subtype is primarily a handy way of sneaking
smartmatching into multiple dispatch.  Just as a role allows you to
specify something more general than a class, a subtype allows you to
specify something more specific than a class.
...
bind or assign to the container.
    ...
    subtype Str_not2b ::= Str where /^[isnt|arent|amnot|aint]$/;
    subtype EvenNum   ::= Num where { $^n % 2 == 0 }
...
An enum is a low level class that can function as a role.  A given
enum value can function either as a subtype or as an ordinary value.
The values are specified as a list:

    my enum day is «Sun Mon Tue Wed Thu Fri Sat»;
    my enum day is ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];

The default return type is C<int> or C<str> depending on the type of
the first value.  The type can be specified:

    my enum day returns int is «Sun Mon Tue Wed Thu Fri Sat»;
    my enum day of int is «Sun Mon Tue Wed Thu Fri Sat»;
    my int enum day is ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];

The enum installer inspects the values for things that look like
pairs, where the value of the pair sets the next value explicitly.
Non-pairs C<++> the previous value.

    my enum DayOfWeek is «:Sun(1) Mon Tue Wed Thu Fri Sat»;

    our str enum Phonetic is « :Alpha«A» Bravo Charlie Delta
				Echo Foxtrot Golf Hotel India Juliet
				Kilo Lima Mike November Oscar Papa
				Quebec Romeo Sierra Tango Uniform
				Victor Whiskey X-ray Yankee Zulu »

    enum roman is [i => 1,   v => 5,
		   x => 10,  l => 50,
		   c => 100, d => 500,
		   m => 1000];

You may import enum types; non-colliding values function as
argumentless constant subs.  Colliding enum values are hidden and must
be disambiguated with the type name.  (All colliding values are hidden,
not just the new one, or the old one.)  Any sub definition hides all
...
that any class stay open or non-final, and the class closer/finalizer
must honor that.

    use class :open«Mammal Insect» :base«Str»
