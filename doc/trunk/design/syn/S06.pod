...
Required parameters may optionally be declared with a C<+>:
...
Named parameters are optional unless marked C<+>.  Default values for
...
=head2 Class parameters

Unlike normal parameters, class parameters often come in piggybacked on
the actual value, and you'd like a way to capture both the value and
its class at once.  A Class variable can be used anywhere a class name
can, but instead of asserting that the value must conform to a particular
class, instead captures the actual class of object and declares a
temporary type by which you can refer to that class later in the
signature or body.  For instance, if you wanted to match any two
Dogs or Cats as long as they were of the same type, you can say:

    sub matchedset (Dog|Cat ¢T $fido, T $spot) {...}

(Note that C<¢T> is not required to contain C<Dog> or C<Cat>, only
a type that is compatible with one or the other.)

The C<¢> sigil is short for "class" in much the same way that C<&> is
...
an array of children named C<.kids> or C<.<kids>, use something like:
...
is the same as:
    ...
    submethod initialize(:name($.name), :($.age)) {}
...
The C<¢> sigil is just short for 'class', so if your keyboard is
deficient in Latin-1 you can write:

    sub compare (Any class T $x, T $y) {
	return $x eqv $y;
    }
...
that variable is declared "C<is env>".  (Implicit lexicals such as C<$_>
are automatically assumed to be environmental.)

C<< ENV::<$varname> >> specifies the C<$varname> visible in the
innermost dynamic scope that declares the variable as "C<is env>".
