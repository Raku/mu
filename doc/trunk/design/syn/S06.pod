...
There is a corresponding shortcut for hash keys if you prefix the subscript
instead of the sigil:
...
The C<:> is not functioning as an operator here, but as a modifier of
the following token.
...
doesn't commit to any further processing.)  If there is only one
argument list remaining, C<@args> will contain only one element,
and that one element is exactly what would have been bound to C<$args>
had you used that instead.
...
Since the pipe is bound to the final variable, the variable is just as
lazy as the pipe.

The "missing" list may be indicated by the C<< (*) >> "pipe here" token,
otherwise known as the "unrequited star".  The parens are optional when
followed by a comma, semicolon or right bracket, or by any operator looser
than comma.  So the above is equivalent to:
...
This is more useful when you wish to pipe into an expression that is
not an ordinary list operator:

    picklist() ==>
	my @foo = @bar[*];

Various contexts may or may not be expecting multi-dimensional
slices or pipes.  It is an error to feed multiple slices or pipes to
a destination that is not expecting it.  It must be fed to a context
that is expecting semicolons to separate lists, such as a subscript,
or a zip-like function that specifies the order to process multiple lists:
...
The C<^> sigil is just short for 'type', so you can also write:
