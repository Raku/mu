...
The C<\G> sequence is gone.  Use C<< <( .pos == $oldpos )> >>.
...
The numeric variables (C<$1>, C<$2>, etc.) are automatically bound
"hypothetically".
...
You may also bind the numberic variables explicitly in closures, in which
...
In string context it evaluates to the captured substring (if there
was exactly one capture in the pattern) or to the entire text that
was matched (if the pattern does not capture, or captures multiple
elements):
...
For this reason $0 is not considered part of the array.
