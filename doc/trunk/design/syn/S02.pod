...
that the native type can speed it up.)
...
method that returns the metaclass managing the current class.
Any object (whether defined, undefined, class or instance) can be
used as a class when stored in a class variable such as C<¢MyClass>.
...
are predeclared, or prefixed with the C<::> marker.)  A consequence of
