...
You may interpolate any Unicode codepoint by name using C<\c> and square brackets:

    "\c[NEGATED DOUBLE VERTICAL BAR DOUBLE RIGHT TURNSTILE]"

Multiple codepoints constituting a single may be interpolated with
...
Unicode support level of the current lexical scope.  It is also possible to
