...
When applied to nothing, it iterates the magic C<$*ARGS> iterator (or
the current set of subscripts for this dimension, when used within a subscript).
So Perl 5's

    while (<>) {...}

construct is instead written:
