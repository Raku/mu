...
# Attempting the smoke
system($^X, qw(-w ./util/yaml_harness.pl), @yaml_harness_args) == 0 
    or die "Could not run yaml harness: $!";
system($^X, qw(-w ./util/testgraph.pl), ('--inlinecss', $yml_location), $html_location) == 0
    or die "Could not convert .yml to testgraph: $!";

if (defined $ENV{PUGS_SMOKE_UPLOAD}) {
    system("$^X $ENV{PUGS_SMOKE_UPLOAD} $html_location $yml_location") == 0
        or die "Couldn't run user smoke upload command: $!";
}
...
if ($smoke_upload) {
  if (defined $smoke_upload_script) {
    system($^X, $smoke_upload_script, $html_location, $yml_location) == 0
        or die "Couldn't run smoke upload script: $!";
  }
} else {
    print <<EOF;
