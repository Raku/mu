...
sub fix_randomness {
    ...
    while (<$in>) {
        ...
        s/\bsmartlink_skip_\d+\b/smartlink_skip_XX/g;
        s/\bsmartlink_skipto_\d+\b/smartlink_skipto_XX/g;
        s/\bPerl v5\.\d+\b at [^\n<>]+/Perl v5.XXX at XXX./g;
        s/\btog_quote\(\d+\)/tog_quote(XX)/g;
        s/(generated by Pod::Simple::HTML) v[\d\.]+/$1 vX.XX/g;
        s/(using Pod::Simple::PullParser) v[\d\.]+/$1 vX.XX/g;
        s/\s+$/\n/gms;
        print $out $_;
    }
    close $out;
