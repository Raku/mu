...
the same thing.  But exactly how are they different?  The context tree
is a subtree of the type tree.  However, the context tree has a
constraint:  No multiple inheritance[3].  We need this so we can do
well-defined multi dispatch on contexts[4].

Recall that we also required that there be no common supercontext of
C<Singular> and C<Plural>.  This means that operators need only decide
what to do in those two contexts and the dispatch is completely
...
In order to define a type to be a context, derive from some other
context.  

    class Foo does Int {...}
    class Bar does Plural {...}
...
The reason that a type being a context is simply based on derivation
instead of marking it in some way (like C<is context>) is that if a
"regular type" derived from two contexts, you wouldn't be able to use
that type in explicit typing.  The way we can keep this from imposing a
"no multiple inheritance" law on the whole language is to only have
almost-leaves of the standard hierarchy be contexts.   Perhaps the only
contexts by default are:  C<Singular>, C<Plural>, C<List>, C<Num>,
C<Str>, C<Ref>.  You wouldn't be able to derive from more than one of
those at once, but all other multiple inheritance would be okay.
