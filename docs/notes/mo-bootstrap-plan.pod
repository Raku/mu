...
to allow for truly unassignable (even for class-local methods) slots:
    ...
    class C {
        ...
        method BUILD ($.foo) {
            ...
            # allow this; instead of (as in nothingmuch++'s MO)
            # initializing a slot and then assigning it a value
            # supplied by the constructor method, we need to mangle
            # the CREATE call so it happens right _after_ the binding
            # stage for BUILD, but _before_ any ENTER block fires
            # for the BUILD method body.
        }
        method foo {
...
(Map TypeRep InterfaceTable) that simply assumes a layout of native
Haskell types and respond to them using IT.  It's unfortunate that we'll
have to use fromTypeable on this one, but since it's all monotypes
(we don't have parametric types until 6.2831 anyway), this layout should
carry us forward for another couple of milestones.  (And by that time
we'll have acces to GHC 6.8's type families.)
