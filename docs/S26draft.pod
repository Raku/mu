...
The first thing to tackle is how various interpreters including the Perl
interpreter distinguish which characters in a file or stream are actual
Perl code and which ones are Perldoc.
...
Perldoc currently keeps this restriction for two reasons:
...
acts not only as containment starting marker, but as part of the content
(a heading).

=head2 Containment Differences in Perldoc

In general Perldoc is backwards compatible with Pod. This give the Pod
...
are not necessary in pure Perldoc files.
...
embedding other Kwid dialects (as well as opaque structures).
...
In the Pod dialect:
    ...
    The trademark symbol is E<trade>
