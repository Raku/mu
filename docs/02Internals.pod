...
sends it's envBody into the evaluator.
...
just a representation of the program in something a bit more manipulatable
...
value it contains. Here is the type signature of C<evalRef>, for refernce:
...
and gives it C<these> as it's paramters on the line starting with C<map>.
...
When all the auxillery functions have been define, we can run the body with
...
Subroutine application be very simple, in the case of a C<Prim>. At other
