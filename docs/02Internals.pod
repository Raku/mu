...
declaration using C<VControl>, while C<reduce (Cxt cxt exp)> forces the 
subexpression C<exp> to be evaluated in the context C<cxt>.
...
lines are some details which we don't care about right now. Let's pretend they
