...
words, prod someone on #perl6, or read http://use.perl.org/~autrijus/journal and
see if there's been any big change.

The currenct copy was last revised on 2005-05-21.
...
itself fits into a star toplogy, optionally using these projects to gain more
features.
...
Pugs' own core is also componentized. The separation roughly coincides with Perl
...
This file implements the evaluation logic for the AST. It's main job is reducing
...
Declarations of sorts create a node in the syntax tree called a C<Syn>. These
nodes, when evaluated, create a type of C<Exp> that will modify the C<Env>,
adding a new symbol, and then roll back the change later. They are also
evaluated immediately using C<unsafeEvalExp>.
...
serves a different purpose, and most are pretty streight forward.
...
This takes care the case where there are no more lements in C<list>. Contrast it
with:
