...
蓄的文本陈述。缺省地，Perl 以 “NFG” 结构呈现 Unicode，它每一个字形视为一
个字符。一个字形就是初级用户在他们每天正常生活会认为的一个字符，包括任何
发音符号。
...
Perl 会把 Unicode 行和段落分隔符视为行标记，但这种行为最好可配置，以
便 Perl 的行匹配符合编辑器对 Unicode 行的思想。
...
Unicode 横向空白被视为空白，但最好不要在使毗邻记号看起来像单一记号的时候使
用紧间距。另一方面，Perl 不使用缩进作为语法，所以你可以在任何空白有意义的
地方自由使用任意数量的空白。注释常常被视为空白。
...
出于某些语法目的，Perl 区分括号字符和非括号字符。括号字符被定义为任何双向
镜像（Bidirectional mirrorings）或 Ps/Pe/Pi/Pf 属性的 Unicode 字符。
...
和向后映射）。对于任何给定的 Ps 字符，下一个 Pe 字符码（按数字顺序）被假
...
C<U+301D> 字符码有两个闭括号可选，C<U+301E> 和 C<U+301F>; Perl 6 只识别较小的字符
码号码 C<U+301E> 作为闭括号。这项政策也适用于后面新推出的一对多映射。
...
然而，多对一映射是可以的：多个开始符号，可以映射到同样的结束符号。举个例子，
...
（实际上，C<=cut> 现在没有了。）格式名字不一定是 C<comment> —— 任何未认可的格式
名字都使之成为注释。（然而，赤裸的 C<=begin> 和 C<=end> 可能不大好，因为它们之间
所有的注释将会在格式化输出中显示出来。
...
除了在逐字字符串中，C<#>操作符常常在 Perl 6 中引入注释。基于 C<#> 有两种格式的
...
在引用语法中，支持一个嵌入式注释变体，采用 C<#`> 加上任何用户选择的括号字符
（正如上面 C<词法约定> 的定义）
...
单元的结构。（也就是说，Perl 6 遵循 标准的 I<最长记号> 原则，或
...
例如：如果你添加你自己的 C<< infix:<++> >> 操作符，那么它之前必须有个空格。
...
Perl 6 有个可选的类型系统来帮助你写效率更高的安全代码。
...
根据 S12，properties 实际上是通 1 种混合机制实现，
...
然而，一些内置思想比如 filehands（文件句柄）在用户可见习惯将会比 Perl 5 
更加面向对象。
