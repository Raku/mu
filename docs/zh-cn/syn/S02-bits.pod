...
C<.codes> 或者 C<.graphs> 以字节，字符编码或者字形来请求数组元素的总字符串长
...
像 C<int> 和 C<num> 小写类型意味着对整数和浮点数的本地机器表现。
...
层，并巧妙地参与当前理想字符词法范围的构想，不管那是字节，字符编码，字形或者某些
...
示 18 字节，7 个字符编码，3 个字形 和 马拉雅拉姆语的一个字母，但它可能只懒洋洋地
