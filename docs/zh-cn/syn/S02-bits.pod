...
进行语法分析。也就是说，不鼓励一个以上“最长标记”预测。目前语法分析器已
知必须例外的地方是：
...
概括地，Perl 使用 Unicode 编写，并且拥有一致的 Unicode 语义学，而不考虑含
...
Unicode 横向空白被视为空白，但最好不要在使邻近标记看起来像单一标记的地方使用窄
空格。另一方面，Perl 不使用缩进作为语法，所以你可以在任何空白有意义的地方自由使
用任意数量的空白。注释常常被视为空白。
...
C<#`> 与开括号字符之间必须不能有空格。（不过双宽度字符可能有空格I<视觉呈现>，比
如上面的角引号。）
...
式）允许反斜杠空格字符。Per 6 概括这种观念为任何种类的空白字符。通过使用前
缀 C<\>，任何连续的空白（包括注释）都会从语法分析器中隐藏。这称为“虚空白”。
在 Perl 中，虚空白可以阻止任意若干个空白关联。举个例子，既然 Perl 需要在名词
和后缀操作符之间缺席，使用虚空白让你与后缀操作符排成行。
...
作为支持上面使用方式的特例，后缀操作符要求有反斜杠的地方被认为是虚空白的退化。
注意在反斜杠之前是不允许有空白，因此：
...
然而，空白实际上可以紧跟着其它形式的虚空白。（因此反斜杠的单一使用后面绝不可
能有空白或它们将被视为虚空白。）
...
另外一种你看不见该空白的普遍用法通常是放一个点后缀操作符到下一行：
...
但虚空白主要是语言的扩展性：它让你在任何可能搞混语法分析器的地方继续该行，不管
你现在安装的语法分析器。（当然，除非你覆盖了它自己的虚空白规则……）
...
尽管我们说虚空白从语法分析器中隐藏了空白，但它没有从词法分析器中隐藏。因此，虚
空白不允许在标记里面。此外，如果虚空白包含一个以上的新行，行数仍然会被统计。既
然对于该语言 Pod 块被视为空白，它们也被虚空白吞噬。然而 Heredoc 分界线是抑制的，
...
注意这是可以让空白放在虚空白之前的其中一个例子，因为我们只是尝试抑制新行转换，
...
虚空白可以包含注释，但注释不可以包含虚空白，特别地，行尾注释认为反斜杠符号没有意义。
如果你写：
...
通常，Perl 6 中的空白是可选的，除非它需要用来分割被误解成一个单标记或者其它语法
单元的结构。（也就是说，Perl 6 遵循 标准的 I<最长标记> 原则，或大型结构，一
个 I<喜欢转移而不是减少> 原则。关于 Perl 程序如何分析为标记，参考下面
的 L<文法分类> 以获得更多信息。）
...
符与后缀操作符之间有个自然冲突，它们任何一个都会出现在术语后面。如果给定的标记
被解释成后缀操作符或中缀操作符，那么中缀操作符之前需要一个空格。后缀操作符应该
决没有介于中间的空格，尽管它们可能有一个介于中间的点。如果进一步要求分离，虚
空白或者嵌入式注释应该正如上描述的那样使用，只要没有空白出现在虚空白或者嵌入式
注释之外。
...
例如：如果你添加自己的 C<< infix:<++> >> 操作符，那么它之前必须有个空格。标准的
自增 C<< postfix:<++> >> 操作符之前可能从来没有空格，但是可以以任何这些形
式编写：
...
一个后缀规则的推论是（除非划分引用界限或者终止虚空白），点之前的空白常常被认
...
根据 S12，property 实际上是通过一种混合机制实现，这样的混合机制是为对象通过一个
...
变量类型是一种约束，它说明了变量可能包含什么类型的值。更确切地说，它承诺变量中
...
任何作为值使用的类型名称是该类型原型对象的未定义实例化，或简称“类型对象”。参
考 S12 以获得更多那方面的信息。
...
被修改；C<StrPos> 不是一个标记，也不需要遵循可变字符串的变化。举个例子，如果你请
