...
Perl 会把 Unicode 行和段落分隔符视为行标记，但
...
式）允许反斜杠空格字符。Per 6 概括这种观念为任何种类的空格字符。使用前缀 C<\>，
任何连续的空格（包括注释）都会从语法分析器中隐藏。这称为“无间隙”。
