...
和向后映射）。对于任何给定的 Ps 字符，下一个 Pe 字符编码（按数字顺序）被假
...
C<U+301D> 字符编码有两个闭括号可选，C<U+301E> 和 C<U+301F>; Perl 6 只识别较小的
编码编码 C<U+301E> 作为闭括号。这项政策也适用于后面新推出的一对多映射。
...
为了获取数字中的元素编号，使用 C<.elems> 方法。 你也可以询问数组元素的总串长
度，以字节，字符编码，或者
