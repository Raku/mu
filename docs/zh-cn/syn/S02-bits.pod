...
Unicode 横向空白被视为空白，但最好不要在使毗邻标记看起来像单一标记的时候使用紧
...
C<#`> 与开括号字符之间必须不能有空白。（不过双宽度字符可能有空白视觉出现，比
如上面的角引号。）
...
式）允许反斜杠空白字符。Per 6 概括这种观念为任何种类的空白字符。通过使用前
缀 C<\>，任何连续的空白（包括注释）都会从语法分析器中隐藏。这称为“非空白”。
在 Perl 中，非空白可以阻止任意若干个空白关联。举个例子，既然 Perl 需要在名词
和后缀操作符之间缺席，使用非空白让你与后缀操作符排成行。
...
作为支持上面使用方式的特例，后缀操作符要求有反斜杠的地方被认为是非空白的退化。
注意在反斜杠之前是不允许有空白，因此：
...
然而，空白实际上可以紧跟着其它形式的非空白。（因此反斜杠的单一使用后面绝不可
能有空白或它们将被视为非空白。）
...
但非空白主要是语言的扩展性：它让你在任何可能搞混语法分析器的地方继续该行，不管
你现在安装的语法分析器。（当然，除非你覆盖了它自己的非空白规则……）
...
尽管我们说非空白从语法分析器中隐藏了空白，但它没有从词法分析器中隐藏。因此，反
空白不允许在标记里面。此外，如果非空白包含一个以上的新行，行数仍然会被统计。既
然对于该语言 Pod 块被视为空白，它们也被非空白吞噬。然而 Heredoc 分界线是抑制的，
...
注意这是可以让空白放在非空白之前的其中一个例子，因为我们只是尝试抑制新行转换，
...
非空白可以包含注释，但注释不可以包含非空白，特别地，行尾注释认为反斜杠符号没有意义。
如果你写：
...
符与后缀操作符之间有个天然冲突，它们任何一个都会出现在术语后面。如果给定的标记
被解释成后缀操作符或中缀操作符，那么中缀操作符之前需要一个空白。后缀操作符应该
决没有介于中间的空白，尽管它们可能有一个介于中间的点。如果进一步要求分离，反
空白或者嵌入式注释应该正如上描述的那样使用，只要没有空白出现在非空白或者嵌入式
注释之外。
...
例如：如果你添加自己的 C<< infix:<++> >> 操作符，那么它之前必须有个空白。标准的
自增 C<< postfix:<++> >> 操作符之前可能从来没有空白，但是可以以任何这些形
式编写：
...
一个后缀规则的推论是（除非划分引用界限或者终止非空白），点之前的空白常常被认
