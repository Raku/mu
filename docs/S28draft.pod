...
=head1 VERSION
 ...
 Created:         2 Apr 2005    anonpugster
 Last Modified:  11 Apr 2005    dvergin

This work-in-progress should probably be restructured to something
resembling perlvar.pod once it's current form as a checklist of 
the correspondence of Perl 5 and Perl 6 special vars is reasonably
complete.

This document collects what is known about special variables in
Perl 6 and correlates them with the changes from Perl 5. 
At its present stage of development this file
draws on a few key sources: Perl 5's perlvar.pod (for a full list of special
vars in Perl 5) and a perl6-language list thread documented in 
the APPENDIX below. E02 and S02 have also been helpful in several cases.

=head1 SUMMARY

NOTE: Most/All variables of the form $*SOMETHING should also work in the form
$SOMETHING (without the '*') unless masked by "my $SOMETHING".

=over 4

Because a blank entry in the "Perl 6" column could be taken to mean either 
"not in Perl 6" or "unknown", the information as presented 
below presumes an entry in that column for every entry. 
It must be the appropriate Perl 6 entity, 
'-' for "not in Perl 6", or '?' for "don't know yet". 
Other organizational
schemes will become appropriate when the table is more complete.

=back

 Perl 6  Perl 5         Comment
 ------  ------         -----------------------
 ...
 $_      $_  $ARG       Same as Perl 5 but lexically scoped.
 $^a,$^b...             Parameters of the current closure (block 
                        or subroutine) by position in the invocation.

 -       $a,$b          Ordinary vars. No longer any special use.


 $/                     Object containing the results of the 
                        last regular expression match. All match
                        result info found in the contained data
                        structure. This is available as the current
                        state of the match in progress.
                        
 $1,$2.. $1,$2..        Shortcut to same info in $/ with data
                        structures containing such things as .start
                        and .end (Unicode level independent).

 $0      $&  $MATCH     $0 is what would be returned by $1 if you had
                        put parens around the entire match. $0 is not
                        bound until the match succeeds. 
 $<pre>  $`  $PREMATCH
 $<post> $'  $POSTMATCH <pre> and <post> would automatically exclude 
                        themselves from $0.  Or you need some special 
                        flag to remember them, maybe.
 -       $+  $LAST_PAREN_MATCH
 $*MOST_RECENT_CAPTURED_MATCH  ...or some such. 
                        Or $/[-1]. Or $/[-$n]. Or omit.
         $^N
 ?       @+  @LAST_MATCH_END
 -       $*
...
NOTE: This section should likely be reduced to just the url below in 
the next major revision of this pod.

The new material above was compiled from an a thread on the perl6-language
...
head post found
at the url given above. It is an item by Larry Wall which is reproduced here:
