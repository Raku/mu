...
=head2 qw() has a customary form; new interpolating form
...
Quoting operators now have modifiers that can be used with them (much
like regexes and substitutions in Perl 5), and you can even define
your own quoting operators.  See S03 for details.
...
Many many changes.  Just read S05.  But if you're in a panic and need
to get something working now, you just need to know that you can use
Perl 5-compatible regexes with the C<:P5> modifier.  And that
modifiers come before, not after, regexes in Perl 6.  Oh, and that
C<=~> has changed to C<~~>....

    Was:    $str =~ /^\d{2,5}\s/
    Now:    $str ~~ m:P5/^\d{2,5}\s/
