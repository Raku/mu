...
The arrow operator C<< -> >> for dereferencing is gone. Since (nearly)
everything is a reference, you can directly use the appropriate pair of
...
available, you have to but them into the C<GLOBAL> namespace:
...
C<wantarray> is superseded by C<want.list>. Also available are C<want.item>
