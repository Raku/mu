...
See L<S04/"The for statement"> for details. 
...
The new syntax didn't test out to work. Waiting on confirmation
of correct examples. 

#=head2 references are now Capturers
...
when you want to refer only to a single item.
  ...
  Was: ref $foo eq 'ARRAY'
  Now: $foo ~~ 'Array'

  Was: ref $foo eq 'CODE'
  Now: $foo ~~ 'Code'

The "obsolete" reference above has the details. Also, look
for I<Capture> under L<S02/"Names_and_Variables">.
