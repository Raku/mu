...
Parenthesis don't construct lists any more, they merely group. Lists are
constructed with the comma operator. It has looser precedence than the list
assignment operator, which allows you to write lists on the right hand side
wihtout parens:
    
    my @list = 1, 2, 3;     # @list really has three elements

The arrow operator C<< -> >> for dereferncing is gone. Since (nearly)
everything is a reference, you can directly use the apropriate pair of
parenthesis for either indexing or method calls:
...
=head2 Using code references in dynamic method calls is gone.
...
available, you have to but them into the C<GLOBAL> namespace, which is
indicated by an asterisk.

    multi sub *uc(TurkishStr $str) { ... }
...
C<wantarray> is superseeded by C<want.list>. Also available are C<want.item>
and C<want.count>, the latter gives the number of expected values, if
applicable.

In general you are encouraged to return objects that do the right thing in
each possible context instead of asking for your context explicitly.
