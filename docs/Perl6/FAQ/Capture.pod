...
can never have more than one invocants.  Typically, the presence of an invocant
indicates a method-call (which may fall back to a subroutine-call); the lack of
invocant means a subroutine-call.

    1.foo(2);  # Int.foo with 1 as "self"; if not found, fallback to foo(1, 2)
    foo(1: 2); # same as above

    bar(1, 2); # never looks at Int.foo; calls &foo in lexical/package scope
...
[hw; there's probably a golfier way]
Perl has a few other provisions for this (e.g., WRAP), but if you want
more control over invocation, you need to pass through the Capture but
replace the invocant:

  method front_meth (\$args) { $:real_obj.back_meth( ??? ) }

  # [ does $obj.meth($args) ignore a prebound invocant? ]
