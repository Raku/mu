...
to pre-declare your variable types (though you can if you want -- see L<XXX>).
...
kinds of purposes, and are documented in L<XXX>.  The only one you
need to know about for now is C<$_> which is the "topic".  It's used
as the default argument to a number of functions in Perl, and it's set
...
C<values()>.
    ...
    my @fruits = %fruit_colors.keys;
    my @colors = %fruit_colors.values;
...
Again, no idea here... will get to it eventually.
