...
Perl 6 is a work in progress, so the syntax is still changing, but not 
as fast as it used to be.  The next 4 chapters are likely to be slightly
...
as a list, so parentheses are not needed around the list N<Although
you can have them too, if you prefer>.
...
The C<.keys> method returns a list of all the keys in a
hash. The C<.values> method returns a list of all the
values:
