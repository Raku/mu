...
floating-point numbers, and references to all kinds of objects and
...
Pairs hold a single key and a single value. They don't have a unique
sigil because they rarely appear alone, so they're stored in scalars,
...
hash key must have a C<.id> method that returns a unique value for
...
worry about uniqueness when you define your own C<.id> methods.
...
brackets for the ordinary curly braces around the key:
  ...
  $age = %hashE<laquo>ZaphodE<raquo>; # string
...
X<references>
References are largely transparent in Perl 6. There is a distinction
between references and ordinary variables, but it's minimized as much
as possible in actual use, with automatic referencing and
dereferencing where appropriate. Creating a reference to an array or
hash requires no special syntax. You simply assign it to a scalar
variable:
X<references;to arrays>
X<references;to hashes>
X<arrayrefs;creating>
X<hashrefs;creating>

  $arrayref = @array;
  $hashref = %hash;

X<arrayrefs;dereferencing>
X<hashrefs;dereferencing>
References are implicitly dereferenced in many contexts, so array
indexes and hash keys access individual elements directly from
hashrefs and arrayrefs, just like they do with hashes and arrays:
...
X<arrayrefs>
X<hashrefs>
Methods are called on arrayrefs and hashrefs just like they are on
arrays and hashes. The referent--the underlying data type or
object--determines which methods can be used with a particular
reference, what those methods do, and whether the reference can support
indexed access.
...
References to subroutines can be executed simply by passing the
reference an argument list. The list can be empty, but the parentheses
are required.

  $subref($arg);

Arrayrefs and hashrefs have special syntax (C<@{...}> and C<%{...}>)
for dereferencing them in contexts that normally wouldn't.
...
array with a single arrayref element. To copy the individual elements of
C<$arrayref> to C<@array> you need to dereference it first.
...
sigil imposes a scalar context, C<@> imposes list context, and C<%>
...
X<scalar context> 
Scalar context expects a single value. Any array or list evaluated in
scalar context returns an arrayref. This means that assigning an
array:
...
The comma is the list constructor, even in scalar context. Parentheses
only group. When a single element in parentheses is assigned in scalar
...
A hash-like list assigned to a scalar variable creates a reference to an
ordered array of pairs, following the rule that a list in scalar context
is an arrayref.
...
X<arrayrefs;anonymous constructor>
X<anonymous arrayref constructor> 
X<[] (brackets);anonymous arrayref constructor> 
The anonymous arrayref constructor C<[...]> imposes flattening list
context internally. They don't flatten when used in list context,
though, because flattening list context doesn't flatten references.
In scalar context, a simple list and an arrayref construct produce the
same result. But in list context, a simple list is treated as a
flattened list, while an arrayref construct is treated as a list of
one element, an arrayref:
...
reference to an array with three elements. This is useful for building
...
treated as a simple list, even if the scalar variables are arrayrefs.
...
The hash reference constructor isn't really C<{...}>, but 
...
ambiguity, you can force the right context by specifying C<hash> or
C<sub> before the block:
...
The C<constant> trait specifies that the value of the variable can't
be changed.
...
property means the Perl 6 C<system> call can be checked with a simple
...
benefits in optimization and interfacing between languages. The design
of the type system isn't complete, but the basic groundwork is in
place.
...
X<. (dot);... (infinite range);operator>
The C<...> operator is equivalent to C<..Inf>:
...
operators (C<E<lt>=E<gt>>, C<cmp>) return C<0> if the two arguments are
...
scalar context, so the value of C<$number> is simply copied. With the
...
If one side of a hyper operation is a simple scalar, it is
distributed across the list as if it were a list of identical
elements:
...
The unary C<\> operator returns a reference to its operand. The
referencing operator isn't needed very often, since scalar context
...
operator, @array_of_refs is assigned a list of three arrayrefs.

X<* (asterisk);* (flattening) operator>
X<arrays;flattening>
X<rvalues, using the unary * operator on> 
The unary C<*> operator (known as the splat operator) flattens a list
in a context where it would usually be taken as a reference. On an
rvalue, C<*> causes the array to be treated as a simple list:

  @combo = (\@array, \%hash);
  @a := @combo; # @a is @combo
  (@b, %c) := *@combo; # @b is @array, %c is %hash

Since the C<@combo> array contains an arrayref and a hashref, an
ordinary binding assignment of C<@combo> to C<@a> treats C<@combo> as
a single element and binds it to C<@a>. With the flattening operator,
...
The 'brvbar' entity is unicode character #xA6.

=end for

X<zip operator>
X<E<brvbar> (zip) operator>
The E<brvbar> operator takes two or more lists (arrays, hash keys, etc.)
...
iterate through the elements of several lists at the same time:
  ...
  @c = @a E<brvbar> @b; # @c is (1, 4, 2, 5, 3, 6)

There is no equivalent ASCII operator for the zip operator, but the
C<zip> function is much more fully featured than the operator. It is
described in A<CHP-4-SECT-3.2.3>"The for loop" later in this chapter.
