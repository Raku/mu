...
[[ (EM) added whitespace after "make_car", is that ok? ]]
...
[[ (EM): maybe mention that "Y" is the same as "¥"for the unicode challenged? ]]
...
[[ (EM) write out the same in P5 here maybe? ]]

=item Many (but systematically arranged) operators

If non-ASCII operators and parameters that don't need to be declared
haven't blown your mind yet, good. The last line can also be written
like this:

  @differences = @Xs »-« @Ys;

»« are "hyperoperators" that take a regular operator and "lift" it to
...
Like Perl 5, <...> does single-quoting. If you want interpolation,
...
set to a file called "0". You'd have to spell this the long way:

  my $output_file = defined lookup("output_file") ?
                            lookup("output_file") : $DEFAULT_FILENAME;

[[ (EM) // is part of bleadperl and HMBRANDT has been supplying patches for
all versions of perl since 5.6 to support // and //= ]]
...
[[ (EM): the above looks bad in my editor ]]
...
Thanks to Eric Hodges for material suggestions.
