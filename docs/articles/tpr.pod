...
Another developement that's taken palce is the normalization of bitwise
...
C<+>, C<~>, or C<?>; they mean numeric, string, or boolean context.
...
classes pick blessed hashes for their all-around usefulness, but the
object system itself is flexible and can use other things, such as
arrays or even globs. Certainly this has afforded authors with great
...
class-specific smarts (say, give a default color, or limit cars to a
certain number of doors).
...
When you install B<pugs>, you get plenty of (working) example code. Let's
look at a few of the things there.
...
This example -- one of pugs' oldest, added two months after the
project started -- is a very simple web client. The code is fairly
...
official perl6 release (though there very well maybe an C<IO::All>-ish
way to create lazyily defined IO objects as well). Pugs just went away
...
wasn't available when this code was written so P5 regexp were used. As you
can see, these are accessible through C<rx:perl5>, which is the C<rx> Rule
keyword with the Perl 5 I<adverbial modifier> attached to it.
...
this line becomes:
 ...
 ..and then port the regexp to a rule, which kinda shows XML is annoying
...
fit here, so I'm just going to hilight some nice features. The program
...
have their way, you can expect an analogous C<.yaml> method too (though
...
implementations, Pugs included. They are, like Pugs, work in progress.
