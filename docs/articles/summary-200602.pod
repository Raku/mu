...
* second half of February (9 July)
...
* 1-19 August (20 August)

* normal schedule (27 August)
...
=head3 Reason behind argument choice for {get,set}_<type>_keyed_intvtable

Leopold Toetsch wanted to know if there was a reason get/set keyed_intvtable
methods take INTVAL* instead of INTVAL.  It was suggested that some functions
need more than one keyed item, and the remaining follow the same pattern
for symmetry.
...
questions and resolved the ticket.  Others also made suggestions.
...
Tupshin Harper used the primes.pasm example and converted it to both C and
...
=head3 Configure.pl --cgoto=0 doesn't work
...
a distinct .string asm section.  Leopold Toetsch and Gregor replied.
...
=head3 Patch for jit.c to stop compiler warnings
...
=item * Initialization of various function pointers in packfile.c from NULL
to NULLfunc by Simon Glover

L<http://xrl.us/oxpn>

=item * life.pasm modification by Leon Brocard

L<http://xrl.us/oxpo>

=item * Art Haas posted a patch for examples/mops/mops.py, but there was
no confirmation that the patch was applied.

L<http://xrl.us/oxpp>

=item * formatting fix for docs/dev/rx.dev by Cal Henderson 
...
=head3 CODE {...} mentioning variables without interpolation
...
directory was clarified--they are considered to unofficial.
