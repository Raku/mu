...
the most immediate caller constructed with C<also>.

=head3 also

The C<also> method on an atomic Code object causes a checkpoint to be made
...
If you request an repeated alarm using the C<repeated> named argument, it will
