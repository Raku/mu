...
expliciteness and control capabilities.  This document attempts to describe
...
are actually copies of the original variable's value.
...
In that sense, one could consider the "ithreads" model as a diversion: the
Perl 6 concurrency picks up where the "5.005 threads" path left off.
...
A Code block can be marked as "is atomic".  The start of such a block
becomes a "checkpoint" to which execution can return (in exactly the same
state) if a problem occurs (a.k.a. a L<retry> is done) inside the scope of
the Code block.
