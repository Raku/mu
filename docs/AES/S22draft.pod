...
That will be covered in the advanced flow.
    ...
    4.  Install package from '.jib' file
            * Extract '.jib' to a temporary directoy
...
This directory should be extendable, so new files can be added to extend
functionality.
...
about that installation.
    ...
    * As the base directories bin/ and man/ path will not be in the 
        ...
        standard paths to the current active version of the package. I.e:
    ...
    * In the case of multiple installs of the same package (where 'same'
        ...
        policy will determine which is the current active pacakge, just
...
Prototyping code is in kane's p4 repo for now:
    http://p4.elixus.org:8181/@md=d&c=Ynk@//member/kane/pms/prototype2/

Read the README file to see how to test the prototypes
...
Note shortcomings are mostly mentioned in the code itself with 'XXX' markers.
    ...
    - Got a basic create script:
        ...
        - makes an archive (.jib file):
            ...
            - builds archive in a seperate builddir
    ...
    - Got a basic install script:
        ...
        - installs an archive (.jib file):
            ...
            - verifies (simplistically) if dependencies have been met
                - no Depends: a|b yet
                - no Depends: a >= version yet
            ...
            - links scripts from the bindir to the targetdir
                ...
                - does not do manpage generation or linking yet
                - does not record links yet, nor manages them actively
                    (a la update-alternatives)
                - no support for upgrades/downgrades of linked files
    ...
    - Got a basic uninstall script:
        ...
        - reads in .packlist equiv
            ...
            - unlinks all bin/$script files that were linked
                - not managed by something like update-alternatives yet
                - doesn't unlink manpages
...
=head2 Supported fields
    ...
    - Depends       Packages it depends on[1]   (p5-Foo)
...
=head2 Suggested fields[2]
    ...
    [2] Steal more tags from debian policy
