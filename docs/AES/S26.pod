...
markers, both of which are followed by the typename of the directive. The
C<=begin> marker may also be followed by a multi-word label for the
...
=head3 Modular Perldoc

You can incorporate sections of documentation drawn from other files
using the C<=use> directive. This directive takes one or more URLs or
shellish file globs as its block data, parses the resulting file(s) for
Perldoc, and then adds the resulting internal representation of the
file contents to the current internal Perldoc representation that's
being built.

C<=use> directives are handy for breaking out standard components of your
documentation set into reusable "modules":

     =head1 COPYRIGHT

     =use file:/shared/docs/std_copyright.pod

     =head1 DISCLAIMER

     =use file:/shared/docs/std_disclaimer.pod
...
recognize and for which they cannot find a suitable plug-in.
...
Kwid:
     ...
     =begin section_type                 .section_name
