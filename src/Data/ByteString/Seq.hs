...
-- Seq variant by Audrey Tang.
--

module Data.ByteString.Seq (
...
import qualified Data.Seq as S    -- S for seq
import qualified Data.List as L    -- L for list
...
eq (LPS as) (LPS bs) = eq' (S.viewl as) (S.viewl bs)
  where eq' S.EmptyL S.EmptyL = True
        eq' S.EmptyL _        = False
        eq' _  S.EmptyL       = False
        eq' (a S.:< as) (b S.:< bs) =
          case compare (P.length a) (P.length b) of
            LT -> a == (P.take (P.length a) b) && eq' (S.viewl as) (P.drop (P.length a) b S.:< bs)
            EQ -> a == b                       && eq' (S.viewl as) (S.viewl bs)
            GT -> (P.take (P.length b) a) == b && eq' (P.drop (P.length b) a S.:< as) (S.viewl bs)


compareBytes :: ByteString -> ByteString -> Ordering
compareBytes (LPS as) (LPS bs) = cmp (S.viewl as) (S.viewl bs)
  where cmp S.EmptyL S.EmptyL = EQ
        cmp S.EmptyL _        = LT
        cmp _  S.EmptyL       = GT
        cmp (a S.:< as) (b S.:< bs) =
          case compare (P.length a) (P.length b) of
            LT -> case compare a (P.take (P.length a) b) of 
                    EQ     -> cmp (S.viewl as) (P.drop (P.length a) b S.:< bs)
                    result -> result
            EQ -> case compare a b of
                    EQ     -> cmp (S.viewl as) (S.viewl bs)
                    result -> result
            GT -> case compare (P.take (P.length b) a) b of
                    EQ     -> cmp (P.drop (P.length b) a S.:< as) (S.viewl bs)
                    result -> result
...
packByte :: Word8 -> ByteString
packByte c = LPS (S.singleton (P.packByte c))
{-# NOINLINE packByte #-}
...
pack :: [Word8] -> ByteString
pack str = LPS $ S.fromList (fmap P.pack (chunk defaultChunkSize str))
...
unpack :: ByteString -> [Word8]
unpack (LPS ss) = L.concat (S.toList (fmap P.unpack ss))
{-# INLINE unpack #-}
...
null :: ByteString -> Bool
null (LPS ss) = S.null ss
{-# INLINE null #-}

-- | /O(n/c)/ 'length' returns the length of a ByteString as an 'Int'.
length :: ByteString -> Int
length (LPS ss) = S.foldl' (+) 0 (fmap P.length ss)
{-# INLINE length #-}

-- | /O(1)/ 'cons' is analogous to (:) for lists
cons :: Word8 -> ByteString -> ByteString
cons c (LPS ss) = LPS (P.packByte c S.<| ss)   -- TODO: coalesing and O(1) amortised time
{-# INLINE cons #-}
...
snoc (LPS ss) c = LPS (ss S.|> P.packByte c)
{-# INLINE snoc #-}

-- | /O(1)/ Extract the first element of a ByteString, which must be non-empty.
head :: ByteString -> Word8
head (LPS xs) = case S.viewl xs of
    S.EmptyL    -> errorEmptyList "head"
    x S.:< _    -> P.head x
{-# INLINE head #-}

-- | /O(1)/ Extract the elements after the head of a ByteString, which must be non-empty.
tail :: ByteString -> ByteString
tail (LPS xs) = case S.viewl xs of
    S.EmptyL    -> errorEmptyList "tail"
    x S.:< xs'
        | P.length x == 1 -> LPS xs'
        | otherwise       -> LPS (P.tail x S.<| xs')
{-# INLINE tail #-}

-- | /O(n\/c)/ Extract the last element of a ByteString, which must be finite and non-empty.
last :: ByteString -> Word8
last (LPS xs) = case S.viewr xs of
    S.EmptyR -> errorEmptyList "last"
    _ S.:> x -> P.last x
{-# INLINE last #-}

-- | /O(1)/ Return all the elements of a 'ByteString' except the last one.
init :: ByteString -> ByteString
init (LPS xs) = case S.viewr xs of
    S.EmptyR    -> errorEmptyList "init"
    xs' S.:> x
        | P.length x == 1 -> LPS xs'
        | otherwise       -> LPS (xs' S.|> P.init x)
{-# INLINE init #-}

-- | /O(n)/ Append two ByteStrings
append :: ByteString -> ByteString -> ByteString
append (LPS xs) (LPS ys) = LPS (xs S.>< ys)
{-# INLINE append #-}
...
-- element of @xs@.
map :: (Word8 -> Word8) -> ByteString -> ByteString
map f (LPS xs) = LPS (fmap (P.map f) xs)
{-# INLINE map #-}

-- | /O(n)/ 'reverse' @xs@ efficiently returns the elements of @xs@ in reverse order.
reverse :: ByteString -> ByteString
reverse (LPS xs) = LPS (fmap P.reverse xs)
...
any :: (Word8 -> Bool) -> ByteString -> Bool
any f (LPS xs) = S.foldr (||) False (fmap (P.any f) xs)
-- todo fuse

-- | /O(n)/ Applied to a predicate and a 'ByteString', 'all' determines
-- if all elements of the 'ByteString' satisfy the predicate.
all :: (Word8 -> Bool) -> ByteString -> Bool
all f (LPS xs) = S.foldr (&&) True (fmap (P.all f) xs)
-- todo fuse

-- | /O(n)/ 'maximum' returns the maximum value from a 'ByteString'
maximum :: ByteString -> Word8
-- maximum (LPS []) = errorEmptyList "maximum"
maximum (LPS xs) = S.foldl1 max (fmap P.maximum xs)
{-# INLINE maximum #-}

-- | /O(n)/ 'minimum' returns the minimum value from a 'ByteString'
minimum :: ByteString -> Word8
-- minimum (LPS []) = errorEmptyList "minimum"
minimum (LPS xs) = S.foldl1 min (fmap P.minimum xs)
{-# INLINE minimum #-}
...
replicate :: Int -> Word8 -> ByteString
replicate w c = LPS (S.singleton (P.replicate w c))
