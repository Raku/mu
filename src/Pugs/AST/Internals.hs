...
{-# LINE 1 "src/Pugs/AST/Internals.hs-drift" #-}
...
_FakeEnv = unsafePerformIO $ liftSTM $ do
    ...
    syms <- nullPad
    glob <- newTVar (combine (syms) $ nullPad)
...
instance YAML VRef where
    asYAML (MkRef (ICode cv)) = do
        vsub    <- liftIO $ runEvalIO _FakeEnv $ code_fetch cv
        vsubC   <- asYAML vsub
        return $ mkTagNode (tagHs "ICode") vsubC
    asYAML (MkRef (IScalar sv)) | scalar_iType sv == mkType "Scalar::Const" = do
        sv  <- scalar_fetch sv
        svC <- asYAML sv
        return $ mkTagNode (tagHs "IScalar") sv
