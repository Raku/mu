...
reduceStatements ((exp, pos):rest)
    ...
    | Sym scope name <- exp = const $ do
        ...
        let sym = MkSym name ref
            doRest = reduceStatements rest (Var name)
        case scope of
...
posSyms pos = [ MkSym n v | (n, v) <- syms ]
    where
    file = sourceName pos
...
reduce env exp@(Syn name exps) = case name of
    ...
    "," -> do
        ...
        ifListContext
            (retVal . VList . concat =<< mapM fromVal vals)
            (retVal $ VList vals)
    "val" -> do
        let [exp] = exps
