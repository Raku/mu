...
method is_type ($name) {
    for reverse @PADS {
#       return True if $_.{$name}; #ELFBUG True
        return    1 if $_.{$name}; #ELFFIX
    }
#   return False; #ELFBUG False
    return 0; #ELFFIX
}

method add_type ($name) {
#ELF# what is main::mangle?
    my $typename = main::mangle($name);
    my $qualname = ($+PKG // 'GLOBAL') ~ '::' ~ $typename;
#   @PADS[*-1]{$typename} = 'TYPE'; #ELFBUG *-1
#   @PADS[*-1]{$qualname} = 'TYPE'; #ELFBUG *-1
#   @PADS[*-1]{$name} = 'TYPE'; #ELFBUG *-1
    @PADS[ -1]{$typename} = 'TYPE'; #ELFFIX
    @PADS[ -1]{$qualname} = 'TYPE'; #ELFFIX
    @PADS[ -1]{$name} = 'TYPE'; #ELFFIX
}
...
method is_routine ($name) {
    ...
    for reverse @PADS {
#       return True if $_.{$aname}; #ELFBUG
#       return True if $_.{$name}; # type as routine? #ELFBUG
        return    1 if $_.{$aname}; #ELFFIX
        return    1 if $_.{$name}; # type as routine? #ELFFIX
    }
#   return False; #ELFBUG
    return 0; #ELFFIX
}
...
#   @PADS[*-1]{'&' ~ $name} = 'CODE'; #ELFBUG
    @PADS[ -1]{'&' ~ $name} = 'CODE'; #ELFFIX
}
...
role PrecOp {
    ...
    method coerce (Match $m) {
        # $m but= ::?CLASS;
#       my $var = self.WHAT ~ '::o'; #ELFBUG typenames are Str in p5 elf.
#       my $d = %::($var); #ELFBUG %::($var)
        my $type = self.Str; #ELFFIX
        my $d = symbol_lookup('%'~$type~'::o'); #ELFFIX
        $m<O> = {}; #ELFFIX avoid autovivification
        if not $d<transparent> {
#           for keys(%$d) { $m<O>{$_} = $d.{$_} }; #ELFBUG keys(%$d)
            for $d.keys { $m<O>{$_} = $d.{$_} }; #ELFFIX
#           $m.deb("coercing to " ~ self) if $*DEBUG +& DEBUG::EXPR; #ELFXXX what issue?
            $m.deb("coercing to " ~ self) if $*DEBUG ; #ELFFIX
        }
#       $m<O><kind> = self.WHAT; #ELFBUG
        $m<O><kind> = $type; #ELFFIX
        return $m;
    }
...
# Port of r25030 STD.pm's EXPR to elf.
method EXPR ($preclvl)
{
#ELF
#    temp $CTX = self.callm if $*DEBUG +& DEBUG::trace_call;
...
#   if $highvalid and %$*HIGHEXPECT { #ELFBUG %$*HIGHEXPECT
    if $highvalid and  $*HIGHEXPECT.elems { #ELFFIX
#       my @keys = sort keys %$*HIGHEXPECT; #ELFBUG %$*HIGHEXPECT
        my @keys = sort keys  $*HIGHEXPECT; #ELFFIX
        if @keys > 1 {
#           $m ~= "\n    expecting any of:\n\t" ~ join("\n\t", sort keys %$*HIGHEXPECT); #ELFBUG %$*HIGHEXPECT
            $m ~= "\n    expecting any of:\n\t" ~ join("\n\t", sort keys  $*HIGHEXPECT); #ELFFIX
        }
        else {
            $m ~= "\n    expecting @keys";
        }
    }
#   $m ~~ s|Syntax error|Syntax error (two terms in a row?)| if $m ~~ /infix|nofun/; #ELFXXX ?
...
1; #ELFDEV for independent compilation
