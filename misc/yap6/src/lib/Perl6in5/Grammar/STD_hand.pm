...
sub rule {
    my ($name,$code) = @_;
    my $stub = sub() {$rules{$name}};

    $N{$stub} = ucfirst($name);

    #print "creating stub $name\n";
    $Perl6in5::Grammar::STD_hand::{$name} = $stub;
...
sub finish_rule_creation {
    for (@order) {
...

        {
            ...
            $Perl6in5::Grammar::STD_hand::{$name} = $code;
        }
