...
--    compile lval@(Ann _ rest) = return . error $ ("WV Exp PIL_Stmts:"++(show lval))
    -- XXX: pragmas?
    ...
    compile lval@(Ann (Decl decl) (Var name)) = return . error $ ("WV Exp PIL_Stmts:"++(show lval)) -- WV: never happens?
    compile (Ann _ rest) = compile rest
{- WV: try to uncomment this -}
...
anonBS :: ByteString
anonBS = pack "<anon>"
isVCode (VCode codeval) = True 
isVCode _ = False    
--isMain val = (subPackage codeval == MkPkg [mainBS]) where VCode codeval = val
--isMain _        = False
isMain val 
    | (subPackage codeval == MkPkg [mainBS]) = True
    | otherwise = False
    where VCode codeval = val

--isAnon val = (subName codeval == anonBS) where VCode codeval = val
--isAnon _ = False
isAnon val 
    | (subName codeval == anonBS) = True
    | otherwise = False
    where VCode codeval = val

instance Compile Exp PIL_Stmt where
    compile (Ann (Decl decl) (Var name)) = return . PStmt . PExp $ _PDecl name
    compile lval@(Ann (Decl decl) rest) = return . error $ ("WV Exp PIL_Stmt Decl:"++(show lval))
    ...
    -- WV: Here we have already lost the Decl annotation
    compile (Val val) = do
        cxt     <- asks envContext
        if isVoidCxt cxt
            then case val of
                ...
                _               -> do
                    if ((isVCode val) && (isMain val) && (isAnon val))
                        then do
                                --warn "Main:" val
                                compile val
                        else do
                                ...
                                compile Noop
{-                    case val of
                       (VCode codeval) -> case subPackage codeval of -- we need subName = "<anon>" as well! make this an if
                            MkPkg [mainBS] -> warn "Main in void context" codeval
                            _   -> warn "Useless use of a code constant in void context" codeval
                       _ ->  warn "Useless use of a constant in void context" val
                    compile val -- WV: just trying. val was Noop-}
            else compile val
...
instance Compile Exp PIL_LValue where
    ...
    compile (Ann (Cxt cxt) rest) = enter cxt $ compile rest
    compile (Ann (Decl decl) (Var name)) =  return $ _PDecl name
    compile lval@(Ann (Decl _) rest) = return . error $ "WV Exp PIL_LValue Decl:"++(show lval)
    compile (Ann _ rest) = compile rest
    -- XXX: pragmas?
    -- WV: evil HACK
--    compile (Var name) 
--        | (v_sigil name) == SCode = return . error $ "1:"++(show (v_name name))
--        | otherwise = return $ _PVar name
    compile (Var name) = return $ _PVar name -- WV: Problem is, sometimes this is a function, so it should be _PFun
...
--        funC    <- compile fun
        funC  <- case fun of
             (Var fname)    -> return . PExp $ _PFun fname
             _              -> compile fun      
...
_PDecl :: Var -> PIL_LValue
_PDecl = PDecl . cast

_PFun :: Var -> PIL_LValue
_PFun = PFun . cast

{-| Compiles various 'Exp's to 'PIL_Expr's. -}
instance Compile Exp PIL_Expr where
--    compile lval@(Ann _ rest) = return . error $ ("WV Exp PIL_Expr:"++(show lval))
    compile (Ann Pos{} rest) = compile rest -- fmap (PPos pos rest) $ compile rest
    compile (Ann Prag{} rest) = compile rest
    compile (Ann (Cxt cxt) rest) = enter cxt $ compile rest
    compile (Ann (Decl decl) (Var name)) = return . PExp $ _PDecl name
    compile lval@(Ann (Decl decl) rest) = return . error $ ("WV Exp PIL_Expr:"++(show lval))    
    compile (Ann _ rest) = compile rest
    -- XXX: pragmas?
--    compile (Var name) = return . PExp $ _PVar name
        -- WV: evil HACK -- but it works
    compile (Var name) 
        | (v_sigil name) == SCode = return . PExp $ _PFun name
        | otherwise = return . PExp $ _PVar name
