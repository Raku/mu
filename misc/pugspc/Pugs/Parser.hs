...
WV: ruleExpression returns the Exp _without_ Ann (Decl !!!
-}
...
-- Expressions ------------------------------------------------
{-WV: parseExpWithOps uses parseTerm and thus ruleVarDecl
Here we already lost Decl
-}
ruleExpression :: RuleParser Exp
ruleExpression = do
                    expr <- (<?> "expression") $ parseExpWithOps
                    return expr
--                    return .error $ show expr
...
{-
WV: This is the critical part: deals with variable declarations
Used _only_ in parseTerm.
I verified that indeed ruleVarDecl _is_ called _and_ returns an annotated lexical!

my $x=42; =>
Ann (Decl SMy) (Syn "=" [Var "$x",Ann (Pos (MkPos "test_min_lex.p6" 3 3 3 9)) (Val (VInt 42))])
my $z; =>
Ann (Decl SMy) (Var "$z")
...
--    case (head initializers) of
--        (Syn _ xs) -> return . error $ show (map (\x->(Ann (Decl scope) x)) xs)
--        _  -> return . error $ show $ initializers --Ann (Decl scope) $ case filter (/= Noop) initializers of -- WV: for testing
--    return . Ann (Decl scope) $ case filter (/= Noop) initializers of
--        []          -> Noop
--        [x] | isOne -> x
--        xs          -> Syn "," xs
--    let
--        filtered_initializers = case filter (/= Noop) initializers of
--            []          -> Noop
--            [x] | isOne -> x
--            xs          -> Syn "," xs    
    return  $ case filter (/= Noop) initializers of
        []          -> Noop
        [x] | isOne -> case x of 
                            (Syn s xs) -> Syn s (map (maybeDecl scope) xs)
                            _ -> maybeDecl scope x
        xs          -> Syn "," (map (maybeDecl scope) xs)
    where
    oneDecl = do
    ...
    maybeDecl scope xx@(Var _)  = Ann (Decl scope) xx
    maybeDecl scope x = x
{-
WV: 
parseTerm indeed returns the annotated variable:  Ann (Decl SMy) (Var "$v")

So we have to look up the callchain to see where we lose the annotation
-}
...
--      , ruleTypeLiteral
    ...
    if (col == sourceColumn pos)
        then do
            ...
            fs <- many s_postTerm
            return (combine (reverse fs) term)
--            return . error $ show (combine (reverse fs) term)
