...
data PIL_LValue = PVar { pVarName :: !VarName }
    ...
    | PDecl -- WV: for declaration of lexicals. Not quite right, only OK for SMy
        { pVarName :: !VarName
        }
    | PFun -- Currently, named functions are looked up with find_name but lex vars with find_lex    
        { pVarName :: !VarName
        }    
    deriving (Show, Eq, Ord, Typeable)
