...
anyFromVal v = case fromTypeable (fromVal v) of
...
--  , subLexical        :: !Pad                   -- ^ Cached merged pads -- WV: check
...
data Exp
    ...
    | Pad !Scope !Pad !Exp              -- ^ Lexical pad -- WV: why commented?
...
transformExp f (Pad s p e) = f =<< liftM (Pad s p) (transformExp f e)  -- WV: check
...
instance Unwrap Exp where
    unwrap (Ann (Decl x) exp)      = (Ann (Decl x) (unwrap exp)) -- WV: We need the Ann for PIR
...
extractPlaceholderVars (Ann ann ex) vs = ((Ann ann ex'), vs')
    where
    (ex', vs') = extractPlaceholderVars ex vs
extractPlaceholderVars (Pad scope pad ex) vs = ((Pad scope pad ex'), vs')  -- WV: check
     where
     (ex', vs') = extractPlaceholderVars ex vs
