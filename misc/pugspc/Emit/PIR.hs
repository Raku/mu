...
data Ins
    ...
    {- This will be a problem: 
        given 
            my $v=42;
        the 42 will be assigned to a literal:
            .local pmc pv42_lit
            pv42_lit = new .Undef
            pv42_lit = assign 42
        we need to replace the current
            pv = assign pv42_lit
        with
            store_lex "$v", pv42_lit
        which means that the translation from PIL1 should keep the $v
            pLV = PAssign {
                        pLHS = [PDecl {pVarName = "$x"}],
                        pRHS = PLit {pLit = PVal {pVal = VInt 42}}
                        }
        So maybe it means we should actually have a PAssignLex in PIL1
        Makes me wonder if Decl should not be Lex, or alternatively,
        if I should not make a distinction somehow between our, my, local etc
        (see Pugs/AST/Scope)
        I think I should do that: keep the type of Scope in the expression                
        The problem is even more complicated because the assignment can of course
        happen much later.
        So really we must do it differently
    -}
    | InsAssignLex  !LValue !Expression             -- WV: to assign lexicals -- not used
    ...
    | InsDecl       !Expression !LValue                -- WV: For instantiation of lexicals
    deriving (Show, Eq, Typeable)
...
data ObjType
    = PerlScalar | PerlArray | PerlHash | ResizablePMCArray
...
instance Emit Decl where
    ...
    emit (DeclSub name styps stmts)
        =  (emit ".sub" <+> doubleQuotes (emit $ quoted name) <+> spaceSep styps)
...
instance Emit Ins where
    ...
    emit (InsDecl litv name) = emit ".lex" <+> emit litv <+> emit ", " <+> emit name -- WV
...
--      emit (InsPrim Nothing "store_global" args)
    emit (InsPrim Nothing name args)
        | name == ".return" = emit name <+> (parens $ commaSep args) -- WV: to get .return ($P8)
        | otherwise = emit name <+> commaSep args
    emit (InsFun rets (ExpLit (LitStr name)) args) = emitFunName "invokecc" name args rets
    emit (InsFun rets fun args) = emitFun "invokecc" fun args rets -- WV: this code is called in trans PCode
...
instance Emit ObjType where
    emit PerlScalar = emit ".Undef"  -- XXX special case
    emit PerlPair   = emit ".Pair"  -- XXX special case
    emit PerlRef    = emit ".Ref"   -- XXX special case
    emit PerlArray = emit ".Array"
    emit PerlInt = emit ".Integer"
    emit PerlHash = emit ".Hash"
    emit PerlEnv = emit ".Env"
...
sub name sigs body = DeclSub name [] stmts
    where
    param = "get_params" .- sigList sigs -- WV: ".param" .- (map sigIdent sigs) -- WV 
...
callThunkCC2 :: Expression -> [Ins]
callThunkCC2 fun =
    [ "set_args" .- sigList []
    , "get_results" .- sigList [tempPMC2]
    , "invokecc" .- [fun]
    , "get_results" .- sigList [tempPMC]
    , "invokecc" .- [tempPMC2]    
    ] 
...
stmtControlCond name comp =
    ...
    body = concat
        [ [ comp .- [arg0, bare altL] ]
        , callThunkCC2 arg1 -- WV: but we need to callThunkCC on the return value as well!
        , [ "goto" .- [bare postL] ]
        , [ InsLabel altL ]
        , callThunkCC2 arg2
        , [ InsLabel postL ]
...
--WV        , tempPMC2 <-- "compile" $ [tempPMC, tempSTR]
        ] ~-> [tempPMC2]
    , sub "&eval_pir" [arg0]
