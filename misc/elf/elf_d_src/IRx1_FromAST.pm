...
$^blackboard::expect_term_base;
    ...
    IR0::Call.newp($m,$^blackboard::expect_term_base,irbuild_ir($m.{'hash'}{'ident'}),IR0::Capture.newp($m,irbuild_ir($m.{'hash'}{'semilist'})));
    });
...
IR0::Call.newp($m,$^blackboard::expect_term_base,$ident,IR0::Capture.newp($m,$args));
    });
...
IR0::Apply.newp($m,'self',IR0::Capture.newp($m,[]))
} else {
die "AST term partially unimplemented.\n";
};
    });
    $main::irbuilder.add_constructor('integer', sub ($m) {
    IR0::NumInt.newp($m,($m.match_string));
    });
    $main::irbuilder.add_constructor('subcall', sub ($m) {
    my $t = irbuild_ir($m.{'hash'}{'subshortname'}.{'hash'}{'twigil'});
if $t && $t eq '.' {
IR0::Call.newp($m,IR0::Apply.newp($m,'self',IR0::Capture.newp($m,[])),irbuild_ir($m.{'hash'}{'subshortname'}.{'hash'}{'desigilname'}.{'hash'}{'ident'}),IR0::Capture.newp($m,irbuild_ir($m.{'hash'}{'semilist'})))
} else {
IR0::Apply.newp($m,irbuild_ir($m.{'hash'}{'subshortname'}),IR0::Capture.newp($m,irbuild_ir($m.{'hash'}{'semilist'})))
};
    });
    ...
    IR0::Use.newp($m,'use',irbuild_ir($m.{'hash'}{'module_name'}));
    });
...
IR0::Apply.newp($m,irbuild_ir($m.{'hash'}{'ident'}),IR0::Capture.newp($m,[$not_really_an_arglist]))
} else {
IR0::Apply.newp($m,irbuild_ir($m.{'hash'}{'ident'}),IR0::Capture.newp($m,[]))
};
    });
    $main::irbuilder.add_constructor('quote:q', sub ($m) {
    IR0::Buf.newp($m,irbuild_ir($m.{'hash'}{'text'}));
    });
...
if $op eq 'str' { $op = '=' };
IR0::Apply.newp($m,"infix:"~$op,IR0::Capture.newp($m,[irbuild_ir($m.{'hash'}{'left'}),irbuild_ir($m.{'hash'}{'right'})]));
    });
    $main::irbuilder.add_constructor('scope_declarator:my', sub ($m) {
    my $vd = irbuild_ir($m.{'hash'}{'scoped'});
IR0::VarDecl.newp($m,'my',undef,undef,$vd.[0],undef,undef,'=',$vd.[1]);
    });
    $main::irbuilder.add_constructor('scope_declarator:has', sub ($m) {
    my $vd = irbuild_ir($m.{'hash'}{'scoped'});
IR0::VarDecl.newp($m,'has',undef,undef,$vd.[0],undef,undef,'=',$vd.[1]);
    });
    $main::irbuilder.add_constructor('scope_declarator:our', sub ($m) {
    my $vd = irbuild_ir($m.{'hash'}{'scoped'});
IR0::VarDecl.newp($m,'our',undef,undef,$vd.[0],undef,undef,'=',$vd.[1]);
    });
...
my $args = irbuild_ir($m.{'hash'}{'kludge_name'});
if $args && ($args.ref eq 'SCALAR')  { $args = [$args] }
IR0::Apply.newp($m,"circumfix:"~$name,IR0::Capture.newp($m,$args));
    });
    ...
    IR0::Loop.newp($m,irbuild_ir($m.{'hash'}{'expr'}),irbuild_ir($m.{'hash'}{'block'}));
    });
    $main::irbuilder.add_constructor('statement_control:if', sub ($m) {
    my $els = irbuild_ir($m.{'hash'}{'else'});
if $els { $els = $els[0] }
IR0::Cond.newp($m,[[irbuild_ir($m.{'hash'}{'if_expr'}),irbuild_ir($m.{'hash'}{'if_block'})]].push(irbuild_ir($m.{'hash'}{'elsif'}).flatten),$els);
    });
...
my $sig = IR0::Signature.newp($m,[],undef);
if irbuild_ir($m.{'hash'}{'multisig'}) { $sig = irbuild_ir($m.{'hash'}{'multisig'}).[0] };
IR0::SubDecl.newp($m,undef,undef,undef,$ident,$sig,undef,irbuild_ir($m.{'hash'}{'block'}));
    });
    $main::irbuilder.add_constructor('routine_declarator:method_def', sub ($m) {
    IR0::MethodDecl.newp($m,undef,undef,undef,irbuild_ir($m.{'hash'}{'ident'}),irbuild_ir($m.{'hash'}{'multisig'}).[0],undef,irbuild_ir($m.{'hash'}{'block'}));
    });
    $main::irbuilder.add_constructor('signature', sub ($m) {
    IR0::Signature.newp($m,irbuild_ir($m.{'hash'}{'parsep'}),undef);
    });
    $main::irbuilder.add_constructor('parameter', sub ($m) {
    IR0::Parameter.newp($m,undef,undef,undef,irbuild_ir($m.{'hash'}{'param_var'}));
    });
    $main::irbuilder.add_constructor('param_var', sub ($m) {
    IR0::ParamVar.newp($m,irbuild_ir($m.{'hash'}{'sigil'}),irbuild_ir($m.{'hash'}{'twigil'}),irbuild_ir($m.{'hash'}{'ident'}));
    });
...
irbuild_ir($m.{'hash'}{'package_def'});
    ...
    IR0::PackageDecl.newp($m,undef,undef,$^blackboard::package_declarator,irbuild_ir($m.{'hash'}{'module_name'}).[0],irbuild_ir($m.{'hash'}{'traits'}),irbuild_ir($m.{'hash'}{'block'}));
    });
...
IR0::Hash.newp($m,irbuild_ir($m.{'hash'}{'block'}.{'hash'}{'statementlist'}))
} else {
