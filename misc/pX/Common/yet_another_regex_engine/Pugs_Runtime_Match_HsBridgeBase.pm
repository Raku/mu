package Pugs::Runtime::Match::HsBridgeBase;
...
sub __RUN__ {
    ...
    while (my ($name, $body) = each %subrules) {
        ...
        $self->reinstall( $name => $body, \%sub_opts );
    }

    my %opts = (grammar => __PACKAGE__);
    ($1 and $opts{$1} = 1) while $rule_text =~ s/^:(\w*)\(1?\)\[(.*)\]\z/$2/s;

    #XXX This belongs elsewhere as it's part of the regex implementation.
...
#    }
    ...
    my $rule    = $self->compile( $rule_text, \%opts );
    my $match   = $rule->match( $match_text );

    return $self->match_dump_hs($match);
}
sub match_dump_hs {
    my($self,$match)=@_;
    $match->dump_hs();
