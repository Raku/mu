...
multi sub *infix:<∋> ($one, $member) returns Bool {
    $one.includes($member);
}
multi sub *infix:<∈> ($member, $set) returns Bool {
    $set.includes($member);
}
multi sub *infix:<∍> ($one, $member) returns Bool {
    $one.includes($member);
...
# necessary.
multi sub *infix:<~~> ($member, $one) returns Bool {
    $one.includes($member);
}
