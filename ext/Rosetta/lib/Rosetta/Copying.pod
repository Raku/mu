...
is the most common case.  Engines are distributed separately.  Each
distribution may have additional files that are tied to the named file,
because they serve as documentation or examples or tests or localization
data for it, and the whole collection in the distribution is considered a
single unit as far as ownership and licensing goes, unless explicitly
...
readily substituted for by third party components, are these Perl files:
    ...
    * /lib/Rosetta/Utility/SQLParser.pm
...
available as a separate distribution on CPAN.  The 'Utility' files only
depend on Model.pm, while the 'Engine' and 'Emulator' files depend on both
the 'Utility' files and Rosetta.pm.
...
an exception granted to linked but independent components that are fully
useable when not linked to said distribution.
