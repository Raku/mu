...
an operator / function or a procedure; the former has a special return
value which is the value of the evaluated operator invocation within a
value expression; the latter has no such special return value.
...
corresponds to SQL's IN or OUT+INOUT parameter types).  When invoking a
routine, an argument corresponding to a read-only parameter can be an
arbitrarily complex value expression, but an argument corresponding to a
read-write parameter must be a valid target variable, and that target
variable may be updated during the procedure invocation.  For a procedure,
the only way to pass output directly to its invoker (meaning, without
updating global variables) is to assign that output to read-write
parameters.  An operator / function may do that too, but it additionally
must provide its namesake special return value using the standard "return"
keyword.  Note that "return" can be used in a procedure too for flow
control, but it doesn't pass a value as well.
...
returns a new Rosetta::Interface::Routine object.  You then associate a
...
the Routine object's "execute" method.  Any Variable/Value objects
...
parameters.  When the Routine is a function, "execute" will generate and
...
including: value assignment of a value expression's result to a named
target ("assign"), conditionals ("if"), loops ("for", "while"), procedure
...
operators / functions.  Note that a thrown exception which falls out of an
anonymous procedure will result in an exception thrown out to the
application (in this distribution, it will be as a thrown new
Rosetta::Interface::Exception object).  For our purposes, transaction
control statements ("start", "commit", "rollback") are also grouped with
these standard keywords.

The "assign" keyword is the most frequently used one in Rosetta D, except
sometimes for "return", and it updates a target variable to a new value
provided by a Rosetta D expression.  If the target variable is an anonymous
...
parameters, but only procedures (that are not invoked by operators /
functions) are allowed to target global variables, which are declared in a
repository directly.  In other words, an operator / function may not have
side-effects (save its own read-write parameters), though it can I<read>
from global variables.  Moreover, any procedure that is invoked by an
operator / function is subject to the same restriction, since it is
effectively part of the operator / function.  A few special exceptions may
...
more than once in the same Rosetta D statement; target meaning either an
assignment target or as an argument for a read-write parameter.
...
may not have side-effects (other than their read-write parameters), so that
avoids complicating the issue due to environment updates occuring during
their invoker statement's first phase.

Regarding invoked operators / functions with read-write parameters, any
changes made by said routines to those parameters are not propagated to
their corresponding arguments except simultaneously with the main
statement's second phase, if the main statement is an "assign", essentially
making the main statement a multi-assignment; if the main statement is a
"call", then the propagation is done just before the main statement is
invoked.  I<TODO: This proposal may not work as is.>
