...
Note that the virtual machine itself does not have its own set of named
users where one must authenticate to use it.  Rather, any concept of such
users is associated with individual persistent repositories, such that you
may have to authenticate in order to mount them within the virtual machine;
moreover, there may be user-specific privilege constraints for that
...
types of Rosetta D commands, which are summarized next.

One type of command, that can be done at any time, is a bare value expression;
it can involve any system-defined or user-defined operators or variables or
literal values as input, and the resolved value is returned
