...
Rosetta D commands are structured as arbitarily complex routines, either
named or anonymous, and they can have (named) parameters, can contain
single (usually) or multiple Rosetta D statements or value expressions, and
can return one or more values.

Rosetta D command routine definitions can either be stored in a persistent
repository for reuse like a repository's data types or variables, or they
can be furnished by an application at run-time for temporary use.  A named
command routine can take the form of either an operator / function or a
procedure; the former has a special return value which is the value of the
evaluated operator invocation within a value expression; the latter has no
such special return value.  An anonymous command routine can only take the
form of a procedure.

An application can only ever directly define and invoke an anonymous
command procedure, but that anonymous procedure can in turn define and/or
invoke any named command routines within the DBMS environment.
...
function, and the parameters of a Rosetta D anonymous procedure correspond
to SQL host parameters or bind variables.

A Rosetta D routine parameter can be read-only or read-write (which
corresponds to SQL's IN or OUT+INOUT parameter types).  But moreover,
updating a read-write parameter is the only way that Rosetta D provides to
return output from a routine, and the way you return multiple values is to
update multiple read-write parameters; a routine without any read-write
parameters does not return anything.  Now, an operator / function has a
special read-write parameter whose name is the same as the operator /
function itself; this special parameter corresponds to the special return
value of the operator / function (you can not match an argument to it), so
it needs to be set before the routine returns; a procedure has no such
thing.  Note that Rosetta D does provide the 'return' statement for help in
control flow within a routine, but it does not take any arguments.
...
as well as to return results that probably differ with each execution.
Presumably, any values that will be constant through the life of a command
routine will be coded as literal values in its definition rather than
parameters.

(In this distribution, you furnish an anonymous command procedure
definition for reuse using a DBMS object's "prepare" or "compile" method;
that method returns a new Rosetta::Interface::Command object.  You then
associate a Rosetta::Interface::Variable (or similar) object with each of
the routine's parameters using the Command object's "bind_param" method,
and then invoke the Command object's "execute" method.  Any Variable
objects corresponding to input parameters need to be set by the application
prior to "execute", and following the "execute", the application can read
the routine's output from the Variable objects associated with output
parameters.)

The Rosetta D language has all the standard keywords for control flow, any
of which a Rosetta D routine (both anonymous and named) can contain,
including: conditionals ("if"), loops ("for", "while"), procedure
invocation ("call"), normal routine exit ("return"), plus exception
creation and resolution ("throw", "try", "catch").  Note that a thrown
exception which falls out of an anonymous procedure will result in an
exception thrown out to the application (in this distribution, it will be
as a thrown new Rosetta::Interface::Error object).  For our purposes,
transaction control statements ("start", "commit", "rollback") are also
grouped with these standard keywords.

All Rosetta D statements, aside from the flow control keywords, are
assignments which update a target variable to a new value provided by a
Rosetta D expression.  If the target variable is an anonymous procedure's
read-write parameter, the statement corresponds to an unencapsulated SQL
"select" query.  If the target variable is an ordinary variable, and
particularly if it is a repository's component data variable, the
...
which target their own lexical variables, including their own read-write
parameters, but only procedures are allowed to target global variables,
which are declared in a repository directly.  In other words, an operator /
function may not have side-effects (save its own read-write parameters),
though it can I<read> from global variables.  A few special exceptions may
be made to this restriction on operators / functions, but for the most
part, the restriction is in place to prevent inconsistencies between reads
of the environment/globals from multiple operators / functions that are
invoked in the same Rosetta D expression; all reads in the same expression
need to see the same state.
...
to any target variables being updated; moreover, all operators / functions
may not have side-effects, so that avoids complicating the issue due to
...
TODO: Rewrite what follows.

How transactions are supposed to work is as follows.

First, every Rosetta D statement (including multi-update statements) which
is or is part of a Rosetta D command is itself a complete implicit
auto-committing transaction; it sees a static view of the environment, and
if it is an update, either all parts of that update will succeed and
commit, or none of it will and no changes are left.

Second, explicit transactions can be respectively started and committed or
rolled back with appropriate Rosetta D statements.

Third, transactions can be nested, by starting a new one before concluding
a previous one, and the parent-most transaction has the final say on
whether all of its committed children actually have a final committed
effect or not.

Fourth, all currently mounted repositories (persistent and temporary both)
are joined at the hip with respect to transactions; a commit or rollback is
...
Fifth, certain Rosetta D commands can not be executed within the context of
an explicit transaction, the main examples being those that mount or
unmount a persistent repository; this is because such a change in the
environment mid-transaction would result in an inconsistent state.




=head1 UM, STUFF FOLLOWS, AND THINGS
