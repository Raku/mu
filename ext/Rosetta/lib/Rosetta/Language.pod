...
=head2 Transactional Behaviour

The Rosetta DBMS is fully transactional, and attempts to be fully ACID
compliant, although the actual reliability will vary with the
implementation; that aside, how transactions are supposed to work is as
follows.

First, every Rosetta D statement or multi-statement which is or is part of
a Rosetta D command is itself a complete implicit auto-committing
transaction; it sees a static view of the environment, and if it is an
update, either all parts of that update will succeed and commit, or none of
it will and no changes are left.
