...
as the ANSI/ISO SQL:2003 standard, it may be tempting to just assume
everything you know from them is also true here.  While that will work in
some cases (and in fact I intentionally don't repeat many details here for
brevity), it won't in others, so you should always check here for any
possible conflicts.
