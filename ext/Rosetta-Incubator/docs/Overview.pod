...
product's native way of doing things, those who I<want> to talk to their
...
A B<rosetta_interface> is a Rosetta::Interface tree root object; you usually
...
A B<sql_server> is the database management system software itself and
...
is exactly one "sql_server", since the Engine I<is> it.  In the case of any
...
opens a connection to a database, a "depot" I<is> what it is connecting to,
...
multiplexer, I<all> of the visible catalogs would be the REMOTE type, and
