...
Add new documentation file /docs/Language (or equivalent) that defines the
valid structures and contents of the SQL-equivalent AST that Rosetta takes
as input and that SQL::Routine stores.  This document is to SQL::Routine
and Rosetta what the published SQL:2003 standard documents are to SQL and
conformant databases (or they are what the Perl 6 Synopsis are to the Perl
6 language and its implementations).

=item *

Add new documentation file /docs/Architecture (or equivalent) that defines
what the Rosetta::Interface::* classes/roles conceptually represent, how
they relate and work together.  Alternately, this information may be added
to /docs/Overview instead.
...
Rosetta::Engine::Native, these will be serialized-isolation mode only.
...
which Native implements is working correctly.  Or it may not even do that
...
Upload first post-rewrite versions of SQL::Routine-0.710.0, Rosetta-0.490.0
(with Rosetta::Validator-0.490.0), and Rosetta::Engine::Native-0.1.0 to
CPAN, and make a list announcement for "Rosetta/SQL::Routine Developer
Release #4".
...
Update Rosetta::Engine::Native (REN) so databases can persist on disk.  Its
...
Consider supporting object-relational database extensions, such as those
that violate first normal forms by storing compound user-defined data types
in record fields or 'references' or arrays or 'set' data types.

By contrast, what Rosetta otherwise natively supports is proper relational
databases where every intersection of a row and a column in a table (a
table field) can hold just one scalar value, such as a number or character
string, or a LOB, or null.

These extensions would be something an order of magnitude more difficult to
make portable than most other issues.
