...
The SQL::Routine language is primarily declarative, in that it defines how
implementations should appear and behave from a user's point of view, and
it does not typically make any demands as to how implementations accomplish
this behind the scenes.

=head1 OPERATIONAL CONTEXT

The SQL::Routine language is designed for a specific virtual environment,
...
the SQL::Routine language provides the means to create, read, modify, and
delete both entire depots and other items that live in them.  An
implementation (or instance thereof) of the SQL::Routine language is a
...
(SQLite natively does not, Oracle natively does, etc).  SQL::Routine
...
As with typical programming languages, the SQL::Routine language also
...
SQL::Routine defines 4 types of catalogs, called LOCAL, SYSTEM, TEMPORARY,
and REMOTE; a depot can potentially have all of these kinds at once, but
...
schema", but that all the meta-data is atomic like the SQL::Routine
language itself.
...
The SQL::Routine language is natively built around strong data types and
...
The SQL::Routine language gains rigor from this requirement for strong
...
The SQL::Routine language gains simplicity from this same requirement,
...
The SQL::Routine language has 2 main classes of data types, which are
...
infinite, the SQL::Routine language requires you to explicitly declare a
...
the SQL::Routine language provides to simulate both weak data types and
