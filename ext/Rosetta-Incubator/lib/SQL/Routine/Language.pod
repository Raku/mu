...
other SQL::Routine native primitive types can have.  But some built-in
...
All not-nullable built-in data types default to their concept of empty or
nothingness, such as zero or the empty string.  All nullable built-in
types, and all not-nullable built-in types that you customize with a true
...
This not-nullable data type is a logical boolean, whose only possible
values are FALSE and TRUE.  Its concept of nothingness is FALSE.

=item C<INTEGER>

This not-nullable data type is a rational integer of unlimited (but not
infinite) precision.  Its concept of nothingness is zero.  It is
conceptually a full sub-domain of RATIONAL, such that all INTEGER values
have one-on-one RATIONAL equivalents, but for widely pervasive matters of
efficiency, SQL::Routine will treat the two types as exclusive, so you need
to do explicit conversions between them.

=item C<RATIONAL>

This not-nullable data type is a rational number of unlimited (but not
infinite) precision and scale.  Its concept of nothingness is zero.

=item C<CHAR_STR>

This not-nullable data type is a character string of unlimited (but not
infinite) length.  Its abstract character repertoire is that of the current
newest Unicode Standard.  Its concept of nothingness is the empty string.

=item C<BIT_STR>

This not-nullable data type is a bit string of unlimited (but not infinite)
length.  Its concept of nothingness is the empty string.
...
default scalar variables.  This not-nullable data type is a union of the
domains of the BOOLEAN, INTEGER, RATIONAL, CHAR_STR, and BIT_STR
data types.  Its concept of nothingness is the empty string.
