...
defined.  However, its predefined set of data types also includes
ones that have the range of typical weak types such as some database
products and languages like Perl use.

All data types are based on the concept of domains; any variable or literal
that is of a particular data type may only hold a value that is part of the
domain that defines the data type.  SQL::Routine has some native data types
that it implicitly understands (eg, booleans, integers, rational numbers,
character strings, bit strings), and you can define custom ones too that
are based on these (eg, counting numbers, character strings that are
limited to 10 characters in length).
...
directly to a built-in data type, or to one you customized; this is so
...
considered inequal.  Following this, it is mandatory that every built-in
...
though the built-in ones will include others.  However, it is strongly
...
This special data type represents situations where a value of an arbitrary
data type is desired but none is yet known; it sits in place of the absent
value to indicate that fact.  NULL artifacts will always explicitly compare
as being unequal to each other; since they all represent unknowns, we can
not logically say any are equal, so they are all treated as distinct.  This
...
This data type is a logical boolean, whose only possible values are FALSE
and TRUE.  This data type is not nullable and defaults to FALSE.

=item C<INTEGER>

This data type is a rational integer of unlimited (but not infinite)
precision.  It is a sub-domain of RATIONAL and will natively cast to (and
sometimes from) that.  This data type is not nullable and defaults to zero.

=item C<RATIONAL>

This data type is a rational number of unlimited (but not infinite)
precision and scale.  This data type is not nullable and defaults to zero.

=item C<CHAR_STR>

This data type is a character string of unlimited (but not infinite)
length.  Its abstract character repertoire is that of the current newest
Unicode Standard.  This data type is not nullable and defaults to the empty
string.

=item C<BIT_STR>

This data type is a bit string of unlimited (but not infinite)
length.  This data type is not nullable and defaults to the empty string.

=item C<SCALAR>

This data type is a union of the domains of the BOOLEAN, RATIONAL (includes
INTEGER), CHAR_STR, and BIT_STR data types.  It is a weakly typed scalar
like Perl's default variables, or SQLite's default table columns.  This
data type is not nullable and defaults to the empty string.

=item C<NULL_BOOLEAN>

This data type is a union of the domains of the NULL and BOOLEAN data
types; it is a nullable logical boolean and defaults to NULL.

=item C<NULL_INTEGER>

This data type is a union of the domains of the NULL and INTEGER data
types; it is a nullable rational integer and defaults to NULL.

=item C<NULL_RATIONAL>

This data type is a union of the domains of the NULL and RATIONAL data
types; it is a nullable rational number and defaults to NULL.

=item C<NULL_CHAR_STR>

This data type is a union of the domains of the NULL and CHAR_STR data
types; it is a nullable character string and defaults to NULL.

=item C<NULL_BIT_STR>

This data type is a union of the domains of the NULL and BIT_STR data
types; it is a nullable bit string and defaults to NULL.

=item C<NULL_SCALAR>

This data type is a union of the domains of the NULL and SCALAR data
types; it is a nullable weakly typed scalar and defaults to NULL.
