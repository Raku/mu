...
by-the-book using the grammar published by Darwin and Date, but it could
...
stores or returns duplicate rows, always uses 2-valued logic (not 3-valued
logic), and has case-sensitive table/column/etc names and character
strings.  An application written to use this SQL dialect rather than some
other dialect should behave more deterministically, and perform faster,
than ones written to some other SQL dialect, since there is less
