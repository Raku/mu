...
member tuples have identical cardinality and name-sets, but that a relation
data type knows what its allowed names are even if it has no tuples.  Like
with I<tuple>, the term I<relation> legitimately refers to a set or
"ordinal tuple" in other contexts, but it does not in this documentation.
Terms like I<record set> or I<row set> or I<table> are not used in this
...
more) named object attributes (of any type, scalar or nonscalar) that the
...
comprises a list of zero or more named I<attributes> of any type, scalar or
nonscalar.  This list definition is called the tuple's I<heading>.
...
scalar or nonscalar types which have at least one attribute which is itself
infinite (for example, a type that has an I<Int> attribute).
...
as are the C<Scalar> and C<Universal> types.
...
types of scalar possrep or tuple or relation attributes.  The declared type
of each of the latter must be either a scalar type, or a specific tuple or
relation subtype (meaning tuple or relation types that have specific
attribute lists defined for them).
