...
of any declared type).  A Sequence is considered dense, and all C<index>
values in one are numbered consecutively from 0 to 1 less than the count of
tuples, like array indices in typical programming languages.  A C<Sequence>
...
one is a C<Sequence(1..N) of Text>; one is a specially encoded C<Text>
...
C<Text> possrep is of the format C<(sys|nat|lex)\.rtn\..*>; it is for routine
names specifically.  Its default value is C<lex.rtn.>.
