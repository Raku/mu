...
of 1..N C<Text>, ordered from greatest to least significance; names of
attributes typically have just 1 part and so are conceptually just a
C<Text>, though often database attribute names, which are its component
...
possreps; one is a C<Sequence(1..N) of Text>; one is a specially encoded
C<Text> where parts are ordered from greatest to least signficance, and
adjacent parts are separated with a single "period" character, and any
literal period or backslash characters in parts are backslash-escaped as
C<\[pd]> and C<\[bh]> respectively (no other characters are escaped).  Its
default value consists of a single part that is the empty string.
