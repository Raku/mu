...
all the files included with this QDRDBMS distribution in its C<lib/>
directory.  This also includes its main test suite, since that is
implemented using modules in C<lib/> such as C<QDRDBMS::Validator>.  While
there are a few other files in this distribution, they are of relatively
little importance as far as copyright issues of the QDRDBMS core go, and
...
such would likely be part of a separate C<QDRDBMS::Cookbook> distribution.
