...
with the final version of that license (which is currently in the draft
stage; the final version should be released on Friday, June 29, 2007).

I<TODO:  Consider changing the terminology of the QDRDBMS to say "convey"
rather than "distribute" as per the GPL v3, after the latter is released
final, if doing so is reasonable.>
...
do since the other components are likely to be easily separatable from the
QDRDBMS components, as they are likely in separate files.>

These licensing terms are being applied to QDRDBMS mainly because the
copyright holder agrees with, and wants to support, the principles of user
freedom provided by use of the GPL.  In addition, this provides a better
foundation on which the copyright holder can have the option to
dual-license QDRDBMS, for a fee, to third parties that want to distribute
it in ways that the GPL doesn't allow (although any proprietary
alternatives would have a limited scope).
...
or Artistic or BSD licenses.  Doing so would be irrevokably granting the
legal means for anyone to make non-free derivitives, due to other licenses
having weaker or nonexistent copyleft protections; also, there would be
less incentive to pay for a dual-license by those that wouldn't want to
accept the GPL's terms.  However, workable and limited scope arrangements
can be made on a case by case basis if someone has different needs.
...
relicense QDRDBMS in the future, should that become necessary.  (But you
