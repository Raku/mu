...
Now, the next we will add are Abstract classes to our model. We will define an 
...
The last thing we will add to our object model are ThreadSafe classes. These are 
classes which will be guaranteed to be thread safe. And despite what you might 
...
An AbstractClass does not have a 'instance_var' slot since they are not allowed.
We also implement a 'add_method_to_class' method in our AbstractClass method_table which 
