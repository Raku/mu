...
Here is a diagram of the entire object environment, and each of it's subsets:
...
=item B<Every I<class> is itself an I<object>.>
...
=item to get all ordinart meta-classes do (C<B<M> - B<MM>>)
...
A I<Class> is itself just another data dictionary, however it's structure is a bit 
more complex. 
...
it's ancestors. These lists make it is possible to determine the instance variable and 
...
The C<instance_vars> or a I<Class> are basically those of an empty parent-less class, 
