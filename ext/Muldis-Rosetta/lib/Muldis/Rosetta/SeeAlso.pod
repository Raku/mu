...
This document is Muldis::Rosetta::SeeAlso version 0.4.4.
...
that could be created to work with Muldis Rosetta, but if so would most
likely be distributed independently from it.  The given package names are
examples, and the examples in question could have other names if
appropriate.

=over

=item C<Muldis::Rosetta::Cookbook(|::\w+)>

Pod files with a potentially large collection of examples, tutorials, FAQs,
and so on that help people learn how to employ Muldis Rosetta for the
things they need to do.  In particular, there should be a lot of help given
for people migrating from other systems that use different paradigms than
Muldis Rosetta does, for example SQL-using systems.

The Cookbook distro may also be the main repository of executing code that
would otherwise go in the /examples directory of the Muldis Rosetta core
distro.
...
that provide the C<new_machine> function and do the
...
line to get Muldis Rosetta bindings: L<Jifty>, L<Catalyst>, L<Maypole>,
...
L<DBIx::CopyRecord>, L<Data::Domain>, L<Fey>, L<SQL::Tokenizer>.
