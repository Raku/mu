...
This Muldis::DB core distribution is currently licensed under the GPL
version 3.0 or later.

Therefore, any third party component that links to Muldis::DB is required
to be distributed under the GPL unless at least one of the following
conditions is met:
...
and Muldis::DB separately.  This condition exists because the GPL is built
on copyright law and gives you privileges that vanilla copyright law does
not when you follow the GPL's terms; if you distribute Muldis::DB, which
vanilla copyright law says you can't do by default, then the terms of the
GPL would apply to you.
...
the entirety, of FLOSS written in Perl probably has a GPL-compatible
...
rather, the copyright holder intends to build a business with Muldis::DB in
...
sponsored improvements, and by using Muldis::DB as a platform to build
other works on; a unique selling proposition of the copyright holder for
this is that, being the creator, they know and can support the product
best.

The main reason that proprietary licenses are also offered for Muldis::DB
is that, for some potential users, various pragmatic or legal or business
...
concerns so that it is still possible for them to employ Muldis::DB to meet
