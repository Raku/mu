...
This document is Muldis::DB::Copying version 0.2.0.
...
complete set of container data types to represent Perl-hosted Abstract
Muldis D (L<Muldis::DB::Literal>); 3. Providing a parser and generator of
Concrete Muldis D (also L<Muldis::DB::Literal>); 4. Providing a complete
public API for the Muldis DB virtual machine (L<Muldis::DB::Interface>); 5.
Providing a complete but simple reference implementation of a Muldis DB
Engine (L<Muldis::DB::Engine::Example>); 6. Providing a complete reusable
test suite to help prove if any Engine implements Muldis D properly
(L<Muldis::DB::Validator>); 7. A test suite for parts of the aforementioned
...
I<Or that is the future intention; meanwhile, read "AGPL" as simply "GPL",
since that is the only choice which currently is legally useable.>
...
willing to consider switching to an LGPL licensed broad release instead, if
doing so would have strategic value to benefit the free software community,
and would increase the adoption of Muldis DB.
