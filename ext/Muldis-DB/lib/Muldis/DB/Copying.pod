...
and QDRDBMS separately.  This condition exists because the GPL is built on
copyright law and gives you privileges that vanilla copyright law does not
when you follow the GPL's terms; if you distribute QDRDBMS, which vanilla
copyright law says you can't do by default, then the terms of the GPL would
apply to you.
...
QDRDBMS and other FLOSS, there is probably no advantage for the QDRDBMS
license including a FLOSS exception.  The vast majority, if not the
entirety, of FLOSS written in Perl probably has a GPL-compatible license,
meaning that a FLOSS exception for them is not necessary to permit
...
QDRDBMS components, as they are likely in separate files.>

These licensing terms are being applied to QDRDBMS because the copyright
...
The copyright holder has little intention of broadly releasing QDRDBMS
itself under any free or open source license but the GPL, such as the Perl
or Artistic or BSD licenses.  Doing so would be irrevocably granting the
legal means for anyone to make non-free derivatives, due to other licenses
having weaker or nonexistent copyleft protections.

The copyright holder would prefer that it be possible for everyone to
employ QDRDBMS according to the GPL.  The GPL/proprietary dual-license is
not intended as a developer-extortion measure to build a business with;
rather, the copyright holder intends to build a business with QDRDBMS in
the same ways that the GPL permits everyone else to, such as by selling
GPL-licensed copies for a fee, and support services, and warranties, and
sponsored improvements, and by using QDRDBMS as a platform to build other
works on; a unique selling proposition of the copyright holder for this is
that, being the creator, they know and can support the product best.

The main reason that proprietary licenses are also offered for QDRDBMS is
that, for some potential users, various pragmatic or legal or business
concerns create an environment where it is not feasible for them to employ
GPL-licensed works, and so proprietary licenses should address these
concerns so that it is still possible for them to employ QDRDBMS to meet
their needs.  The copyright holder values having more users rather than
fewer, however the reasonable means for this to come about.

There should not be any rationale for rejecting QDRDBMS as a solution due
to its license(s); but if that assumption proves to be false, then workable
arrangements should still be possible on a case by case basis.
...
releases of the core modules.  This is done so that sole ownership of said
core modules can be maintained, and Darren can then still unilaterally
re-license QDRDBMS in the future, should that become necessary.  (But you
can still reuse your contributed code as you see fit in external projects.)
