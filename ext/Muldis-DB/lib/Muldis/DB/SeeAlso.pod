...
concrete language that qualifies as a D.  The TTM web site contains various
...
type inheritance what the earlier sections do with the Manifesto proper.
...
which is related to The Third Manifesto, and it was their main
...
=item C<Muldis::DB::AST::Refactor(|::\w+)>
...
=item C<Muldis::DB::AST::YAML> and C<Muldis::DB::AST::XML> etc
...
=item C<Muldis::DB::AST::StringRepr>

Utility module that parses the string representation format of Muldis D
...
=item C<Muldis::DB::AST::Translate::TutorialD> etc

Utility module that parses the Tutorial D language into Muldis::DB AST
...
could also handle compatible extensions by other D language
implementations.  In addition, separate utility modules can be made which
translate to and from other D languages that don't look like Tutorial D.

=item C<Muldis::DB::AST::Translate::SQL::Native>
...
=item C<Muldis::DB::AST::Translate::SQL::\w+>
...
=item C<Muldis::DB::AST::Translate::\w+>
...
that deal with I<D> languages or SQL, we can have utilities that translate
