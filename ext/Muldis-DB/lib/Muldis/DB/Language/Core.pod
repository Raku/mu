...
values, then C<min> returns a same-typed C<Maybe> with zero elements.  Note
that, while C<min> could conceivably have been defined instead to not
return a C<Maybe>, but rather to return the appropriate ordinal type's
concept of negative infinity, this option is avoided for now citing
