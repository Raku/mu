...
=item C<sys.rtn.Ordinal.is_inc of Bool (RO: Some.Ordinal $v1, Some.Ordinal
$v2)>
...
=item C<sys.rtn.Ordinal.is_dec of Bool (RO: Some.Ordinal $v1, Some.Ordinal
$v2)>
...
function throws an exception if C<$text> can't be mapped as specified.
...
C<contains> to results in C<Bool.True>; if either flag is C<Bool.False>, its
...
whose single element is which is the function's result.  This function
throws an exception if not all input values are of the same length in bits.
If C<$v> has zero values, then C<and> results in a same-typed C<Maybe> with
zero elements.  I<Note that, while C<and> could conceivably have been
defined instead to not result in a C<Maybe>, but rather to result in the
...
whose single element is which is the function's result.  This function
throws an exception if not all input values are of the same length in bits.
If C<$v> has zero values, then C<or> results in a same-typed C<Maybe> with
zero elements.  I<The note for C<and> applies to C<or> too.>
...
whose single element is which is the function's result.  This function
throws an exception if not all input values are of the same length in bits.
If C<$v> has zero values, then C<xor> results in a same-typed C<Maybe> with
zero elements.  I<The note for C<and> applies to C<xor> too.>
