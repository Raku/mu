...
is_deeply(
    [ map { ::dispatch($_, 'name') } ::dispatch(Vulcan->meta, 'class_precedence_list', 0, (':preorder')) ],
    [ qw(Vulcan Intelligent Sentient LifeForm Perl6::Object Humanoid BiPedal) ],
    '... got the right :preorder class precedence list');
    
# pre-order class precedence list
is_deeply(
    [ map { ::dispatch($_, 'name') } ::dispatch(Vulcan->meta, 'class_precedence_list', 0, (':breadth')) ],
...
is_deeply(
    [ map { ::dispatch($_, 'name') } ::dispatch(Vulcan->meta, 'class_precedence_list', 0, (':ascendant')) ],
...
is_deeply(
    [ map { ::dispatch($_, 'name') } ::dispatch(Vulcan->meta, 'class_precedence_list', 0, (':descendant')) ],
...
is_deeply(
    [ map { ::dispatch($_, 'name') } ::dispatch(F->meta, 'class_precedence_list', 0, (':ascendant')) ],
