...
sub alternation {
    ...
    return sub {
        ...
        while ( defined $state ) {
            ...
            $match = $$match if ref($match) eq 'Pugs::Runtime::LrepMatch';
            ### match: $match
            if ( $match->{state} ) {
                $state->[1] = $match->{state};
            }
...
sub concat {
    ...
    return sub {
        ...
        while (1) {
            ...
            $matches[0] = ${$matches[0]} if ref($matches[0]) eq 'Pugs::Runtime::LrepMatch';
            ### 1st match: $matches[0]
            ...
            $matches[1] = ${$matches[1]} if ref($matches[1]) eq 'Pugs::Runtime::LrepMatch';
            ### 2nd match: $matches[1]
            if ( ! $matches[1]{abort} ) {
                # die "not implemented";
...
sub rule_wrapper {
    my ( $str, $match ) = @_;
    # print "rule_wrapper $str - ", Dumper($match);
    $match = $$match if ref($match) eq 'Pugs::Runtime::LrepMatch';
    return unless $match->{bool};
    if ( $match->{return} ) {
        ...
        $match2{capture} = $match->{return}( 
            Pugs::Runtime::LrepMatch->new( $match ) 
        );
...
sub _preprocess_hash {
    ...
    if ( UNIVERSAL::isa( $h, 'Pugs::Compiler::LrepRule') ) {
